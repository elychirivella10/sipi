...
$pdf->SetFont('Arial','B',9);
$pdf->SetXY(14,114);$pdf->Cell(0,0,'SOLICITANTE(S):');
$pdf->SetFillColor(230);
...
$pdf->SetXY(54,110);$pdf->Multicell(140,4,utf8_decode($texto_solicitante.' '. $texto_caracter),0,'J');
...
if ($filas_found_regtit <> 0)  { 
   ...
   if ($letra=='V' or $letra=='E') {
       $titular=" ".$info['1']." Cédula N°".$cedula_sol."";}
        ...
        if (trim($cedula_sol)=='' or empty($cedula_sol) or $cedula_sol=='J000000000') {
           ...
           $titular=" ".$info['1']." Rif N°".$cedula_sol."";} }
}
   ...
   if ($filas_found_regtran <> 0)  { 
      $pdf->SetXY(18,188);$pdf->Multicell(170,4,utf8_decode($titular).' - Transferencia: '.utf8_decode($restran['transferencia']),0,'J');}
   else { $pdf->SetXY(18,188);$pdf->Multicell(170,4,utf8_decode($titular),0,'J');}
