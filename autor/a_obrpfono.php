...
if ($vopc==2) {
  ...
  if ($accion=="I") { //Comienzo de INCLUIR   
    ...
    $insert_str = "'$vder','$vsol','$fecha_solpf','$titulobr','PF','$vtraduccion','N','N','N','$cod_idioma',1,'$annopripubli','$p_origen','$n_ejemplares','$tipo_soporte','$vobservacion','$n_hojas_adic','$vdatos_ampli','$vdatos_adicio','$nplanilla'";
    ...
    if ($ins_obras) {
      ...
      Mensajenew("ERROR: Falla de Ingreso de Datos en la BD, Transacciones Abortadas, Error en datos asociados a: $error_obras ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); exit(); 
    }
...
datos_adicio='$vdatos_adicio',titulo_obra='$vtitulobr',traduccion='$vtraduccion',nplanilla='$nplanilla'";
  ...
  if ($accion=="I") { 
    Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","a_obrpfono.php?vopc=3&nconex=$n_conex&salir=1&conx=0","S");}
  else {
    Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","a_obrpfono.php?vopc=5&nconex=$n_conex&salir=1&conx=0","S"); }
  $smarty->display('pie_pag.tpl'); exit();
