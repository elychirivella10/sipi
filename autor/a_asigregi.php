...
$smarty ->assign('fechahoy',$fecha); 
   ...
   if (($usuario=='rmendoza') || ($usuario=='ngonzalez')) { }	 
     ...
     Mensajenew("ERROR: Usuario NO tiene Permiso para este modulo Temporalmente ...!!!","../index1.php","N");
   ...
   */ 
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
        ...
        mensajenew('ERROR AL PROCESAR LA BUSQUEDA','a_asigregi.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
        $smarty->display('encabezado1.tpl');
        mensajenew('NO EXISTEN DATOS ASOCIADOS','a_asigregi.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      ...
      if ($vesta<>410) {
        $smarty->display('encabezado1.tpl');
        mensajenew('Solo aplica para Solicitudes en Estatus 410','a_asigregi.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($filas_reg>0) {
        $smarty->display('encabezado1.tpl');
        mensajenew('Nro. de Registro YA EXISTE en la Base de Datos ...!!!','a_asigregi.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      ...
      $insert_valores="'$vder',795,'$vfechavig',410,0,'$vfec','$vuser','$vdese','$vcomen','$hh'";
      $ins_evento= $sql->insert("stdevtrd","$insert_campos","$insert_valores","");	 

      // Mensaje final
      if ($ins_obras and $ins_evento) { 
         //correo($nombre,$vemail,$vsol);
         pg_exec("COMMIT WORK");
         // Desconexion de la Base de datos 
         $sql->disconnect();
         
         $smarty->display('encabezado1.tpl');
         Mensajenew("DATOS GUARDADOS CORRECTAMENTE!!!","a_asigregi.php","S");
   ...
   $smarty->display('encabezado1.tpl');
   $smarty ->display('a_asigregi.tpl'); 
   $smarty->display('pie_pag.tpl');


function correo($nombre,$vemail,$vsol) {
 ////////
 $nombre="Nelson Gonzalez";
 $vemail="ngonzalez@sapi.gob.ve";
 ...
 $mail->Subject = "Notificaci&oacute;n de Aprobaci&oacute;n de Obra";
 
 $mail->AddAddress($vemail,$nombre);

 $body  = "<strong>Estimada(o): </strong>".$nombre." <br><br>";
 $body .= "Le informamos que la obra identificada con el n&uacute;mero de expediente: ".$vsol." a sido aprobada y debe proceder a efectuar el pago de Registro correspondiente.<br><br>";

 $body .= "Lo invitamos a visitar nuestra pagina Web: http://www.sapi.gob.ve, para obtener mayor informaci&oacute;n del procedimiento que usted debe realizar, el monto a cancelar y nuestras cuentas bancarias.<br><br>"; 
 
 $body .= "Tambi&eacute;n puede comunicarse con nosotros por la central telef&oacute;nica (0212) 481.64.78 / 484.29.07<br><br>";
