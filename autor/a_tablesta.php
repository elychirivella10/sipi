...
$smarty->assign('subtitulo','Mantenimiento de Estatus / '.$accion); 
if ($vopc==3) {
...
if ($vopc==1 && $accion=='Modificacion') {
  ...
  $resultado=pg_exec("SELECT * FROM stdstobr WHERE estatus='$estatus'");
  $filas_found=pg_numrows($resultado); 
  if ($filas_found==0) {
    mensajenew("NO EXISTEN DATOS ASOCIADOS ...!!!",'a_tablesta.php?vopc=4',"N");
...
if ($vopc==1 && $accion=='Ingreso') {
  ...
  $filas_found=pg_numrows($resultado);
  if ($filas_found>0) { 
    mensajenew("EL ESTATUS YA ESTA REGISTRADO...!!!",'a_tablesta.php?vopc=3',"N");
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }	 
...
if ($vopc==2) {
  ...
  //al Incluir
  if ($accion==1) {
    pg_exec("LOCK TABLE stdstobr IN SHARE ROW EXCLUSIVE MODE");
    $insert_cam = "estatus,descripcion";
    $insert_str = "$estatus,'$nombre'";
    $sql->insert("stdstobr","$insert_cam","$insert_str","");
  }
  //al Modificar
  if ($accion==2) {
    ...
    $sql->update("stdstobr","$update_str","estatus='$estatus'");
  }
  pg_exec("COMMIT WORK");
  ...
  if ($accion==1) {
    mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','a_tablesta.php?vopc=3','S');
    $smarty->display('pie_pag.tpl'); exit(); }
  else {
    mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','a_tablesta.php?vopc=4','S');
    $smarty->display('pie_pag.tpl'); exit(); }
}
