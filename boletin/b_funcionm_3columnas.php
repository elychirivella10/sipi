...
function Footer()
{
 ...
 //   $this->Cell(0,5,'________________________________________________________________',0,1,'J');
 //   $this->SetFont('Arial','I',8);
 //   $this->SetTextColor(128);
 //   $this->Cell(0,5,utf8_decode('Boletín de la Propiedad Industrial'.'                                                                                                                                                                           No. '.$numbol),0,0,'J');
...
// Montando los resultados en el formato boletin solicitadas
      ...
      $subtitle=$titul;
      $pdf->ChapterTitle('MARCAS SOLICITADAS',$subtitle);
      
      $pdf->Setfont('Arial','B',12);
      $pdf->MultiCell(59,4,'________________________',0,'J',0);
      $pdf->Setfont('Arial','B',8);
      $pdf->MultiCell(59,4,'REPUBLICA BOLIVARIANA DE VENEZUELA, MINISTERIO DEL PODER POPULAR PARA EL COMERCIO - SERVICIO AUTONOMO DE LA PROPIEDAD INTELECTUAL - REGISTRO DE LA PROPIEDAD INDUSTRIAL',0,'J',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(2); 
      $pdf->MultiCell(59,4,'Caracas, '.strtolower(Cambiar_fecha_mes($fec_soli)),0,'J',0);     $pdf->Setfont('Arial','B',8);
      $pdf->MultiCell(59,4,$anoi.' y '.$anof,0,'J',0);
      $pdf->MultiCell(59,4,utf8_decode('RESOLUCIÓN N° '.$nro_resoluc),0,'J',0);
      $pdf->ln(4); 
      $pdf->MultiCell(59,4,'MARCAS COMERCIALES SOLICITADAS',0,'C',0);
      $pdf->Setfont('Arial','B',8);
      $pdf->ln(4); 
      $pdf->MultiCell(59,4,utf8_decode('Esta Autoridad Administrativa, en cumplimiento a lo establecido en los articulos 76 y 77 de la Ley de Propiedad Industrial, una vez cumplidos los requisitos de Ley ordena la publicación en el Boletín: de las marcas, de productos y servicios, nombres y lemas comerciales solicitadas; a los efectos de que los interesados presenten sus observaciones, en un lapso de treinta (30) días hábiles a la publicación del presente Boletín, de acuerdo a lo establecido en el artículo 77.'),0,'J',0);
   ...
   for($cont=0;$cont<$filas_resultado;$cont++) { 
      ...
      $pdf->MultiCell(59,4,'________________________',0,'J',0);
      $pdf->Setfont('Arial','B',8);
      $pdf->MultiCell(59,4,'Insc. '. $registro['solicitud'].' del '.Cambiar_fecha_mes($registro['fecha_solic']),0,'J',0);
	...
	for($cont1=0;$cont1<$filas_found1;$cont1++)   { 
	   ...
	   $regt = pg_fetch_array($res_titular);
         ...
         $pdf->MultiCell(59,4,$texto.utf8_decode($titular),0,'J',0); 
		...
		if (file($nameimage)) {   
		   ...
		   if ($y >= 226) {
		      if ($x == 15) { 
       		  	 $pdf->MultiCell(59,38,$pdf->Image("$nameimage",(80+8),20,40,35,'JPG'),0,'J',0); }
		      if ($x == 80) {
		         $pdf->MultiCell(59,38,$pdf->Image("$nameimage",(145+8),20,40,35,'JPG'),0,'J',0); }
		      if ($x == 145) { 
		         $pdf->AddPage();
	                 $pdf->SetCol(0);
       		  	 $pdf->MultiCell(59,38,$pdf->Image("$nameimage",(15+8),20,40,35,'JPG'),0,'J',0); }
		   }
		   else {   
		   ...
		   //  $pdf->MultiCell(59,5,$x.'y: '.$y,0,'J',0); 
   		   ...
   		   $pdf->Setfont('Arial','B',8);
   		   $pdf->ln(4);  $numbol = $nbol;
   	           $pdf->MultiCell(59,4,trim(utf8_decode($registro['nombre'])),0,'C',0);  
	...
	$pdf->Setfont('Arial','',8);
        $pdf->MultiCell(59,4,'Para distinguir: '.trim(strtolower(utf8_decode($registro['distingue']))).' Clase '.$registro['clase'],0,'J',0);
	//busqueda del tramitante
	$tram = agente_tram($nagen,$registro['tramitante'],'1');
	$pdf->MultiCell(59,4,'TRAMITANTE: '. trim(utf8_decode($tram)),0,'J',0); 
  ...
  // Fin de Pagina (Firma del Registrador)
       $pdf->ln(2); 
       $pdf->Setfont('Arial','B',8);
       $pdf->MultiCell(59,5,'Total de Solicitudes : '.$total,0,'J',0);
       $pdf->Setfont('Arial','B',8);
       ...
       $pdf->ln(20); 
       $pdf->MultiCell(59,5,utf8_decode($registrador1),0,'C',0);
       $pdf->Setfont('Arial','B',7);
       $pdf->MultiCell(59,5,utf8_decode($registrador2),0,'C',0);
       $pdf->MultiCell(59,5,utf8_decode($registrador3),0,'C',0);       
       $pdf->MultiCell(59,5,utf8_decode($registrador4),0,'C',0); 
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado_prio;$cont++) { 
	...
	$data[4]['TEXT'] = trim(utf8_decode($registro_prio['titular'])).'. '.trim(utf8_decode($registro_prio['domicilio'])).'. '.trim(utf8_decode($registro_prio['nacionalidad']));
