...
if ($filas_resultado==0) {$mensaje= $mensaje.'* No se genero Solicitadas  ';} 
      ...
      $pdf->AddPage();
     // $pdf->Image('marca_agua.jpg',25,25,150,200,'JPEG');
// Montando los resultados en el formato boletin solicitadas
   ...
   for($cont=0;$cont<$filas_resultado;$cont++) { 
      ...
      $pdf->Setfont('Arial','B',8);
      $pdf->MultiCell(190,4,'Insc. '. $registro['solicitud'].' del '.Cambiar_fecha_mes($registro['fecha_solic']),0,'J',0);
	...
	for($cont1=0;$cont1<$filas_found1;$cont1++)   { 
	   ...
	   $regt = pg_fetch_array($res_titular);
         ...
         $pdf->MultiCell(190,4,$texto.utf8_decode($titular),0,'J',0); 
		...
		//   $x = $pdf->Getx();
		   ...
		   //      $pdf->Image('marca_agua.jpg',25,25,150,200,'JPEG');
	               //  $pdf->SetCol(0);
	                $pdf->MultiCell(190,30,$pdf->Image("$nameimage",(90),20,30,25,'JPG'),0,'J',0); 
	            //    $pdf->Image("$nameimage",90,20,30,25,'JPG');
		   }
		   else {		
		     $pdf->ln(1);
		     // $x = $pdf->Getx();
      	             $y = $pdf->Gety();
	             $pto = $y;
	             $pdf->Image("$nameimage",90,$y+3,30,25,'JPG');
		     $pdf->ln(30); }		
		}
                else {	
   		   $pdf->Setfont('Arial','B',8);
   		   $pdf->ln(3);
   		   $numbol = $nbol;
    	           $pdf->MultiCell(190,5,trim(utf8_decode($registro['nombre'])),0,'C',0);  
	...
	$pdf->Setfont('Arial','',8);
        $pdf->MultiCell(190,4,'Para distinguir: '.trim(strtolower(utf8_decode($registro['distingue']))).' Clase '.$registro['clase'],0,'J',0);
	//busqueda del tramitante
	$tram = agente_tram($nagen,$registro['tramitante'],'1');
	$pdf->MultiCell(190,4,'TRAMITANTE: '. trim(utf8_decode($tram)),0,'J',0); 
