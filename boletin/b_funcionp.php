...
//      $pdf->MultiCell(190,5,'SOLICITADAS DE '.utf8_decode($titulo),0,'C',0);
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('DE CONFORMIDAD CON EL ARTÍCULO 60 DE LA LEY DE PROPIEDAD INDUSTRIAL, Y POR CUANTO LOS INTERÉSADOS HAN CUMPLIDO DE ACUERDO A LA LEY CON LAS ÓRDENES DE PUBLICACIÓN EN PRENSA QUE SE HICIERA EN EL BOLETÍN DE LA PROPIEDAD INDUSTRIAL CORRESPONDIENTE, SE PROCEDE A PUBLICAR LAS SIGUIENTES SOLICITUDES DE REGISTRO DE '.$titulo.' CON EL FIN DE QUIEN TENGA LEGÍTIMO INTERÉS PUEDA PRESENTAR SUS OPOSICIONES DE ACUERDO A LO ESTABLECIDO EN EL ARTÍCULO 63 DE LA LEY DE PROPIEDAD INDUSTRIAL.'),0,'J',0);      
...
if ($filas_resultado==0) {$mensaje= $mensaje.'  - No se genero Devueltas '.$titulo;} 
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(2); 
      $pdf->MultiCell(190,5,'Caracas, '.strtolower(Cambiar_fecha_mes($fecp_devfon)),0,'J',0);           
      $pdf->Setfont('Arial','B',8);
      $pdf->MultiCell(190,5,$anoi.', '.$anof.' y '.$anor,0,'J',0);
      //$pdf->MultiCell(190,5,$anoi.' y '.$anof,0,'J',0);
      $pdf->MultiCell(190,5,utf8_decode('RESOLUCIÓN N° '.$nro_resoluc),0,'J',0);
      ...
      $pdf->MultiCell(190,5,utf8_decode(trim($titp_devfon)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('VISTAS LAS SOLICITUDES DE PATENTE DE '.$subpate.', QUE A CONTINUACIÓN SE ESPECIFICAN, Y POR CUANTO LOS INTERÉSADOS NO CUMPLIERON CON LOS REQUISITOS TÉCNICO-JURÍDICOS DE FONDO, SE DEVUELVEN DICHAS SOLICITUDES A FIN DE QUE SE DE CUMPLIMIENTO A LO EXIGIDO EN LOS CORRESPONDIENTES OFICIOS DE DEVOLUCIÓN DENTRO DE UN LAPSO DE TREINTA (30) DÍAS HÁBILES CONTADOS A PARTIR DE LA FECHA DE PUBLICACIÓN DEL PRESENTE BOLETÍN, DE CONFORMIDAD CON LO ESTABLECIDO EN EL ARTÍCULO 61 DE LA LEY DE PROPIEDAD INDUSTRIAL. DE NO DARSE CABAL CUMPLIMIENTO A LO SOLICITADO EN EL OFICIO DE DEVOLUCIÓN, SE PROCEDERÁ A DECLARAR EXTINGUIDA LA PRIORIDAD DE LA SOLICITUD.'),0,'J',0);
      $pdf->ln(4); 
...
// Montando los resultados en el formato boletin solicitadas
      ...
      //if($counter==4) {
      if($counter==3) {
        $pdf->MultiCell(190,5,utf8_decode('CUMPLIDOS COMO HAN SIDO LOS EXTREMOS LEGALES EXIGIDOS EN EL ARTÍCULO 65 DE LA LEY DE PROPIEDAD INDUSTRIAL, ESTE DESPACHO ACUERDA POR EL TÉRMINO CORRESPONDIENTE, EL REGISTRO DE LAS SOLICITUDES DE PATENTE DE MODELO INDUSTRIAL QUE A CONTINUACIÓN SE MENCIONAN; ENTENDIENDOSE QUE LA PROTECCIÓN QUE CONFIERE DICHO REGISTRO HA DE RECAER EXCLUSIVAMENTE SOBRE EL MODELO REPRESENTADO EN LAS FIGURAS CONSIGNADAS.'),0,'J',0);      
      } else {
        if($counter==5) {
          $pdf->MultiCell(190,5,utf8_decode('CUMPLIDOS COMO HAN SIDO LOS EXTREMOS LEGALES EXIGIDOS EN EL ARTÍCULO 65 DE LA LEY DE PROPIEDAD INDUSTRIAL, ESTE DESPACHO ACUERDA POR EL TÉRMINO CORRESPONDIENTE, EL REGISTRO DE LAS SOLICITUDES DE PATENTE DE DIBUJO INDUSTRIAL QUE A CONTINUACIÓN SE MENCIONAN; ENTENDIENDOSE QUE LA PROTECCIÓN QUE CONFIERE DICHO REGISTRO HA DE RECAER EXCLUSIVAMENTE SOBRE EL DIBUJO REPRESENTADO EN LAS FIGURAS CONSIGNADAS.'),0,'J',0);      
        } else {
          $pdf->MultiCell(190,5,utf8_decode('CUMPLIDOS COMO HAN SIDO LOS EXTREMOS LEGALES EXIGIDOS EN EL ACUERDO SOBRE LOS ASPECTOS DE LOS DERECHOS DE PROPIEDAD INTELECTUAL RELACIONADOS CON EL COMERCIO (ACUERDO SOBRE LOS ADPIC) Y EN EL ARTÍCULO 65 DE LA LEY DE PROPIEDAD INDUSTRIAL, ESTE DESPACHO ACUERDA POR EL TÉRMINO CORRESPONDIENTE, EL REGISTRO DE LAS SOLICITUDES DE PATENTE '.utf8_decode($titulo).' QUE A CONTINUACIÓN SE MENCIONAN.'),0,'J',0);
        }  
      }
      $pdf->ln(4); 
...
//****************************************************************************************
//  PRIORIDAD EXTINGUIDA X EXAMEN DE FONDO
//****************************************************************************************
if($priopfon==1) {
$counter= 1;
$tipo_derecho='A';
...
//Eliminacion de todo el Estatus 25 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '2032' AND boletin= '$boletin'");

$resultadopais=pg_exec("SELECT b.nro_derecho, b.solicitud, c.boletin, c.estatus, 'P' as tipo, d.pais_domicilio, d.nacionalidad,'V' as tipo_nacionalidad
    INTO temp stztmpext
    FROM  stzderec b, stztmpbo c, stzottid d
    WHERE c.estatus='2032'
     AND c.boletin = '$boletin'
     AND c.tipo = 'P'
     AND c.nro_derecho = b.nro_derecho
        AND c.nro_derecho = d.nro_derecho
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Prioridades Extinguidas '.$titulo;} 
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(2); 
      $pdf->MultiCell(190,5,'Caracas, '.strtolower(Cambiar_fecha_mes($fecp_priofon)),0,'J',0);           
      $pdf->Setfont('Arial','B',8);
      $pdf->MultiCell(190,5,$anoi.', '.$anof.' y '.$anor,0,'J',0);
      //$pdf->MultiCell(190,5,$anoi.' y '.$anof,0,'J',0);
      $pdf->MultiCell(190,5,utf8_decode('RESOLUCIÓN N° '.$nro_resoluc),0,'J',0);
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('SOLICITUDES DE PATENTE NACIONALES '.$titulo.' DECLARADAS CON PRIORIDAD EXTINGUIDA POR EXAMEN DE FONDO'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($titp_priofon)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('VISTAS LAS SOLICITUDES DE PATENTE DE '.$subpate.', QUE A CONTINUACIÓN SE ESPECIFICAN, Y POR CUANTO LOS INTERÉSADOS NO CUMPLIERON CON LOS REQUISITOS TÉCNICO-JURÍDICOS DE FONDO EXIGIDOS EN LOS RESPECTIVOS OFICIOS DE DEVOLUCIÓN, SE DECLARA EXTINGUIDA LA PRIORIDAD DE LAS MENCIONADAS SOLICITUDES, DE CONFORMIDAD CON LO ESTABLECIDO EN EL ARTÍCULO 61 DE LA LEY DE PROPIEDAD INDUSTRIAL.'),0,'J',0);
      $pdf->ln(4); 
...
}//fin de PRIORIDAD EXTINGUIDA X EXAMEN DE FONDO
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Perimidas por no Publicación en Prensa '.$titulo;} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('PERIMIDAS NACIONALES POR NO PUBLICAR EN PRENSA '.$titulo),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($titp_peri)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      //$pdf->MultiCell(190,5,utf8_decode('De conformidad con el artículo 64 de la Ley Orgánica de Procedimientos Administrativos y por cuanto los INTERÉSados no cumplieron de acuerdo a la Ley con las Ordenes de Publicación en Prensa que se hiciera en el Boletín de la Propiedad Industrial correspondiente, se procede a declarar la perención de las siguientes solicitudes de registro de patentes.'),0,'J',0);
      $pdf->MultiCell(190,5,utf8_decode('DE CONFORMIDAD CON EL ARTÍCULO 64 DE LA LEY ORGÁNICA DE PROCEDIMIENTOS ADMINISTRATIVOS Y POR CUANTO LOS INTERÉSADOS NO CUMPLIERON DE ACUERDO A LA LEY CON LAS ORDENES DE PUBLICACIÓN EN PRENSA QUE SE HICEIRA EN EL BOLETÍN DE LA PROPIEDAD INDUSTRIAL CORRESPONDIENTE, SE PROCEDE A DECLARAR LA PERENCIÓN DE LAS SIGUIENTES SOLICITUDES DE REGISTRO DE PATENTES.'),0,'J',0);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Negadas '.$titulo;} 
    ...
    $pdf->Setfont('Arial','B',12);
    $pdf->ln(4); 
    //$pdf->MultiCell(190,5,utf8_decode('SOLICITUDES DE PATENTE DE INVENCIÓN '),0,'C',0);
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '2752' AND boletin= '$boletin'");
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Solicitudes Sin Efecto x No Pago de Derechos ';} 
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('POR CUANTO LAS SOLICITUDES DE PATENTE, QUE A CONTINUACIÓN SE ESPECIFICAN, CONCEDIDAS EN LOS BOLETINES DE LA PROPIEDAD INDUSTRIAL Nº 585, 587, 589, 590, 592, 594, 595 Y 596 NO CONSIGNARON DENTRO DEL PLAZO LEGALMENTE ESTABLECIDO LOS DERECHOS DE REGISTRO CORRESPONDIENTES, ESTA OFICINA DE REGISTRO DECLARA SIN EFECTO LA RESOLUCIÓN QUE ACUERDA LA EXPEDICIÓN DE LAS MISMAS, DE CONFORMIDAD CON LO DISPUESTO EN EL ARTÍCULO 65 DE LA LEY DE LA PROPIEDAD INDUSTRIAL.'),0,'J',0);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron OPOSICIÓNes ' .$titulo;} 
else { $nro_resoluc = $nro_resoluc+1;
      $pdf->AddPage();
      $pdf->Setfont('Arial','B',12);
      $pdf->MultiCell(190,5,'_______________________________________________________________________________',0,'J',0);
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      //$pdf->MultiCell(190,5,utf8_decode('Se hace del conocimiento de los usuarios e INTERÉSados en las solicitudes de patente, que a continuación se señalan, publicadas como solicitadas en el Boletín de la Propiedad Industrial, que deben comparecer por ante este Servicio Autónomo a fin de retirar copia fotostática de el/los escritos de oposición que le fue formuladas a dichas solicitudes, a efectos que se notifiquen y contesten las mismas si lo estimaren conveniente en un lapso de treinta (30) días hábiles, contados a partir de la fecha, de conformidad con el artículo 63 de la Ley de Propiedad Industrial.'),0,'J',0);
      //$pdf->MultiCell(190,5,utf8_decode('Se hace conocimiento de los usuarios e INTERÉSados y/o apoderados de las solicitudes de Registro de las patentes detallados a continuación; que las mismas fueron objetadas, en consecuencia deberán comparecer por esta oficina a retirar la copia fotostática de el escrito(s) de oposición(es) que le fue formulada a la solicitud , a efectos de informarse de aquella en el plazo de quince (15) días hábiles a contar desde la presente publicación. Vencido dicho plazo los INTERÉSados cuentan con un lapso de quince (15) días hábiles, para la contestación, a fin de que ejerzan su derecho a la defensa, de acuerdo a los establecido en el artículo 63 de la Ley de Propiedad Industrial, G. O. N° 25.227 del 10/12/1956.'),0,'J',0);
      $pdf->MultiCell(190,5,utf8_decode('SE HACE DEL CONOCIMIENTO DE LOS USUARIOS E INTERÉSADOS Y/O APODERADOS DE LAS SOLICITUDES DE REGISTRO DE LAS PATENTES DETALLADAS A CONTINUACIÓN; QUE LAS MISMAS FUERON OBJETADAS, EN CONSECUENCIA DEBERAN COMPARECER POR ESTA OFICINA A RETIRAR LA COPIA FOTOSTÁTICA DE EL ESCRITO(S) DE OPOSICIÓN (ES) QUE LE FUE FORMULADA A LA SOLICITUD, A EFECTOS DE INFORMARSE DE AQUELLA EN EL PLAZO DE QUINCE (15) DÍAS HÁBILES A CONTAR DESDE LA PRESENTE PUBLICACIÓN.  VENCIDO DICHO PLAZO LOS INTERÉSADOS CUENTAN CON UN LAPSO DE QUINCE DÍAS HÁBILES, PARA LA CONTESTACIÓN, A FIN DE QUE EJERZAN SU DERECHO A LA DEFENSA, DE ACUERDO A LO ESTABLECIDO EN EL ARTÍCULO 63 DE LA LEY DE PROPIEDAD INDUSTRIAL, G.O.N° 25.227 DEL 10/12/1956.'),0,'J',0);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Patentes en Rehabilitacion '.$titulo;} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('REHABILITACIÓN DE PATENTES '.$titulo),0,'C',0);
   ...
   $data[3]['TEXT'] = trim(utf8_decode($tram)); 
	...
	$pdf->Draw_Data($data);
      ...
      // Fin de Pagina (Firma del Registrador)
       ...
       $pdf->Setfont('Arial','',8);
       $pdf->ln(8); 
       $pdf->MultiCell(190,5,utf8_decode('SE INFORMA A LOS TRAMITANTES E INTERÉSADOS EN LOS REGISTROS MENCIONADOS ANTERIORMENTE, QUE PODRÁN EJERCER EL RECURSO ADMINISTRATIVO ESTABLECIDO EN EL ARTÍCULO 94 DE LA LEY ORGÁNICA DE PROCEDIMIENTOS ADMINISTRATIVOS; DENTRO DE UN LAPSO DE QUINCE (15) DÍAS SIGUIENTES, A PARTIR DE LA PRESENTE PUBLICACIÓN.'),0,'J',0);
       //$pdf->MultiCell(190,5,utf8_decode('SE INFORMA A LOS TRAMITANTES E INTERÉSADOS EN LAS SOLICITUDES DE REGISTROS MENCIONADOS ANTERIORMENTE, QUE PODRÁN EJERCER EL RECURSO ADMINISTRATIVO ESTABLECIDO EN EL ARTÍCULO 94 DE LA LEY ORGÁNICA DE PROCEDIMIENTOS ADMINISTRATIVOS; DENTRO DE UN LAPSO DE QUINCE (15) DÍAS SIGUIENTES, A PARTIR DE LA PRESENTE PUBLICACIÓN.'),0,'J',0);
       $pdf->Setfont('Arial','B',8);
       $pdf->ln(5); 
       $pdf->MultiCell(190,5,utf8_decode('Publíquese,'),0,'L',0);
