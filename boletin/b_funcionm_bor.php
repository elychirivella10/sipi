...
function Header()
{
     ...
     $this->SetY(10);
     $fechahoy = hoy();
     $this->Cell(0,5,utf8_decode('Boletín de la Propiedad Industrial                     '.$fechahoy.'                                                                                                                   No. '.$numbol),0,0,'J');
     $this->SetY(8);
...
//$pdf->AliasNbPages();
//Armando el query
  $resultado=pg_exec("SELECT stzderec.solicitud,stzderec.nombre,stmmarce.clase, stzderec.nro_derecho, stzderec.poder, stzderec.tramitante, stzderec.agente
...
//      $nagen=$registro['agente'];
	...
	//busqueda del tramitante
        $nagen=$registro['agente'];
	$poder = $registro['poder'];
	$tram = agente_tramp1($nagen,$registro['tramitante'],$poder);
...
//  DESISTIDAS X LEY
//****************************************************************************************
//if($desi_ley==1) { Modificado por ROMULO MENDOZA el 13/09/2011 ya que no salia por tener variable que no era OJO 
if($desi_ley==1) {
$nro_resoluc = $nro_resoluc+1;
$resultado=pg_exec("SELECT b.solicitud, b.nombre,b.nro_derecho,b.agente, b.tramitante, a.clase	
	FROM  stmmarce a, stzderec b, stztmpbo c
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Desistidas de Observacion por Ley';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('OPOSICIONES DESISTIDAS POR LEY'),0,'C',0);
