...
if ($vopc==2) {
  ...
  if ($accion=="I") { //Comienzo de INCLUIR   
    ...
    if ($nfil>0) {
      mensajenew("AVISO: Número de Boletín $nbol ya existe en la Base de Datos ...!!!","b_newbol.php?vopc=3","N");
      $smarty->display('pie_pag1.tpl'); $sql->disconnect(); exit(); }
...
if (empty($fecp_derp)) {pg_exec("update stzdetbol SET fecp_derp=NULL WHERE nro_boletin= '$nbol'"); }
  ...
  if ($accion=="I") { 
    Mensajenew("DATOS GUARDADOS CORRECTAMENTE...!","b_newbol.php?vopc=3&nconex=$n_conex&salir=1&conx=0","S");}
  else {
    Mensajenew("DATOS GUARDADOS CORRECTAMENTE...!","b_newbol.php?vopc=5&nconex=$n_conex&salir=1&conx=0","S"); }
  $smarty->display('pie_pag1.tpl'); exit();
