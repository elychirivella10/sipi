...
// Montando los resultados en el formato boletin solicitadas
     ...
     // $pdf->MultiCell(190,5,'_______________________________________________________________________________',0,'J',0);
      ...
      $pdf->Setfont('Arial','B',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('Esta Autoridad Administrativa, en cumplimiento a lo establecido en los articulos 76 y 77 de la Ley de Propiedad Industrial, una vez cumplidos los requisitos de Ley ordena la publicación en el Boletín de la Propiedad Industrial de marcas: comerciales, denominaciones comerciales y lemas comerciales solicitadas; a los efectos de que los interesados presenten sus observaciones, en un lapso de treinta (30) días hábiles a la publicación del presente Boletín, de acuerdo a lo establecido en el artículo 77.'),0,'J',0);
   ...
   for($cont=0;$cont<$filas_resultado;$cont++) { 
	...
	for($cont1=0;$cont1<$filas_found1;$cont1++)   { 
       ...
       //  $pdf->MultiCell(135,4,$texto.utf8_decode($titular),0,'J',0); 
        $texto_titular= "[b]SOLICITADA POR:   "."[/b] ".utf8_decode($titular);
	...
	$tram = agente_tram($nagen,$registro['tramitante'],'1');
        ...
        if ($poder !='') {
         echo  "entre";
     	   $res_poder = pg_exec("SELECT stzagenr.nombre FROM stzpoder, stzpohad, stzagenr 
  				WHERE stzpoder.poder= '$registro[poder]'
  				AND stzpohad.poder = stzpoder.poder
  				AND stzpohad.poderhabi = stzagenr.agente ");
           $fil_poder=pg_numrows($res_poder);
	   for($conp=0;$conp<$fil_poder;$conp++)   { 
	       $regpoder = pg_fetch_array($res_poder);
               $poderhab= $poderhab.trim($regpoder['nombre']).' - ';            
           }
          $pdf->JLCell("$poderhab",135,'j');	
        }
        else { $pdf->JLCell("$texto_tramitante",135,'j');}      
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Desistidas de Observacion por Ley';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('OBSERVADAS DESISTIDAS POR LEY'),0,'C',0);
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado_prio;$cont++) { 
	...
	$data[3]['TEXT'] = trim(utf8_decode($registro_prio['titular1']));	
        ...
        $data[5]['TEXT'] = trim(utf8_decode(strtoupper($registro_prio['domicilio']))).' - '.$pais_nombre;
        $data[6]['TEXT'] = trim(utf8_decode($registro_prio['tramitante']));
	$registro_prio = pg_fetch_array($resultado);
	$pdf->Draw_Data($data);
