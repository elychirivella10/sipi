...
// Coordinación de Informática / Dirección de Soporte Administrativo / SAPI / MPPEF / MPPCN
// Desarrollado Año: 2010
// Modificado Año ... 2017, 2018, 2019
// *************************************************************************************
 ...
 $registrador1= "";
 $registrador2= "LIGIA CRISTINA RODRÍGUEZ";
 ...
 $vnomsolmarn= "boletin_sol_mar_nac"."_".$fechagen."_".$horagen.".pdf";
...
// Montando los resultados en el formato boletin solicitadas
      ...
      $pdf->MultiCell(190,5,utf8_decode('MARCAS NACIONALES SOLICITADAS A EFECTO DE OPOSICIÓN'),0,'C',0);
      $pdf->ln(6); 
      //$pdf->MultiCell(190,5,'_______________________________________________________________________________',0,'J',0);
      $pdf->Setfont('Arial','B',8);
      //$pdf->MultiCell(190,5,'REPÚBLICA BOLIVARIANA DE VENEZUELA, MINISTERIO DEL PODER POPULAR DE INDUSTRIAS Y PRODUCCION NACIONAL - SERVICIO AUTÓNOMO DE LA PROPIEDAD INTELECTUAL - REGISTRO DE LA PROPIEDAD INDUSTRIAL',0,'J',0);
      $pdf->MultiCell(190,5,'REPÚBLICA BOLIVARIANA DE VENEZUELA, '.$ministerio.' - SERVICIO AUTÓNOMO DE LA PROPIEDAD INTELECTUAL - REGISTRO DE LA PROPIEDAD INDUSTRIAL',0,'J',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(2); 
...
//      $pdf->MultiCell(190,5,'MARCAS COMERCIALES SOLICITADAS',0,'C',0);
   ...
   //busqueda de la descripcion de la etiqueta
   if (($modalidad == 'M') OR ($modalidad == 'G')) {
      $etiqueta='';
      $res_etiqueta = pg_exec("SELECT descripcion FROM stmlogos WHERE stmlogos.nro_derecho='$nderec'");
      $filas_etiqueta=pg_numrows($res_etiqueta);
      $reg_etiqueta = pg_fetch_array($res_etiqueta);
      $etiqueta=trim(mb_strtoupper(utf8_decode($reg_etiqueta['descripcion'])));
      $texto_etiqueta= "[b]DESCRIPCIÓN DE ETIQUETA:   "."[/b] ".$etiqueta;
...
$tipo_derecho='M';
$indrec==0;
$titulo='SOLICITUDES DE MARCAS NACIONALES DE PRODUCTOS CONCEDIDAS';
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '$estatus' AND boletin= '$boletin'");
...
// Armando el query segun las opciones
$resultado=pg_exec("SELECT b.solicitud, b.nombre,b.nro_derecho,b.agente, b.tramitante, b.poder, b.fecha_solic, b.tramitewebpi, a.modalidad,a.clase 
	FROM  stmmarce a, stzderec b, stztmpbo1 c
	WHERE c.estatus='$estatus'
    	AND c.boletin = '$boletin'
   	AND c.tipo = 'M'
  ...
  //$resultado=pg_exec("SELECT b.solicitud, b.nombre,b.nro_derecho,b.agente, b.tramitante, ,b.poder ,b.fecha_solic, b.tramitewebpi, a.clase,a.modalidad
...
//       $pdf->MultiCell(190,5,utf8_decode($registrador5),0,'C',0);         
  ...
  $counter = $counter + 1; 
  if($counter==2) { $tipo_derecho='N'; $titulo='SOLICITUDES DE NOMBRES COMERCIALES NACIONALES CONCEDIDAS';}
  if($counter==3) { $tipo_derecho='L'; $titulo='SOLICITUDES DE LEMAS COMERCIALES NACIONALES CONCEDIDAS';}
  if($counter==4) { $tipo_derecho='S'; $titulo='SOLICITUDES DE MARCAS DE SERVICIOS NACIONALES CONCEDIDAS';}
  if($counter==5) { $tipo_derecho='C'; $titulo='SOLICITUDES DE MARCAS COLECTIVAS NACIONALES CONCEDIDAS ';}
  if($counter==6) { $tipo_derecho='O'; $titulo='SOLICITUDES DE DENOMINACIÓN DE ORIGEN NACIONALES CONCEDIDAS';}
  if($counter==7) { $tipo_derecho='M'; $estatus=1390; $indrec=1; $titulo='RECLASIFICADAS MARCAS DE PRODUCTOS NACIONALES CONCEDIDAS';}
  if($counter==8) { $tipo_derecho='N'; $estatus=1390; $indrec=1; $titulo='RECLASIFICADAS NOMBRE COMERCIALES NACIONALES CONCEDIDAS';}
  if($counter==9) { $tipo_derecho='L'; $estatus=1390; $indrec=1; $titulo='RECLASIFICADAS LEMAS COMERCIALES NACIONALES CONCEDIDAS'; }
  if($counter==10) { $tipo_derecho='S'; $estatus=1390; $indrec=1; $titulo='RECLASIFICADAS MARCAS DE SERVICIOS NACIONALES CONCEDIDAS';}
  if($counter==11) { $tipo_derecho='C'; $estatus=1390; $indrec=1; $titulo='RECLASIFICADAS MARCAS COLECTIVAS NACIONALES CONCEDIDAS';}
  if($counter==12) { $tipo_derecho='O'; $estatus=1390; $indrec=1; $titulo='RECLASIFICADAS DENOMINACIÓN DE ORIGEN NACIONALES CONCEDIDAS'; }
  if($counter==13) { $tipo_derecho='D'; $titulo='SOLICITUDES DE DENOMINACION COMERCIAL NACIONALES CONCEDIDAS';}
}//fin del while
 //echo " fin del while ";
 //Salida del Reporte
 //$pdf->Output("../../boletin/boletin_con.pdf");        
 $vnomconmar= "boletin_con_mar_nac"."_".$fechagen."_".$horagen.".pdf";
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1002' AND boletin= '$boletin'");
...
// Armando el query segun las opciones
$resultado=pg_exec("SELECT b.solicitud, b.nombre, b.nro_derecho, b.fecha_solic, b.agente, b.tramitante, b.poder, b.tramitewebpi, a.modalidad,a.clase, a.distingue 	
	FROM  stmmarce a, stzderec b, stztmpbo1 c
	WHERE c.estatus='1002'
    	AND c.boletin = '$boletin'
   	AND c.tipo = 'M'
...
//	ORDER BY b.solicitud");	
  ...
  if ($filas_resultadoc==0) { $mensaje= $mensaje.' - No se genero Orden de Publicación  '; } 
    ...
    for($cont=0;$cont<$filas_resultadoc;$cont++) { 
      ...
      if (($modalidad == 'M') OR ($modalidad == 'G')) {
        ...
        $reg_etiqueta = pg_fetch_array($res_etiqueta);
        $etiqueta=trim(mb_strtoupper(utf8_decode($reg_etiqueta['descripcion'])));
        $texto_etiqueta= "[b]DESCRIPCION DE ETIQUETA:   "."[/b] ".$etiqueta;
        $pdf->JLCell("$texto_etiqueta",135,'j');        
      }
 ...
 $vnomprenmar= "boletin_pren_mar_nac"."_".$fechagen."_".$horagen.".pdf";
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1102' AND boletin= '$boletin'");

// Armando el query
$resultadopais=pg_exec("SELECT b.nro_derecho, b.solicitud, c.boletin, c.estatus, 'M' as tipo, d.pais_domicilio, d.nacionalidad,'V' as tipo_nacionalidad
    INTO temp stztmpneg
    FROM  stmmarce a, stzderec b, stztmpbo c, stzottid d
    WHERE c.estatus='1102'
    AND c.boletin = '$boletin'
    AND c.tipo = 'M'
    AND c.nro_derecho = b.nro_derecho
    AND c.nro_derecho = a.nro_derecho
...
$resultado1 = pg_exec("INSERT INTO stztmpbo1
  ...
  $resultado=pg_exec("SELECT stmmarce.clase,stmmarce.modalidad, stztmpbo1.solicitud, stmliaor.*, stzderec.nombre,stzderec.fecha_solic,stzderec.tramitante,stzderec.agente,stzderec.poder,stzderec.tramitewebpi
			FROM stmmarce,stmliaor, stztmpbo1, stzderec
			WHERE stztmpbo1.estatus = '1102'
			AND stztmpbo1.boletin = '$boletin'
			AND stztmpbo1.tipo = 'M'
			AND stmliaor.articulo = '$articulo'
			AND stztmpbo1.tipo_nacionalidad = 'V'
			AND stmmarce.nro_derecho = stztmpbo1.nro_derecho
			AND stztmpbo1.nro_derecho = stmliaor.nro_derecho
			AND stztmpbo1.nro_derecho = stzderec.nro_derecho
			ORDER BY stztmpbo1.solicitud");
...
$resultado1 = pg_exec("INSERT INTO stztmpbo1
  ...
  for($contl=1;$contl<=$tope;$contl++) {
    ...
    $resultado=pg_exec("SELECT stmmarce.clase,stmmarce.modalidad, stztmpbo1.solicitud, stmliaor.*, stzderec.nombre,stzderec.fecha_solic,stzderec.tramitante,stzderec.agente,stzderec.poder,stzderec.tramitewebpi
			FROM stmmarce, stzderec, stztmpbo1, stmliaor
			WHERE stztmpbo1.estatus = '1102'
			AND stztmpbo1.boletin = '$boletin'
			AND stztmpbo1.tipo = 'M'
 ...
 //Salida del Reporte
 $vnomnegmar= "boletin_neg_mar_nac"."_".$fechagen."_".$horagen.".pdf";
...
// Montando los resultados en el formato boletin orden de publicacion
      ...
      $pdf->MultiCell(190,5,mb_strtoupper(utf8_decode('ESTE DESPACHO DE CONFORMIDAD CON LO ESTABLECIDO EN EL ARTÍCULO 76 DE LA LEY DE PROPIEDAD INDUSTRIAL, ORDENA LA PUBLICACIÓN EN PRENSA, DE LAS SOLICITUDES DE MARCAS COMERCIALES, LEMAS COMERCIALES Y DENOMINACIONES COMERCIALES QUE A CONTINUACIÓN SE ESPECIFICAN, A TALES EFECTOS SE LES INFORMA QUE PODRÁN REALIZAR DICHA PUBLICACIÓN EN LOS PERIODICOS: VEA, CIUDAD CARACAS Y A TRAVÉS DEL PERIODICO DIGITAL DE ESTE SERVICIO AUTÓNOMO (SAPI). ESTA PUBLICACIÓN EL INTERESADO DEBERÁ REALIZARLA DE FORMA CORRECTA Y DE ACUERDO A LO SOLICITADO DENTRO DE DOS (2) MESES A PARTIR DE LA VIGENCIA DEL PRESENTE BOLETÍN; DE NO CUMPLIR CON LO DISPUESTO SE DECLARARAN PERIMIDAS LAS SOLICITUDES, DE ACUERDO A LO ESTABLECIDO EN EL ARTÍCULO 64 DE LA LEY ORGANICA DE PROCEDIMIENTOS ADMINISTRATIVOS.')),0,'J',0);
      $pdf->ln(4); 
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1200' AND boletin= '$boletin'");
...
//      $pdf->MultiCell(190,5,utf8_decode('DEVUELTAS DE FORMA POR REQUISITOS MINIMOS'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode('DEVUELTAS DE FORMA NACIONALES'),0,'C',0);
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1116' AND boletin= '$boletin'");

// Armando el query
$resultadopais=pg_exec("SELECT b.nro_derecho, b.solicitud, c.boletin, c.estatus, 'M' as tipo, d.pais_domicilio, d.nacionalidad,'V' as tipo_nacionalidad
    INTO temp stztmpbodfd
    FROM  stmmarce a, stzderec b, stztmpbo c, stzottid d
...
if ($filas_resultadod==0) {$mensaje= $mensaje.' - No se genero Devueltas de Fondo';} else {
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('DEVUELTAS DE FONDO NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_devu)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1003' AND boletin= '$boletin'");

// Armando el query
$resultadopais=pg_exec("SELECT b.nro_derecho, b.solicitud, c.boletin, c.estatus, 'M' as tipo, d.pais_domicilio, d.nacionalidad,'V' as tipo_nacionalidad
    INTO temp stztmpbobs
    FROM  stmmarce a, stzderec b, stztmpbo c, stzottid d
...
$resultado=pg_exec("SELECT DISTINCT a.solicitud, a.nro_derecho, a.nombre, a.agente, a.tramitante, b.clase, a.poder, c.comentario, a.tramitewebpi 
                    INTO temp tmpobsleg
                    FROM stzderec a, stmmarce b, stzevtrd c, stzottid d, stztmpbo e  
                    WHERE a.estatus IN (1120,1124)  
                     AND a.tipo_mp='M' 
                     AND c.evento=1040  
                     AND e.boletin = '$boletin'
                     AND (c.nro_derecho=a.nro_derecho)
                     AND (c.nro_derecho=b.nro_derecho)
                     AND (c.nro_derecho=d.nro_derecho)
...
// Armando el query segun las opciones
$resultado=pg_exec("SELECT DISTINCT a.solicitud, a.nro_derecho, a.nombre, b.clase, a.agente, a.tramitante, a.poder, a.tramitewebpi, c.comentario
	FROM  stmmarce a, stzderec b, stztmpbo1 c, stzevtrd d
	WHERE c.estatus='1003'
    AND  c.boletin = '$boletin'
    AND  c.tipo = 'M'
    AND  c.tipo_nacionalidad = 'V'
...
//Eliminacion de todo el Estatus 6 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1914' AND boletin= '$boletin'");
...
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1025' AND boletin= '$boletin'");

// Armando el query
$resultadopais=pg_exec("SELECT b.nro_derecho, b.solicitud, c.boletin, c.estatus, 'M' as tipo, d.pais_domicilio, d.nacionalidad,'V' as tipo_nacionalidad
    INTO temp stztmpbopex
    FROM  stmmarce a, stzderec b, stztmpbo c, stzottid d
...
// Armando el query segun las opciones
$resultado=pg_exec("SELECT distinct on (solicitud) b.solicitud, b.nombre, b.nro_derecho, b.agente, b.tramitante, b.poder, a.clase, b.tramitewebpi
	FROM  stmmarce a, stzderec b, stztmpbo1 c
	WHERE c.estatus='1025'
    	AND c.boletin = '$boletin'
   	AND c.tipo = 'M'
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Prioridades Extinguidas';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('SOLICITUDES DE MARCAS NACIONALES DECLARADAS CON PRIORIDAD EXTINGUIDA'),0,'C',0);
...
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1023' AND boletin= '$boletin'");
...
//Eliminacion de todo el Estatus 24 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1024' AND boletin= '$boletin'");
...
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1030' AND boletin= '$boletin'");
...
//Eliminacion de todo el Estatus 750 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1750' AND boletin= '$boletin'");
...
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1910' AND boletin= '$boletin'");
...
//Eliminacion de todo el Estatus 125 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1125' AND boletin= '$boletin'");
...
//Eliminacion de todo el Estatus 130 de stztmpbo1 
$delbol1    = pg_exec("DELETE FROM stztmpbo1 WHERE estatus= '1130' AND boletin= '$boletin'");

// Armando el query
$resultadopais=pg_exec("SELECT b.nro_derecho, b.solicitud, c.boletin, c.estatus, 'M' as tipo, d.pais_domicilio, d.nacionalidad,'V' as tipo_nacionalidad
    INTO temp tmpdesobsder
    FROM  stmmarce a, stzderec b, stztmpbo c, stzottid d
...
//****************************************************************************************
//  NOTIFICACION DE CANCELACION POR FALTA DE USO QUE RATIFICARON por BOLETIN 588
//****************************************************************************************
if($noti==1) {
// Anteriormente 
...
$resultado=pg_exec("SELECT DISTINCT ON(a.registro) a.nro_derecho,a.solicitud,a.registro,a.tramitewebpi,trim(a.nombre) as marca,trim(d.nombre) as titular,c.comentario, e.domicilio, e.nacionalidad, e.pais_domicilio
from stzderec a,stmmarce b,stzevtrd c,stzsolic d,stzottid e,stztmpbor f
where f.boletin = '$boletin' and
      f.estatus = '1830' and 
      c.evento = '1301' and
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Notificaciones de Cancelación por Falta de Uso';} 
      ...
      $pdf->MultiCell(190,5,utf8_decode('NOTIFICACIÓN'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_desi_mejo)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      ...
      $pdf->MultiCell(190,5,mb_strtoupper(utf8_decode('Se notifica a los titulares de las marcas que se citan a continuación, que deberán comparecer ante el Servicio Autónomo de Propiedad Intelectual, específicamente al área de taquilla integral de atención al usuario, dentro de un plazo de quince (15) días hábiles, contados a partir de la presente notificación en el Boletín de la Propiedad Industrial, a los fines de retirar el escrito de solicitud de CADUCIDAD DE MARCA POR NO USO, RATIFICADAS CON OCASIÓN AL BOLETÍN DE LA PROPIEDAD INDUSTRIAL 588 DE FECHA 14 DE NOVIEMBRE DE 2018, que cursa ante este Registro de la Propiedad Industrial, siendo que, transcurrido dicha oportunidad procesal, tendrá un lapso de veinticinco (25) días hábiles para presentar sus alegatos de defensa y las pruebas que considere pertinentes para desvirtuar tal solicitud. Notificación que se realiza de conformidad con el artículo 73 en concordancia con el artículo 47 eiusdem de la Ley de Procedimientos Administrativos, en armonía con lo dispuesto en el artículo 36 literal "d" de la Ley de Propiedad Industrial y en la Resolución Administrativa Nro. 358 de fecha 30 de enero de 2020.')),0,'J',0);
      $pdf->ln(4); 
...
//****************************************************************************************
//  NOTIFICACION DE CANCELACION POR FALTA DE USO NORMALES NO PUBLICADAS EN BOL 588
//****************************************************************************************
if($noti==1) {
...
where f.boletin = '$boletin' and
      f.estatus = '1830' and 
      c.evento = '1301' and
      (c.fecha_event>='15/01/2019' AND c.fecha_event<='31/01/2020') AND   
      a.nro_derecho = f.nro_derecho and
      a.nro_derecho = b.nro_derecho and
      a.nro_derecho = c.nro_derecho and
      a.nro_derecho = e.nro_derecho and
      d.titular = e.titular AND
      a.nro_derecho NOT IN (SELECT nro_derecho FROM stzevtrd WHERE evento = 1195)
      ORDER BY registro");
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Notificaciones de Cancelación por Falta de Uso';} 
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,mb_strtoupper(utf8_decode('Se notifica a los titulares de las marcas que se citan a continuación, que deberán comparecer ante el Servicio Autónomo de Propiedad Intelectual, específicamente al área de taquilla integral de atención al usuario, dentro de un plazo de quince (15) días hábiles, contados a partir de la presente notificación en el Boletín de la Propiedad Industrial, a los fines de retirar el escrito de solicitud de CADUCIDAD DE MARCA POR NO USO, realizados éstos desde el 15 de enero de 2019 y el 31 de enero de 2020, que cursa ante este Registro de la Propiedad Industrial, siendo que, transcurrido dicha oportunidad procesal, tendrá un lapso de veinticinco (25) días hábiles para presentar sus alegatos de defensa y las pruebas que considere pertinentes para desvirtuar tal solicitud. Notificación que se realiza de conformidad con el artículo 73 en concordancia con el artículo 47 eiusdem de la Ley de Procedimientos Administrativos, en armonía con lo dispuesto en el artículo 36 literal "d" de la Ley de Propiedad Industrial y en la Resolución Administrativa Nro. 358 de fecha 30 de enero de 2020.')),0,'J',0);
      $pdf->ln(4); 
...
}//Fin de  Notificacion de cancelacion por falta de uso 2

//****************************************************************************************
//  NOTIFICACION DE CANCELACION POR FALTA DE USO NOTIFICADAS CON RATIFICACION PUBLICADAS EN BOL 588 EVENTO 196
//****************************************************************************************
if($noti==1) {
...
where f.boletin = '$boletin' and
      ...
      a.nro_derecho = e.nro_derecho and
      d.titular = e.titular AND
      a.nro_derecho IN (SELECT nro_derecho FROM stzevtrd WHERE evento = 1196)
      ORDER BY registro");
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Notificaciones de Cancelación por Falta de Uso';} 
      ...
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,mb_strtoupper(utf8_decode('Se notifica a los titulares de las marcas que se citan a continuación, que deberán comparecer ante el Servicio Autónomo de Propiedad Intelectual, específicamente al área de taquilla integral de atención al usuario, dentro de un plazo de veinticinco (25) días hábiles, contados a partir de la presente notificación en el Boletín de la Propiedad Industrial, a los fines de presentar sus alegatos de defensa y las pruebas que considere pertinentes para desvirtuar la solicitud de CADUCIDAD DE MARCA POR NO USO, RATIFICADAS CON OCASIÓN AL BOLETÍN DE LA PROPIEDAD INDUSTRIAL 588 DE FECHA 14 DE NOVIEMBRE DE 2018, que cursa ante este Registro de la Propiedad Industrial. Notificación que se realiza de conformidad con el artículo 73 en concordancia con el artículo 47 eiusdem de la Ley de Procedimientos Administrativos, en armonía con lo dispuesto en el artículo 36 literal "d" de la Ley de Propiedad Industrial y en la Resolución Administrativa Nro. 358 de fecha 30 de enero de 2020.')),0,'J',0);
      $pdf->ln(4); 
...
}//Fin de  Notificacion de cancelacion por falta de uso 3
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Registros de Anotaciones Marginales de Renovación';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('RENOVACIONES DE MARCAS Y OTROS SIGNOS DISTINTIVOS NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_anot)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,mb_strtoupper(utf8_decode('Vista las solicitudes de RENOVACION, presentada por los interesados, y cumplidos como han sido los requisitos establecidos en la Ley de Propiedad Industrial; este Despacho en ejercicio de las atribuciones que le confiere el artículo 42 literales "b" y "j" de la Ley de Propiedad Industrial Nro. 25.227 del 10 de diciembre 1956, notifica las siguientes RENOVACIONES:')),0,'J',0);
      $pdf->ln(4); 
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado_prio;$cont++) { 
  ...
  $pdf->ln(4); 
  $pdf->MultiCell(190,5,mb_strtoupper(utf8_decode('Se deja constancia que por error involuntario en el presente listado aparecen solicitudes que fueron publicadas en el Boletín de la Propiedad Industrial Nº 595, en relación a las mismas se deja constancia que la notificación del acto administrativo objeto de las mismas tuvo lugar en el Boletín de la Propiedad Industrial supra mencionado, y los lapsos legales relacionados con dichas solicitudes comenzaron a correr a partir de la vigencia del mismo.')),0,'J',0);
  // Fin de Pagina (Firma del Registrador)
  $pdf->ln(8); 
  $pdf->Setfont('Arial','B',8);
  $pdf->MultiCell(190,5,'Total de Solicitudes : '.$total_prio,0,'J',0);
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Registros de Anotaciones Marginales de Cambio de Nombre';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('CAMBIO DE NOMBRE DE MARCAS Y OTROS SIGNOS DISTINTIVOS NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_anot)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Registros de Anotaciones Marginales de Cesiones';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('CESIONES DE MARCAS Y OTROS SIGNOS DISTINTIVOS NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_anot)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Registros de Anotaciones Marginales de Fusiones';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('FUSIONES DE MARCAS Y OTROS SIGNOS DISTINTIVOS NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_anot)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Registros de Anotaciones Marginales de Cambio de Domicilio';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('CAMBIO DE DOMICILIO DE MARCAS Y OTROS SIGNOS DISTINTIVOS NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_anot)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
...
if ($filas_resultado_prio==0) {$mensaje= $mensaje.' - No se generaron Registros de Anotaciones Marginales de Licencias';} 
      ...
      $pdf->Setfont('Arial','B',12);
      $pdf->ln(4); 
      $pdf->MultiCell(190,5,utf8_decode('LICENCIAS DE USO DE MARCAS Y OTROS SIGNOS DISTINTIVOS NACIONALES'),0,'C',0);
      $pdf->MultiCell(190,5,utf8_decode(trim($tit_anot)),0,'C',0);
      $pdf->Setfont('Arial','',8);
      $pdf->ln(4); 
...
$vnommar= "boletin_mar_nac"."_".$fechagen."_".$horagen.".pdf";
