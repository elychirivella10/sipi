...
$subtitulo = "Consulta Interna de Indicaciones Geogr&aacute;fica";
$fecha   = trim(fechahoy());
$sql = new mod_db();
...
$fecha   = trim(fechahoy());
$subtitulo = "Consulta Interna de Indicaciones Geogr&aacute;ficas";
?>
...
if ($vopcion==1)
   {$resultado=pg_exec("SELECT * FROM stzderec WHERE solicitud='$varsol' and tipo_mp='I' ");
   }

if ($vopcion==2)
   {$resultado=pg_exec("SELECT * FROM stzderec WHERE registro='$varreg' and tipo_mp='I' ");
   }
...
$res_mod=pg_exec("SELECT * FROM stiindig WHERE nro_derecho='$reg[nro_derecho]' ");
...
if ($reg[tipo_derecho]=='I') {$vtip='INDICACION GEOGRAFICA';}
...
<?php echo "<td width='83%' colspan='2' class='celda2'>$reg[nombre]</td>"; ?>
  ...
  <tr>
    <td class="celda-titulo">Productos Designados:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[productodesig]</td>"; ?>
  </tr>
  <tr>
    <td class="celda-titulo">Zona Geografica:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[zonageograf]</td>"; ?>
  </tr>
  <tr>
    <td class="celda-titulo">Tratado Internacional:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[tratadoint]</td>"; ?>
  </tr>
  <tr>
    <td class="celda-titulo">Descripcion del Producto:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[distingue]</td>"; ?>
  </tr>
  <tr>
    <td class="celda-titulo">Descripcion de la Zona:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[describezona]</td>"; ?>
  </tr>
  <tr>
    <td class="celda-titulo">Vinculo Producto-Territorio:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[vinculo]</td>"; ?>
  </tr>
  <tr>
    <td class="celda-titulo">Observaciones:</td>
<?php echo "<td width='83%' colspan='2' class='celda2'>$resmod[observacion]</td>"; ?>
  </tr>
  <tr>
...
$resultado = pg_exec("SELECT stzottid.titular, stzsolic.nombre, stzottid.nacionalidad, stzottid.domicilio, stzottid.pais_domicilio, stzsolic.indole, stzsolic.identificacion
       FROM stzottid, stzsolic WHERE stzottid.nro_derecho='$nderec' AND stzsolic.titular = stzottid.titular");
   $filas_found=pg_numrows($resultado);
   $reg = pg_fetch_array($resultado);
...
<p align="center"><b><font size="4" face="Tahoma">Representante(s) Legal(es)</font></b></p>
<table border="0" width="100%" class="celda1">
  <tr>
    <td class="columna-titulo">C&oacute;digo</td>
    <td class="columna-titulo">Nombre</td>
...
$resultado = pg_exec("SELECT stirepid.titular, stzsolic.nombre, stirepid.nacionalidad, stirepid.domicilio, stirepid.pais_domicilio, stzsolic.indole, stzsolic.identificacion
       FROM stirepid, stzsolic WHERE stirepid.nro_derecho='$nderec' AND stzsolic.titular = stirepid.titular");
   $filas_found=pg_numrows($resultado);
   $reg = pg_fetch_array($resultado);
   for($cont=0;$cont<$filas_found;$cont++) 
     {
...
<table border="0" width="100%" class="celda1">
  ...
  <?php
   ...
   for($cont=0;$cont<$filas_found;$cont++) 
     { 
	$evento= $reg[evento]-3000;
	$estatus_ant = $reg[estat_ant]-3000;
	$boletin = $reg[documento];
     ...
     if (($evento==122) AND (($estatus_ant==200) ) AND ($boletin>=602)) { 
       if ($estatus_ant==200) { 
         $archivodev = "../documentos/devueltas/indicacionesg/forma/boletin".$boletin."/".$varsol1.$varsol2.".pdf"; } 
       else {
         $archivodev = "../documentos/devueltas/indicacionesg/fondo/boletin".$boletin."/".$varsol1.$varsol2.".pdf"; }         
     //if (is_file($name)) {
