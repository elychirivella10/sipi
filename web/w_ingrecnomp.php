...
function planilla(v1,v2,v3) {
  open("w_planillafp06.php?vsol="+v1.value+"&vtramt="+v2.value+"&vopc=","Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
//MYSQL
//SELECT fac_num,fac_fecha,dtalle1_cantidad_ser,ser_id,fac_descripcion INTO OUTFILE '/tmp/fichero_salida.csv' FIELDS TERMINATED BY ','
//FROM sfa_factura,sfa_dtalles_1 WHERE fac_descripcion like 'Tramite #:%' AND fac_estado !='2'
//                  and sfa_factura.fac_id=sfa_dtalles_1.fac_id
//                  and sfa_dtalles_1.ser_id in ('FM05','FM06','FM07')
//                  and fac_fecha > '2019-05-31'
//ORDER BY 1

//POSTGRESQL
//SELECT * FROM "public"."stzfactram" WHERE "nro_factura" = 'F0123456'
...
$smarty->assign('subtitulo','Ingreso de Cambio de Nombre o Titular de Patente al SIPI');
...
//if (($usuario!='rmendoza') AND ($usuario!='ngonzalez') AND ($usuario!='agil')) {	 
...
<p>&nbsp;</p>
<p align='center'><b><font > Cambios de Nombre Asociados al Tramite No.: <?php echo $vtramt; ?> para Ingresar al SIPI </font></b></p>
...
//         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[solic_sipi]</td>";
         ...
         echo "<td width='14% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER><a href='w_planillafp06.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>$registro_tram[solicitud]</a></td>";
//         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[estatus]</td>";
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingrecnomp.php?vopc=3","N");
    ...
    $resultado_tram = pg_exec("SELECT * FROM stzsolcam, stztramite 
    			 	...
    			 	AND stztramite.estatus_tra IN ('13','14','09','10','15','02') ORDER BY stzsolcam.nro_referencia");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingrecnomp.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus_sol']<>'15') {echo "<a href='w_planillafp06.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
...
//*** Guardar la solicitud de CAMBIO DE NOMBRE en el SIPI  
if ($vopc== 6) {
    $vpag=$_POST['vfact'];
    ...
    if ($vdoc=='' or $vdoc=='0000000000' or $vdoc=='0' or empty($vdoc)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de Documento del Reloj asociado al Cambio de Nombre ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if ($vcarp=='' OR $vcarp=='000000' OR $vcarp=='0000000000' OR $vcarp=='0' OR empty($vcarp)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de la Carpeta asociada al Cambio de Nombre ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if (empty($fecha_reno)) {
      mensajenew('ERROR: La Fecha del Cambio de Nombre esta vacia ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
...
if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       //Datos del Detalle 
       $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id='0203' AND fac_id=$fac_id"); 
       $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Cambio de Nombre de Patentes ...!!!','javascript:history.back();','N');
         $smarty->display('pie_pag.tpl'); exit(); }
    ...
    $sql = new mod_db();
    $sql->connection();
    $resultado_tram=pg_exec("SELECT * FROM stzevder WHERE evento=2209");
    ...
    if ($filas_cfound1==0) {
      mensajenew('ERROR: Nro. de Tr&aacute;mite de Cambio de Nombre NO presenta ning&uacute;n Titular o Solicitante ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
...
//    $sql1->disconnect1();
    ...
    $vcomenta= 'Cambio de Nombre presentado en fecha: '.$fecha_reno.", TITULAR NUEVO: ".$cesionario.', WEBPI Tramite No: '.$vtramt.' y Referencia: '.$vsol.'.  S/'.$comenpagotasa;

    $vcomenta= $vcomenta.' '.$vcomadi;
    //echo " $vcomenta "; exit();
    
    // Insertar evento 2209 en stzevtrd
    ...
    $insert_valores ="'$vnder','2209','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    $insert_valores ="$vnder,$vdoc,'$vnsol','P',2209,$codigo,'$titular','$domicilio','$pais','',0,'N','$cesionario',0";
    ...
    $insert_str = "'$nfac','$fac_fe',0,0,0,'1','N',$vcant_am";  
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','P','$usuario','$fechahoy','$horactual','$vcarp','FP06'";    
    ...
    $sql1 = new mod_db();
    $sql1->connection1();    
    $actualiza=pg_exec("UPDATE stzsolcam SET estatus_sol='15',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where nro_referencia ='$vsol' AND nro_tramite='$vtramt'");             

      $resultado_cant = pg_exec("SELECT * FROM stzsolcam where nro_tramite='$vtramt' ");
      $filas_cant = pg_numrows($resultado_cant); 
      $resultado_estatus = pg_exec("SELECT * FROM stzsolcam where nro_tramite='$vtramt' and estatus_sol = '15'");
      ...
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingrecnomp.php?vopc=4&vtramt='.$vtramt,'S');
...
<p>&nbsp;</p>
<p align='center'><b><font > Cambio de Nombre o Titular de Patente Asociadas al Tramite para Ingresar al SIPI </font></b></p>
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  <tr>
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus']== 15) {
         ...
         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[estatus]</td>";
         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER>$registro_tram[solic_sipi]</td>";
         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER><a href='w_formsipi.php?vsol=$registro_tram[solic_sipi]''>FP-06</a></td>";
         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[fecha]</td>";
         echo "<td width='12%'  style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[hora]</td>";
         }
     else {
...
$smarty->display('w_ingrecnomp.tpl');
$smarty->display('pie_pag.tpl');
//ob_end_clean(); 
?>
