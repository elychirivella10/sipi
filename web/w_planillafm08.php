...
$smarty->assign('subtitulo','Ingreso de Cambio de Nombre o Titular de Marcas al SIPI');
...
if ($vopc==4) {
    ...
    $resultado_tram = pg_exec("SELECT * FROM stmsolces, stztramite
    			 	WHERE stmsolces.nro_tramite = '$vtramt'  
    			 	AND  stmsolces.nro_referencia = '$vsol'
...
//   $resultadosoltra=pg_exec("select a.nro_tramite,a.solicitud,a.nombre,a.tipo_derecho,b.clase,c.clase_nac,d.usuario from stzderec a,stmmarce b,stmclnac c,stztramite d where a.nro_tramite='$vtramt' and a.solicitud='$vsol' and a.nro_tramite=b.nro_tramite and a.solicitud=b.solicitud and a.nro_tramite=c.nro_tramite and a.solicitud=c.solicitud and a.nro_tramite=d.nro_tramite order by a.nro_tramite,a.solicitud");
   $resultadosoltra=pg_exec("select a.nro_tramite,a.nro_referencia,a.nombre,a.registro,a.nro_solicitud,a.tipo_derecho,a.clase,a.clase_nac,a.ind_clase,a.poder,b.usuario,b.f_tramite from stmsolces a,stztramite b 
where a.nro_tramite='$vtramt' and a.nro_referencia='$vsol' and a.nro_tramite=b.nro_tramite order by a.nro_tramite,a.nro_referencia");
   ...
   //}
   if ($vcansol<1) {
     mensajenew('AVISO: Nro. de Tramite No Registrado!, Verifique ...!!!','../web/w_ingrecnomm.php?vopc=3','N');
     $smarty->display('pie_pag.tpl'); exit(); 
   }
...
$smarty->display('w_ingrefm08.tpl');
