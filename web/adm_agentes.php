...
if ((($tper==2 or $tper==3) and ($vced=='000000000' or empty($vced) or $vced==' ' or $vced==0) and ($vpas=='000000000' or empty($vpas) or $vpas==' ' or $vpas==0)) or 
   ...
   echo "<tr><td>";
   echo "<p align='center'><b>Introduzca primero la identificaci&oacute;n del Agente/Tramitante que desea buscar</b>";
...
if ($tper=='1' or $tper=='' or $tper=='3') {
   $res_tab = pg_exec("select * from stztmpage where nro_tramite=$vtra and solicitud=$vsol and tipo_per='2'");
} else {
   $vcedula=$lced.$vced;
   $res_tab = pg_exec("select * from stztmpage where nro_tramite=$vtra and solicitud=$vsol and (tipo_per<>'2' or (agente=0 and identificacion<>'$vcedula'))"); }
...
if ($vage>0 and ($tper=='1' or empty($tper))) {
   ...
   if ($filas_found>0) {
      ...
      $sql= new mod_db();
      $sql->connection(); 
      $res_tab = pg_exec("select * from stztmpage where agente=$vage and tipo_per='1' order by nro_tramite desc");
      $rega = pg_fetch_array($res_tab); 
      $filas_found=pg_numrows($res_tab); 
      if ($filas_found>0) {
...
if ($tper=='1' or empty($tper) or $tper=='2' or $tper=='3') { 
   ...
   if ($tper==2 or $tper==3){$tnom='Nombre Completo:';
                 ...
                 $res_tab = pg_exec("select * from stztmpage where identificacion='$vide' order by nro_tramite desc");
                 $rega = pg_fetch_array($res_tab); 
                 $filas_found=pg_numrows($res_tab); 
                 if ($filas_found>0) {
                    $vnom=trim($rega[nombre]);
                    ...
                    $res_tab = pg_exec("select * from stztmptit where identificacion='$vide' order by nro_tramite desc");
                    $rega = pg_fetch_array($res_tab); 
                    $filas_found=pg_numrows($res_tab); 
                    if ($filas_found>0) {
                       $vnom=trim($rega[nombre]);
   ...
   echo "<form action='z_gbagente.php' name='formtitular' method='post'>";
