   ...
   if ($grabarcorrecto=='SI') {
      ...
      if ($vtip==1) {
         ...
         if (pg_numrows($resul_verif)>0) {  
            $valido=$sql->del("stztmpage","nro_tramite='$vtra' and solicitud='$vsol' and agente='$vage' and tipo_mp='$vtmp'");
         ...
         if (pg_numrows($resul_verif)>0) {  
            $valido=$sql->del("stztmpage","nro_tramite='$vtra' and solicitud='$vsol' and identificacion='$vced' and tipo_mp='$vtmp'");
            if (!$valido) {$cant_error=$cant_error+1;}
         }
      }
      ...
      $insert_campo="nro_tramite,solicitud,agente,identificacion,nombre,domicilio,nacionalidad,
                  telefono1,telefono2,fax,email,tipo_mp,fecha_carga,email2,poder,tipo_per,genero,discapacitado";
      $insert_valor="$vtra,$vsol,$vage,'$vced','$vnom','$vdom','$vnac',
                  '$vtel','$vcel','$vfax','$vcor','$vtmp','$fechahoy','$vcor2','$vpod','$vtip','$vgen','$vdis'";
...
if ($vact=='Eliminar') {
   ...
   if ($vtip=='') {$vtip='1';}
   if ($vtip=='1') {
       $valido=$sql->del("stztmpage","nro_tramite='$vtra' and solicitud='$vsol' and agente='$vage' and tipo_mp='$vtmp' and tipo_per='$vtip'");
   } else {
       $valido=$sql->del("stztmpage","nro_tramite='$vtra' and solicitud='$vsol' and identificacion='$vide' and tipo_mp='$vtmp' and tipo_per='$vtip'");
   }
}
