...
$smarty->assign('subtitulo','Ingreso de Renovacion Marca al SIPI');
...
<p>&nbsp;</p>
<p align='center'><b><font > Renovaciones Asociadas al Tramite No.: <?php echo $vtramt; ?> para Ingresar al SIPI </font></b></p>
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingreren.php?vopc=3","N");
    ...
    $resultado_tram = pg_exec("SELECT * FROM stmsolren, stztramite 
    			 	WHERE stztramite.nro_tramite = '$vtramt' 
    			 	AND stmsolren.nro_tramite = stztramite.nro_tramite
    			 	AND stztramite.estatus_tra IN ('13','14','09','10','15','02') order by stmsolren.solicitud");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingreren.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
       ...
       //
       $resul_insert =pg_exec("INSERT INTO consulta (tramite,solicitud,estatus,solic_sipi,fecha,hora) VALUES ('$vtramt','$registro_tram[solicitud]','$registro_tram[estatus_sol]','$registro_tram[nro_solicitud]','','')");
			    
     $varsolic=trim($registro_tram['solicitud']);
     echo "<tr>";
     echo "<td style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER >";
     if ($registro_tram['estatus_sol']<>'15') {echo "<a href='w_planillafm04.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
...
//*** Guardar la solicitud de RENOVACION en el SIPI  
if ($vopc== 6) {
    $vpag=$_POST['vfact'];
    ...
    if ($vdoc=='' or $vdoc=='0000000000' or $vdoc=='0' or empty($vdoc)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de Documento del Reloj asociado a la Renovaci&oacute;n ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if ($vcarp=='' OR $vcarp=='000000' OR $vcarp=='0000000000' OR $vcarp=='0' OR empty($vcarp)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de la Carpeta asociada a la Renovaci&oacute;n ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if (empty($fecha_reno)) {
      mensajenew('ERROR: La Fecha de la Renovacion esta vacia ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
...
//if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Renovaci&oacute;n de Marcas ...!!!','javascript:history.back();','N');
         $smarty->display('pie_pag.tpl'); exit(); }
       ...
       $query_tasas=pg_exec("SELECT nro_factura FROM stmfactura WHERE nro_factura='$nfac' and tipo_anotamar='R'");
       $filas_tasas = pg_numrows($query_tasas);    
...
//    $sql1->disconnect1();
    ...
    $vcomenta= 'Presentada en fecha: '.$fechahoy.', en Clase Int: '.$vcla.' y Clase Nac: '.$vcln.', WEBPI Tramite No: '.$vtramt.' y Referencia: '.$vsol.'.  S/'.$comenpagotasa;
    ...
    $insert_valores ="'$vnder','1204','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    $insert_str = "'$nfac','$fac_fe',0,0,0,'1','R',$vcant_am";  
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','M','$usuario','$fechahoy','$horactual','$vcarp','FM04'";    
    ...
    $sql1 = new mod_db();
    $sql1->connection1();    
    $actualiza=pg_exec("UPDATE stmsolren SET estatus_sol='15',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where solicitud ='$vsol' AND nro_tramite='$vtramt'");             

      $resultado_cant = pg_exec("SELECT * FROM stmsolren where nro_tramite='$vtramt' ");
      $filas_cant = pg_numrows($resultado_cant); 
      $resultado_estatus = pg_exec("SELECT * FROM stmsolren where nro_tramite='$vtramt' and estatus_sol = '15'");
      ...
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingreren.php?vopc=4&vtramt='.$vtramt,'S');
