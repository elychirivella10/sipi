...
<head>
  ...
  function isnomvalida(who,formulario,campo) {
    if (campo.value.length<3) { alert('¡ Nombre no Valido ...!!! Coloque al menos 3 caracteres');   
       campo.value=''; campo.focus(); return false }
  return
  }
...
if ($tper<>4 or ($tper==4 and $vrad<>'')) {
   ...
   if ($vrad=='') {$res_tab = pg_exec("select * from stzsolic where identificacion='$vide'");}
   else           {$res_tab = pg_exec("select * from stzsolic where titular='$vrad'");}
   ...
   if ($filas_found>0) {
      ...
      $vgen=trim($rega[genero]);
      $vdis=trim($rega[discapacitado]);
   } else {
   ...
   else {$res_tab = pg_exec("select * from stztmptit where titular='$vide' order by nro_tramite desc");}
   $rega = pg_fetch_array($res_tab); 
   $filas_found=pg_numrows($res_tab); 
   if ($filas_found>0) {
      $vtit=trim($rega[titular]);
   ...
   echo "<form action='z_gbtitular.php' name='formtitular' method='post'>";
   ...
   echo "</tr>";
   if ($tper<>2 and $tper<>3 and $tper<>4) {
      $vgm=''; $vgf=''; 
      if ($vgen=='F') { $vgf='selected'; }
      if ($vgen=='M') { $vgm='selected'; }
      echo "<tr>";
      echo "<td class='der8-color'>Genero:</td>";
      echo "<td>";
      echo "<select size='1' name='vgen'>";
      echo "<option value='F' $vgf>Femenino </option>";   
      echo "<option value='M' $vgm>Masculino</option>";   
      echo "<font face='Arial' color='#800000' size='3' valign='up'>*</font></td></tr>";
      echo "<tr>";
      echo "<td class='der8-color'>Discapacitado:</td>";
      echo "<td>";
      $vds=''; $vdn=''; 
      if ($vdis=='N') { $vdn='selected'; }
      if ($vdis=='S') { $vds='selected'; }
      echo "<select size='1' name='vdis'>";
      echo "<option value='N' $vdn>No</option>";   
      echo "<option value='S' $vds>Si</option>";   
      echo "<font face='Arial' color='#800000' size='3' valign='up'>*</font></td></tr>";
   }
   echo " <tr>";
...
if ($tper==4 and $vrad=='') {
   ...
   $vgen='';
   //if ($vrad=='') {$res_tab = pg_exec("select * from stztmptit where identificacion='$vide' order by nro_tramite desc");}
   if ($vrad=='') {$res_tab = pg_exec("select * from stztmptit where nombre='$vtex' order by nro_tramite desc");}
   else {$res_tab = pg_exec("select * from stztmptit where titular='$vide' order by nro_tramite desc");}
   $rega = pg_fetch_array($res_tab); 
   $filas_found=pg_numrows($res_tab); 
   if ($filas_found>0) {
...
//   echo "<p align='center'><b>Tra:$vtra Sol:$vsol Per:$tper MP:$vtmp Tex:$vtex Rad:$vrad</b></p>";
   ...
   echo "</tr>";
   echo "<form name='formti' action='adm_titular.php?vtra=$vtra&vsol=$vsol&tper=$tper&vnom=$vnom&vtmp=$vtmp' method='POST'>";
...
//      echo "<p align='center'><font color='#0000FF'>";
   ...
   echo "</form>";
   echo "<form method='POST' action='adm_titular.php?vtra=$vtra&vsol=$vsol&tper=$tper&vnom=$vnom&vtmp=$vtmp'>"; 
?>
