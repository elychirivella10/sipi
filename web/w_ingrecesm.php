...
function planilla(v1,v2,v3) {
  open("w_planillafm05.php?vsol="+v1.value+"&vtramt="+v2.value+"&vopc=","Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
$smarty->assign('subtitulo','Ingreso de Cesion de Marca al SIPI');
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingrecesm.php?vopc=3","N");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingrecesm.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus_sol']<>'15') {echo "<a href='w_planillafm05.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
...
if ($vopc== 6) {
    ...
    if ($vpag=='' or $vpag=='000000' or $vpag=='0' or empty($vpag) or $vpagotasa=='F') {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de Factura del Pago de Tasa de Presentaci&oacute;n ...!!!","../web/w_ingrecesm.php?vopc=3","N");
       $smarty->display('pie_pag.tpl'); exit();
    }
    
    if ($vdoc=='' or $vdoc=='0000000000' or $vdoc=='0' or empty($vdoc)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de Documento del Reloj asociado a la Cesi&oacute;n ...!!!","../web/w_ingrecesm.php?vopc=3","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if ($vcarp=='' OR $vcarp=='000000' OR $vcarp=='0000000000' OR $vcarp=='0' OR empty($vcarp)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de la Carpeta asociada a la Cesi&oacute;n ...!!!","../web/w_ingrecesm.php?vopc=3","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if (empty($fecha_reno)) {
      mensajenew('ERROR: La Fecha de la Cesion esta vacia ...!!!','../web/w_ingrecesm.php?vopc=3','N');
      $smarty->display('pie_pag.tpl'); exit(); }

    $fechahoy = hoy();
    $esmayor=compara_fechas($fecha_reno,$fechahoy);
    if ($esmayor==1) {
      mensajenew('AVISO: NO se pueden ejecutar eventos a Futuros ...!!!','../web/w_ingrecesm.php?vopc=3','N');
      $smarty->display('pie_pag.tpl'); exit();  } 
...
if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       if ($objfilas==0) {
          mensajenew('ERROR: Nro. de Factura NO existe en la Base de Datos ...!!!','../web/w_ingrecesm.php?vopc=3','N');
          $smarty->display('pie_pag.tpl'); exit(); }
       ...
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Cesi&oacute;n de Marcas ...!!!','../web/w_ingrecesm.php?vopc=3','N');
         $smarty->display('pie_pag.tpl'); exit(); }
       ...
       if ($filas_tasas >= $can_tasa) {
          mensajenew('ERROR: Nro. de Factura ya fue utilizada en otra(s) solicitud(es) ...!!! Verifique...','../web/w_ingrecesm.php?vopc=3','N');
          $smarty->display('pie_pag.tpl'); exit(); }
    ...
    $sql = new mod_db();
    $sql->connection();
    $resultado_tram=pg_exec("SELECT * FROM stzevder WHERE evento=1205");
    ...
    if ($obj_filas!=0) {
      ...
      mensajenew('ERROR: Nro. de Carpeta ya Utilizado en otro Tr&aacute;mite ('.$ntramuso.'), Verificar ...!!!','../web/w_ingrecesm.php?vopc=3','N');
      $smarty->display('pie_pag.tpl'); exit(); }
    $sql->disconnect();
    ...
    if ($filas_cfound1==0) {
      mensajenew('ERROR: Nro. de Tr&aacute;mite de Cesi&oacute;n NO presenta ning&uacute;n CESIONARIO ...!!!','../web/w_ingrecesm.php?vopc=3','N');
      $smarty->display('pie_pag.tpl'); exit(); }
...
//    $sql1->disconnect1();
    ...
    $resul_der = pg_exec("SELECT * FROM stzderec where registro='$vreg' AND tipo_mp='M' ");
    ...
    $insert_valores ="'$vnder','1205','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    $insert_valores ="$vnder,$vdoc,'$vnsol','M',1205,$codigo,'$titular','$domicilio','$pais','',0,'N','$cesionario',0";
    ...
    $insert_str = "'$nfac','$fac_fe',0,0,0,'1','C',$vcant_am";  
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','M','$usuario','$fechahoy','$horactual','$vcarp','FM05'";    
    ...
    $actualiza=pg_exec("UPDATE stmsolces SET estatus_sol='15',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where nro_referencia ='$vsol' AND nro_tramite='$vtramt'");             
      ...
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingrecesm.php?vopc=4&vtramt='.$vtramt,'S');
...
$smarty->display('w_ingrecesm.tpl');
$smarty->display('pie_pag.tpl');
//ob_end_clean(); 
?>
