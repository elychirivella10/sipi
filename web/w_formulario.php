...
// Inicio del Formulario
 for($cont=0;$cont<$filas_resultado;$cont++) { 
      ...
      $tramite= 'Tramite: '.$vtramt.' Solicitud:'.$vsol;  
      $pdf->Text(145,27,$tramite);          
	  ...
	  if ($filas_found1 > 1) { $ind_titular= '1'; } // $ind_titular indicador de mas de un titular
 	  //buscar indole
	  if ($regt['indole']=='N') { $pdf->Text(38,46,'X',0,1); } //natural
	  ...
	  $res_prio=pg_exec("SELECT * FROM stzpriod  WHERE nro_tramite='$vtramt' AND solicitud = '$nsolic'");
          ...
          $pdf->Text(120,33,$reg_regprio['prioridad'],0,1); 
          $pais_nombre=pais($reg_regprio['pais_priori']);
          $pdf->Text(72,33,$pais_nombre,0,1);       
          $pdf->Text(170,33,$reg_regprio['fecha_priori'],0,1);       
      ...
      // Signo  o Nombre de la Marca
          if ($ind_logo== 1) { 
             ...
             $reg_logo1 = pg_fetch_array($resul_logo1);      
     	     $ruta = $reg_logo1['nro_busgra'];
             $pdf->Image("../graficos/docutemp/"."$ruta".".png",10,224,55,45,'PNG');
          }     
          else {            
   	    $pdf->SetXY(12,245);
