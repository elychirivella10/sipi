...
function planilla(v1,v2,v3) {
  open("w_planillafm06.php?vsol="+v1.value+"&vtramt="+v2.value+"&vopc=","Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
$smarty->assign('subtitulo','Ingreso de Cambio de Domicilio de Marca al SIPI');
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingredomm.php?vopc=3","N");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingredomm.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus_sol']<>'77') {echo "<a href='w_planillafm06.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
...
if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       //Datos del Detalle 
       $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE (ser_id like '0105%' or ser_id = '0105') AND fac_id=$fac_id"); 
       $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Cambio de Domicilio de Marcas ...!!!','javascript:history.back();','N');
         $smarty->display('pie_pag.tpl'); exit(); }
    ...
    $sql = new mod_db();
    $sql->connection();
    $resultado_tram=pg_exec("SELECT * FROM stzevder WHERE evento=1208");
...
//    $sql1->disconnect1();
    ...
    // Insertar evento 1208 en stzevtrd
    ...
    $insert_valores ="'$vnder','1208','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    $insert_valores ="$vnder,$vdoc,'$vnsol','M',1208,$codigo,'','$domicilio','$pais','',0,'N','$new_domicilio',0";
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','M','$usuario','$fechahoy','$horactual','$vcarp','FM06'";    
    ...
    $actualiza=pg_exec("UPDATE stzsolcam SET estatus_sol='77',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where nro_referencia ='$vsol' AND nro_tramite='$vtramt'");             
      ...
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingredomm.php?vopc=4&vtramt='.$vtramt,'S');
...
$smarty->display('w_ingredomm.tpl');
$smarty->display('pie_pag.tpl');
//ob_end_clean(); 
?>
