...
//        exec($cmd);
    ...
    //ejecuto el shell para traer la imagen
    $cmd="scp -P 3535 www-data@172.16.0.7:/var/www/apl/webpi/tmp_patentes/".$vsol.".jpg /var/www/apl/sipi/docutemp/".$vsol.".jpg"; 
       exec($cmd,$salida);
       $nameimage='../docutemp/'.$vsol.'.jpg';
...
//    exec($cmd);
  ...
  //  unlink("$planilla");
   ...
   //
   $vnsolsipi=$regsoltra['nombre'];
   $vnclasipi=$regsoltra['clase'];*/
   //Para FP01
   $resultadosoltra=pg_exec("select a.nro_tramite,a.solicitud,a.nombre,a.tipo_derecho,d.usuario,d.f_tramite from stzderec a,stztramite d where a.nro_tramite='$vtramt' and a.solicitud='$vsol' and a.nro_tramite=d.nro_tramite order by a.nro_tramite,a.solicitud");
   $vcansol=pg_numrows($resultadosoltra); 
   $regsoltra = pg_fetch_array($resultadosoltra); 
   $vusuario=$regsoltra['usuario'];
   $vnom_pat=$regsoltra['nombre'];
   $vfec_tram=$regsoltra['f_tramite'];
   $tmptip_pat=$regsoltra['tipo_derecho'];
   if ($tmptip_pat=='A') {$tip_pat='INVENCION';}
   if ($tmptip_pat=='B') {$tip_pat='DIBUJO INDUSTRIAL';}
   if ($tmptip_pat=='E') {$tip_pat='MODELO INDUSTRIAL';}
   //
   //if ($vusuario<>$usuario) {
...
$smarty->assign('vnom_pat',$vnom_pat);
$smarty->assign('vtip_pat',$tip_pat);
...
$smarty ->assign('fecha_pres',$fechahoy);

$smarty->display('w_ingrepat1.tpl');
