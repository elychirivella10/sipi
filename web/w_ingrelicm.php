...
function planilla(v1,v2,v3) {
  open("w_planillafm07.php?vsol="+v1.value+"&vtramt="+v2.value+"&vopc=","Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
$smarty->assign('subtitulo','Ingreso de Licencia de Uso de Marca al SIPI');
...
//if (($usuario!='rmendoza') AND ($usuario!='ngonzalez') AND ($usuario!='yeperez')) {	 
...
if ($vopc==2) {
    ...
    $actual=pg_exec("UPDATE stztramite SET estatus_tra = '13'  where nro_tramite='$vtramt'");    
    $sql1->disconnect1();
...
<p>&nbsp;</p>
<p align='center'><b><font > Licencias de Uso de Marcas Asociadas al Tramite No.: <?php echo $vtramt; ?> para Ingresar al SIPI </font></b></p>
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingrelicm.php?vopc=3","N");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingrelicm.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus_sol']<>'15') {echo "<a href='w_planillafm07.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
...
//*** Guardar la solicitud de LICENCIA en el SIPI  
if ($vopc== 6) {
    $vpag=$_POST['vfact'];
    ...
    if ($vdoc=='' or $vdoc=='0000000000' or $vdoc=='0' or empty($vdoc)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de Documento del Reloj asociado a la Licencia ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if ($vcarp=='' OR $vcarp=='000000' OR $vcarp=='0000000000' OR $vcarp=='0' OR empty($vcarp)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de la Carpeta asociada a la Licencia ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if (empty($fecha_reno)) {
      mensajenew('ERROR: La Fecha de Presentaci&oacute;n de la Licencia esta vacia ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
...
if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       //Datos del Detalle 
       $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id like '0106%' AND fac_id=$fac_id"); 
       $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Licencia de Uso de Marcas ...!!!','javascript:history.back();','N');
         $smarty->display('pie_pag.tpl'); exit(); }
       ...
       $query_tasas=pg_exec("SELECT nro_factura FROM stmfactura WHERE nro_factura='$nfac' and tipo_anotamar='L'");
       $filas_tasas = pg_numrows($query_tasas);    
    ...
    $sql = new mod_db();
    $sql->connection();
    $resultado_tram=pg_exec("SELECT * FROM stzevder WHERE evento=1207");
    ...
    $licenciante = "";
    $res_licenciante = pg_exec("SELECT * FROM stztmptit WHERE stztmptit.nro_tramite='$vtramt' AND stztmptit.solicitud='$vsol'");
    $filas_cfound1=pg_numrows($res_licenciante);
    if ($filas_cfound1==0) {
      mensajenew('ERROR: Nro. de Tr&aacute;mite de Licencia NO presenta ning&uacute;n LICENCIANTE ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
    
    $regtc = pg_fetch_array($res_licenciante);
    for($cont1=0;$cont1<$filas_cfound1;$cont1++)   { 
      ...
      if (empty($licenciante)) {
        $licenciante = $licenciante.$titular." con Domicilio en: ".$domicilio. ", ".$pais_domicilio; }
      else {
        $licenciante = ", ".$licenciante.$titular." con Domicilio en: ".$domicilio. ", ".$pais_domicilio; }
      $regtc = pg_fetch_array($res_licenciante);
    } 
    ...
    $licenciatario = "";
    $res_licenciatario = pg_exec("SELECT * FROM stzcesion WHERE stzcesion.nro_tramite='$vtramt' AND stzcesion.solicitud='$vsol'");
    $filas_cfound1=pg_numrows($res_licenciatario);
    if ($filas_cfound1==0) {
      mensajenew('ERROR: Nro. de Tr&aacute;mite de Licencia NO presenta ning&uacute;n LICENCIATARIO ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
    
    $regtc = pg_fetch_array($res_licenciatario);
    for($cont1=0;$cont1<$filas_cfound1;$cont1++)   { 
      $titular   = $regtc['nombre'];
      $domicilio = $regtc['domicilio'];
      ...
      if (empty($licenciatario)) {
        $licenciatario = $licenciatario.$titular." con Domicilio en: ".$domicilio. ", ".$pais_domicilio; }
      else {
        $licenciatario = ", ".$licenciatario.$titular." con Domicilio en: ".$domicilio. ", ".$pais_domicilio; }
      $regtc = pg_fetch_array($res_licenciatario);
    } 
...
//    $sql1->disconnect1();
    ...
    $vcomenta= 'Licencia presentada en fecha: '.$fecha_reno.", LICENCIANTE: ".$licenciante.", LICENCIATARIO: ".$licenciatario.', WEBPI Tramite No: '.$vtramt.' y Referencia: '.$vsol.'.  S/'.$comenpagotasa;

    $vcomenta= $vcomenta.' '.$vcomadi;
    //echo " $vcomenta "; exit();
    
    // Insertar evento 1207 en stzevtrd
    ...
    $insert_valores ="'$vnder','1207','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    $insert_campos="nro_derecho,nanota,solicitud,tipo,evento,nom_tit_2,domicilio,pais,tramitante,agente,verificado,inf_adicional,cod_tit_1,cod_tit_2";
    $insert_valores ="$vnder,$vdoc,'$vnsol','M',1207,'$titular','$domicilio','$pais','',0,'N','$licenciatario',0,0";
    ...
    $insert_str = "'$nfac','$fac_fe',0,0,0,'1','L',$vcant_am";  
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','M','$usuario','$fechahoy','$horactual','$vcarp','FM07'";    
    ...
    $actualiza=pg_exec("UPDATE stmsolces SET estatus_sol='15',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where nro_referencia ='$vsol' AND nro_tramite='$vtramt'");             
      ...
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingrelicm.php?vopc=4&vtramt='.$vtramt,'S');
...
<p>&nbsp;</p>
<p align='center'><b><font > Licencias de Uso de Marcas Asociadas al Tramite para Ingresar al SIPI </font></b></p>
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  <tr>
...
$smarty->display('w_ingrelicm.tpl');
$smarty->display('pie_pag.tpl');
//ob_end_clean(); 
?>
