...
function planilla(v1,v2,v3) {
  open("w_planillafp04.php?vsol="+v1.value+"&vtramt="+v2.value+"&vopc=","Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
$smarty->assign('subtitulo','Ingreso de Cambio de Domicilio de Patente al SIPI');
...
if ($vopc==2) {
    ...
    $actual=pg_exec("UPDATE stztramite SET estatus_tra = '72'  where nro_tramite='$vtramt'");    
    $sql1->disconnect1();
...
<p>&nbsp;</p>
<p align='center'><b><font > Cambio de Domicilio(s) Asociadas al Tramite No.: <?php echo $vtramt; ?> para Ingresar al SIPI </font></b></p>
...
//         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[solic_sipi]</td>";
         ...
         echo "<td width='14% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER><a href='w_planillafp04.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>$registro_tram[solicitud]</a></td>";
//         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[estatus]</td>";
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingredomp.php?vopc=3","N");
    ...
    $resultado_tram = pg_exec("SELECT * FROM stzsolcam, stztramite 
    			 	...
    			 	AND stztramite.estatus_tra IN ('72','77') order by stzsolcam.nro_referencia");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingredomp.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus_sol']<>'77') {echo "<a href='w_planillafp04.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
     ...
     echo "<td style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER>";
     if ($registro_tram['estatus_sol']=='77') {
     ...
     echo "<td style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER>";
     if ($registro_tram['estatus_sol']=='77') {echo "$registro_tram[solicitud_fecha]</td>";} else {echo "-</p></td>";}
     echo "<td style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER>";
     if ($registro_tram['estatus_sol']=='77') {echo "$varhorasol</p></td>";} else {echo "-</p></td>";} 

     echo "<td style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER>";
     if ($registro_tram['estatus_sol']=='77') {echo "$registro_tram[solicitud_sipi]</p></td>";} else {echo "-</p></td>";} 
...
//*** Guardar la solicitud de CAMBIO DOMICILIO en el SIPI  
if ($vopc== 6) {
    $vpag=$_POST['vfact'];
    ...
    if ($vdoc=='' or $vdoc=='0000000000' or $vdoc=='0' or empty($vdoc)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de Documento del Reloj asociado a la Cesi&oacute;n ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if ($vcarp=='' OR $vcarp=='000000' OR $vcarp=='0000000000' OR $vcarp=='0' OR empty($vcarp)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de la Carpeta asociada al Cambio de Domicilio ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }

    if (empty($fecha_reno)) {
      mensajenew('ERROR: La Fecha del Cambio de Domicilio esta vacia ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
...
if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       if ($objfilas==0) {
          mensajenew('ERROR: Nro. de Factura NO existe en la Base de Datos ...!!!','javascript:history.back();','N');
          $smarty->display('pie_pag.tpl'); exit(); }
       ...
       //Datos del Detalle 
       $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE (ser_id = '0204') AND fac_id=$fac_id"); 
       $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Cambio de Domicilio de Patentes ...!!!','javascript:history.back();','N');
         $smarty->display('pie_pag.tpl'); exit(); }
       ...
       if ($filas_tasas >= $can_tasa) {
          mensajenew('ERROR: Nro. de Factura ya fue utilizada en otra(s) solicitud(es) ...!!! Verifique...','javascript:history.back();','N');
          $smarty->display('pie_pag.tpl'); exit(); }
    ...
    $sql = new mod_db();
    $sql->connection();
    $resultado_tram=pg_exec("SELECT * FROM stzevder WHERE evento=2208");
    ...
    $query_res="SELECT * FROM stzsolcam WHERE nro_tramite = '$vtramt' and nro_referencia= '$vsol'";
    ...
    if ($filas_cfound1==0) {
      mensajenew('ERROR: Nro. de Tr&aacute;mite NO presenta ning&uacute;n Cambio de Domicilio ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
    $regtc = pg_fetch_array($res_domicilio);
    for($cont1=0;$cont1<$filas_cfound1;$cont1++) {
      $codigo = $regtc['titular'];
      $anterior_dom = $regtc['domicilio_anterior'];
      $anterior_pais=pais($regtc['pais_anterior']);
      $ant_domicilio = $ant_domicilio.$anterior_dom.", ".$anterior_pais;
      $domicilio = $regtc['domicilio_actual'];
      $pais = $regtc['pais_actual'];
...
//    $sql1 = new mod_db();
//    $sql1->connection1();
//    $var=pg_exec("update stmsolren set estatus_sol='77',solicitud_sipi='$numsol',solicitud_fecha='$fechahoy',solicitud_hora= '$horactual' 
...
//    $sql1->disconnect1();
    ...
    $vcomenta= 'Cambio de Domicilio presentado en fecha: '.$fecha_reno.', ANTERIOR DOMICILIO: '.$ant_domicilio.', DOMICILIO NUEVO: '.$new_domicilio.', WEBPI Tramite No: '.$vtramt.' y Referencia: '.$vsol.'.  S/'.$comenpagotasa;

    $vcomenta= $vcomenta.' '.$vcomadi;
    //echo " $vcomenta "; exit();
    
    // Insertar evento 2208 en stzevtrd
    ...
    $insert_valores ="'$vnder','2208','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    // Insertar datos de cambio domicilio en Stzantma
    ...
    $insert_valores ="$vnder,$vdoc,'$vnsol','P',2208,$codigo,'','$domicilio','$pais','',0,'N','$new_domicilio',0";
    ...
    $insert_str = "'$nfac','$fac_fe',0,0,0,'1','D',$vcant_am";  
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','P','$usuario','$fechahoy','$horactual','$vcarp','FP04'";    
    ...
    $sql = new mod_db();
    $sql->connection();    
    $update_str = "estatus='77',solic_sipi= '$vreg', fecha='$fechahoy', hora= '$horactual'";
    ...
    $sql1 = new mod_db();
    $sql1->connection1();    
    $actualiza=pg_exec("UPDATE stzsolcam SET estatus_sol='77',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where nro_referencia ='$vsol' AND nro_tramite='$vtramt'");             

      $resultado_cant = pg_exec("SELECT * FROM stzsolcam where nro_tramite='$vtramt' ");
      $filas_cant = pg_numrows($resultado_cant); 
      $resultado_estatus = pg_exec("SELECT * FROM stzsolcam where nro_tramite='$vtramt' and estatus_sol = '77'");
      $filas_estatus = pg_numrows($resultado_estatus);  
      if ($filas_estatus == $filas_cant) {
          $actual=pg_exec("UPDATE stztramite SET estatus_tra = '77' where nro_tramite='$vtramt'");   }    
      $sql1->disconnect1();  

      
      //Desconexion de la Base de Datos
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingredomp.php?vopc=4&vtramt='.$vtramt,'S');
      ...
      //Desconexion de la Base de Datos
      $sql->disconnect();
      //*** DesActualizacion tabla stzsolcam
      $sql1 = new mod_db();
      $sql1->connection1();
      $var=pg_exec("update stzsolcam set estatus_sol=null,solicitud_sipi=null,solicitud_fecha=null,solicitud_hora=null 
...
<p>&nbsp;</p>
<p align='center'><b><font > Cambio de Domicilio(s) Asociadas al Tramite para Ingresar al SIPI </font></b></p>
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  <tr>
...
$smarty->display('w_ingredomp.tpl');
$smarty->display('pie_pag.tpl');
//ob_end_clean(); 
?>
