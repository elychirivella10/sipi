...
function planilla(v1,v2,v3) {
  open("w_planillafm08.php?vsol="+v1.value+"&vtramt="+v2.value+"&vopc=","Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
$smarty->assign('subtitulo','Ingreso de Cambio de Nombre o Titular de Marca al SIPI');
...
//         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[solic_sipi]</td>";
         ...
         echo "<td width='14% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER><a href='w_planillafm08.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>$registro_tram[solicitud]</a></td>";
//         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[estatus]</td>";
...
if ($vopc==4) {
  ...
  if (empty($vtramt)) {
    mensajenew("AVISO: Nro. de Tramite en blanco, NO lo ingres&oacute; ...!!!","../web/w_ingrecnomm.php?vopc=3","N");
    ...
    if ($filas_resultado_tram==0) { 
      mensajenew("AVISO: El Nro. de Tramite no esta Registrado o No tiene solicitudes que ingresar ...!!!","../web/w_ingrecnomm.php?vopc=3","N");
      $smarty->display('pie_pag.tpl'); 
      exit(); } 
    else{
...
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus_sol']<>'15') {echo "<a href='w_planillafm08.php?vsol=$vsol&vtramt=$vtramt&vopc=4''>
...
if ($vopc== 6) {
    ...
    if ($vcarp=='' OR $vcarp=='000000' OR $vcarp=='0000000000' OR $vcarp=='0' OR empty($vcarp)) {
       mensajenew("ERROR: Debe colocar el N&uacute;mero de la Carpeta asociada a la Cambio de Nombre ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit();
    }
...
if ($usuario!='rmendoza') {
    ...
    if ($vpagotasa=='F1105928') {$comenpagotasa='Factura: EXONERADA';
       ...
       //Datos del Detalle 
       $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id like '0104%' AND fac_id=$fac_id"); 
       $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
       if ($objtotdtalle==0) {
         mensajenew('ERROR: Nro. de Factura NO presenta ning&uacute;n pago de Tasa de Cambio de Nombre de Marcas ...!!!','javascript:history.back();','N');
         $smarty->display('pie_pag.tpl'); exit(); }
    ...
    $sql = new mod_db();
    $sql->connection();
    $resultado_tram=pg_exec("SELECT * FROM stzevder WHERE evento=1209");
...
//    $sql1->disconnect1();
    ...
    // Insertar evento 1209 en stzevtrd
    ...
    $insert_valores ="'$vnder','1209','$fecha_reno',nextval('stzevtrd_secuencial_seq'),
    ...
    $insert_valores ="$vnder,$vdoc,'$vnsol','M',1209,$codigo,'$titular','$domicilio','$pais','',0,'N','$cesionario',0";
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$vnder','$vnsol','$vreg','M','$usuario','$fechahoy','$horactual','$vcarp','FM08'";    
    ...
    $actualiza=pg_exec("UPDATE stmsolces SET estatus_sol='15',solicitud_fecha='$fechahoy',solicitud_hora='$horactual',solicitud_sipi='$vdoc' where nro_referencia ='$vsol' AND nro_tramite='$vtramt'");             
      ...
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','w_ingrecnomm.php?vopc=4&vtramt='.$vtramt,'S');
...
<p>&nbsp;</p>
<p align='center'><b><font > Cambio de Nombre o Titular de Marca Asociadas al Tramite para Ingresar al SIPI </font></b></p>
<table style="background-color: #015B9E; border: 0 solid #D8E6FF" border="1" cellpadding="0" cellspacing="0" width="80%" align="center">
  <tr>
  ...
  <?php  
    ...
    for($cont=0;$cont<$filas_resultado_tram;$cont++) { 
     ...
     if ($registro_tram['estatus']== 15) {
         ...
         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[estatus]</td>";
         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER>$registro_tram[solic_sipi]</td>";
         echo "<td width='12% 'style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER><a href='w_formsipi.php?vsol=$registro_tram[solic_sipi]''>FM-08</a></td>";
         echo "<td width='12%' style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[fecha]</td>";
         echo "<td width='12%'  style='background-color: #7AC0EF; border: 1 solid #D8E6FF'><P ALIGN=CENTER> $registro_tram[hora]</td>";
         }
     else {
...
$smarty->display('w_ingrecnomm.tpl');
$smarty->display('pie_pag.tpl');
//ob_end_clean(); 
?>
