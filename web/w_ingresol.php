...
$smarty->assign('subtitulo','Ingreso de Solicitud de Marcas al SIPI');
...
//if (($usuario!='rmendoza') AND ($usuario!='ngonzalez') AND ($usuario!='ftorres')) {	 
//    Mensajenew("ERROR: Modulo en Mantenimiento ...!!!","../index1.php","N");
...
if ($vopc==4) {
  ...
  if ($vtramt==187986 or $vtramt==187990 or $vtramt==187995 or $vtramt==187998 or $vtramt==188002 or $vtramt==188005 or $vtramt==188010 or
      $vtramt==188013 or $vtramt==188018 or $vtramt==188020 or $vtramt==188033 or $vtramt==188036 or $vtramt==188039 or $vtramt==188040 or
      $vtramt==188042 or $vtramt==188050 or $vtramt==188052 or $vtramt==188055 or $vtramt==188059 or $vtramt==188062 or $vtramt==188063 or
      $vtramt==188064 or $vtramt==188102 or $vtramt==188103) {
    mensajenew("AVISO: Tramite NO Procede!!! Verificar Domicilio del Titular. El Pago debe realizarse en PETROS.!!!","w_ingresol.php?vopc=3","N");
...
//    $smarty->display('pie_pag.tpl'); exit(); } 
  if ($vtramt==211816) {
    mensajenew("AVISO: Tramite NO Procede!!! Ponerse en contacto con la Coordinacion de Informatica.","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); }
  if ($vtramt==216450 or $vtramt==216449 or $vtramt==216447) {
    mensajenew("AVISO: Tramite NO Procede!!! Problemas con el Deposito, remitir al Coordinador de Taquilla.","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); }  
  if (empty($vtramt)) {
...
if ($vopc== 6) {
    ...
    if (empty($fecha_pres)) {
      mensajenew('ERROR: La Fecha de Presentacion de la Solicitud FM02 esta vacia ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }
    ...
    if ($vpagotasa=='F1105928') {$vpagotasa='EXONERADO'; 
       ...
       //Datos del Detalle 
       $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id='0101' AND fac_id=$fac_id"); 
       ...
       $query_tasas=pg_exec("SELECT evento FROM stzevtrd 
                             WHERE evento=1200 and comentario LIKE '$comenpagotasa%'");
...
//    $smarty->display('pie_pag.tpl'); exit();  
    ...
    $obj_query = $sql->query("SELECT * FROM stzfactram WHERE carpeta = '$vcarp' AND tipo_mpa='M' AND tipo_tram='FM02'");
...
poder,tramitante,agente,nplanilla,idtramitante,tramitewebpi"; 
    $insert_str = "'$prox_derecho', '$tipo_marca', '$numsol', '$fecha_pres', 'M', '$nombre', '$varestatus', '$pais_resid', '$poder', '$tramitante', '$numero_agente','0','$ntramitante','S'";
    ...
    if ($vtramt>=250200) {
       $insert_str = "'$prox_derecho','1200','$fecha_pres',nextval('stzevtrd_secuencial_seq'), '1000', '0', '$fechahoy', '$usuario','$vdes','$horactual',
      'Pago de Tasa y Publicacion en Prensa: $vpagotasa Tramite: $vtramt Ref.: $vsol'";
    } else {
      $insert_str = "'$prox_derecho','1200','$fecha_pres',nextval('stzevtrd_secuencial_seq'), '1000', '0', '$fechahoy', '$usuario','$vdes','$horactual',
      'Pago de Tasa: $vpagotasa Tramite: $vtramt Ref.: $vsol'";
    }
    ...
    $insert_factram = "nextval('stzfactram_nro_controlft_seq'),'W','$vtramt','$vfec_tram',$vsol,'$nfac','$fac_fe','$prox_derecho','$numsol','','M','$usuario','$fechahoy','$horactual','$vcarp','FM02'";    
      ...
      if ($filas_res_titu == "0")
       {
	    ...
	    $numero_poder = $poder;          
            ...
            $col_campos = "poder,titular,fecha_poder,facultad,fecha_trans,usuario";    
//            $insert_str = "'$poder','$act_titular','$regis_poder->fecha_poder',
//                           '$regis_poder->facultad','$regis_poder->fecha_trans'";
            $insert_str = "'$poder','$act_titular','$fechahoy','M','$fechahoy','$usuario'";
     ...
     else
       {
	    ...
	    $numero_poder = $poder;          
            ...
            $col_campos = "poder,titular,fecha_poder,facultad,fecha_trans,usuario";    
            $insert_str = "'$poder','$regtitu[titular]','$fechahoy','M','$fechahoy','$usuario'";
            if (!empty($poder)) {$inspoder = $sql->insert("stzpoder","$col_campos","$insert_str","");}
         }
...
//*** Comprobaciones finales de errores
   //    echo " numtitu=$numtitu inslema=$inslema instram=$instram insprio=$insprio inslogo=$inslogo numprio=$numprio insagen=$insagen numagen=$numagen numapod=$numapod insmarce=$insmarce insderecho=$insderecho insclanac=$insclanac  insautod=$insautod $insautop=$insautop ins_solic=$ins_solic ins_titur=$ins_titur ins_tramt=$ins_tramt ";
    ...
    if ($numtitu==0 AND $inslema AND $instram AND $inslogo AND $ins_tramt AND $insfoneti AND
        $numprio==0 AND $numagen==0 AND $numapod==0 AND $insmarce AND $insderecho AND $insclanac AND $insfactram) {
       pg_exec("COMMIT WORK");
