...
if ($vopc==1) {
   ...
   $resultado=pg_exec("SELECT * FROM $tbname_6 WHERE solicitud='$vsol' and solicitud!='' and tipo_mp='I'");
}
...
//    $smarty->assign('ubicacion',$nameimage);
    ...
    switch ($modal_id) {
      case "I":
         $modal = "Declaraci&oacute;n de Indicaci&oacute;n Geogr&aacute;fica";
         break;
      case "R":
         $modal = "Reconocimiento de Indicaci&oacute;n Geogr&aacute;fica";
         break;
    }
    
    switch ($vtipo) {
      case "I":
         $vtip='INDICACION GEOGRAFICA';
         break;
    }
...
if ($vopc==3) 
{
    ...
    if ($vest!=1) {
      mensajenew("Solicitud no puede ser eliminada de la Base de Datos, estatus diferente a Presentada ...!!!","i_elisolig.php","N");
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
    } 

    //Elimino Datos asociado a la solicitud de la Base de Datos
    $sql->del("stiindig","nro_derecho='$vder'");
...
//    $sql->del("stmlemad","nro_derecho='$vder'");
    ...
    $sql->del("stirepid","nro_derecho='$vder'");
...
//    $sql->del("stmfonetica","nro_derecho='$vder'");
    ...
    mensajenew('DATOS ELIMINADOS CORRECTAMENTE !!!','i_elisolig.php','S');
