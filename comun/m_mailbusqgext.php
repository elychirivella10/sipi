...
$smarty->assign('subtitulo','ENVIO DE RESULTADOS BUSQUEDAS GRAFICAS EXTERNAS PDF POR CORREO');
...
if ($vopc==2) {
 ...
 $where=" estatus_envio='N' AND envio='S' AND estatus='2' "; 
...
if(!empty($pedido1) && !empty($pedido2)) {
  ...
  if(empty($where)) {
    $where = $where." (stmcntrl.pedido>=$pedido1 AND stmcntrl.pedido<=$pedido2)"; }
  else {
    $where = $where." AND (stmcntrl.pedido>=$pedido1 AND stmcntrl.pedido<=$pedido2)"; }
}

if (!empty($fecharec)) {
  $where = $where." AND (stmcntrl.fecharec='$fecharec')"; 
}
...
if (!empty($factura)) { 
  $where = $where." AND"." (stmcntrl.recibo='$factura')";
}

 // Obtencion de los Registros o Filas   
 //$obj_query = $sql->query("SELECT * FROM $tbname_1 WHERE estatus_envio='N' AND envio='S' AND estatus='2' ORDER BY recibo,pedido");
 $obj_query = $sql->query("SELECT * FROM $tbname_1 WHERE $where ORDER BY recibo,pedido");

 $filas_found=$sql->nums('',$obj_query);
 if ($filas_found==0) {
   Mensajenew("ERROR: NO existe informaci&oacute;n en Nuestra Base de Datos para enviar ...!!!","javascript:history.back();","N");
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 

 //Verificacion de los archivos pedidos PDF  
 if (!empty($factura)) {
   $obj_fac = $sql->query("SELECT * FROM $tbname_1 WHERE recibo = '$factura' ORDER BY pedido");
   $filas_fac=$sql->nums('',$obj_fac);
 
   $cadenaped = "";
   $existeped = 0;
   $objregfac = $sql->objects('',$obj_fac);
   $vruta="/home/fonetica/pdfext/grafica/";   
   for($cont=0;$cont<$filas_fac;$cont++) {
     $refbusq = trim($objregfac->pedido);
     $filepdf = $vruta.trim($refbusq).".pdf";
     if (file_exists($filepdf)) { } 
     else { $existeped = $existeped + 1; $cadenaped = $cadenaped.$refbusq.", "; } 
     $objregfac = $sql->objects('',$obj_fac);
   }
 }

 if ($existeped!=0) { 
   Mensajenew("ERROR: La factura $factura NO se puede Enviar por Correo Electr&oacute;nico, faltan los archivos PDF: $cadenaped ...!!!","javascript:history.back();","N");
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
 ...
 $tramant = trim($objs->recibo);
 $indc = 1;
 $vruta="/home/fonetica/pdfext/grafica/";   
 for($cont=0;$cont<$filas_found;$cont++) {
   $cuenta  = trim($objs->email);
   $factura = trim($objs->recibo);
   $refbusq = trim($objs->pedido);
   $nombre  = trim($objs->solicitant);
   if ((($usrant!=$cuenta) || ($tramant!=$factura)) || ($indc==1)) {
     ...
     $indc = 0;
     $update_str = "estatus_envio='S'";
     $act_bus = $sql->update("$tbname_1","$update_str","pedido=$refbusq");
     // Verificacion de actualizacion BD  
     if ($act_bus) { //echo " lo actualizo con pedido='$refbusq' AND recibo='$factura' "; 
     }
   ...
   $objs = $sql->objects('',$obj_query);
   if ($usrant!=trim($objs->email) || $tramant!=trim($objs->recibo)) {  
     ...
     $actbusq    = true;
     $update_str = "estatus_envio='E',fecha_envio='$fechahoy',hora_envio='$horactual'"; 
     $actbusq    = $sql->update("stmcntrl","$update_str","recibo='$factura' AND envio='S'");
     if ($actbusq) { }
     else { $numerror = $numerror + 1; }
     if ($numerror==0) { 
       pg_exec("COMMIT WORK"); } 
 ...
 if ($numerror==0) { 
   //pg_exec("COMMIT WORK"); 
   $vmessage="RESULTADO(S) DE BUSQUEDA(S) GRAFICA(S) PDF ENVIADO(S) CORRECTAMENTE POR CORREO(S)...!!";
   $vmessage1="";
   echo "<br><br>";
   ...
   echo "<p align='center'><a href='../comun/m_mailbusqgext.php?vopc=1'><input type='image' name='continuar' value='Continuar' onclick='window.close()' src='../imagenes/apply_f2.png' alt='Continuar' align='middle' border='0' />Continuar</a></p>";
   echo "<br>";
...
$smarty->display('m_mailbusqgext.tpl');
$smarty->display('pie_pag.tpl');
...
function correo($sql_mail,$vemail,$nombre,$tramite,$adjuntos) {
 ...
 $mail->Subject = "Envio de Solicitud(es) de Busqueda(s) Grafica(s) SAPI, Factura No. ".$tramite;
 ...
 $body .= "Anexo al presente, le hacemos llegar el(los) resultado(s) de la(s) b&uacute;squeda(s) Gr&aacute;fica(s) solicitada(s) por usted en el Sapi, y relacionada(s) a la Factura n&uacute;mero <strong>".$tramite."</strong>.<br><br>";

 $body .= "<table width='100%' border='1'>";
 $body .= " <tr>";
 $obj_filas = 0;
 $obj_fone  = $sql->query("SELECT * FROM $tbname_1 where recibo='$tramite' ORDER BY pedido");
 $obj_filas = $sql->nums('',$obj_fone);
 if ($obj_filas>0) {
   $body .= "   <tr>";
   $body .= "     <td colspan='4' align='left'><b><H3><I>B&uacute;squedas Gr&aacute;ficas Solicitadas</I></H3></b></td>";
   $body .= "   </tr>";
   $body .= "   <tr>";
   ...
   $body .= "     <td width='12%' align='left'><b>Fecha Pedido</b></td>";
   $body .= "   </tr>";
   ...
   for ($cont=0;$cont<$obj_filas;$cont++) {
     ...
     $body .= "   <tr>";
     $body .= "     <td width='14%' align='left'>$objs->pedido</td>";
     //$body .= "     <td width='60%' align='left'>$bnombre</td>";
     $body .= "     <td width='08%' align='left'>$objs->clase</td>";
     $body .= "     <td width='08%' align='left'>$objs->fecharec</td>";
     $body .= "   </tr>";
     $objs = $sql->objects('',$obj_fone); 
   }
   $body .= "   <tr>";
 ...
 $body .= "<b>IMPORTANTE: Se informa al p&uacute;blico general, que pr&oacute;ximamente la(s) solicitud(es) de b&uacute;squeda(s) tanto fon&eacute;tica como gr&aacute;fica ser&aacute;n &uacute;nicamente tramitadas v&iacute;a internet, por medio del Servicio en L&iacute;nea (Webpi) en el portal del Sapi; es por ello, que se les recomienda registrarse lo mas pronto posible y comenzar a disfrutar de los beneficios suministrados por esta v&iacute;a.</b><br><br>";
