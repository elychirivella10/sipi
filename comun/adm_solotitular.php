 ...
 if ($vmod=='Buscar/Incluir' || $vmod=='Buscar')
  {$resultado=pg_exec("SELECT * FROM stzsolic WHERE nombre like '%$vtex%' ORDER BY nombre OFFSET $inicio LIMIT $cuanto");
   ...
   if ($filas_resultado==0){
       ...
       ?>
       <form action="m_gbsolotitular.php" name="formtitular" method="POST" >
       <?php
...
<?php
   ...
   echo "</tr>";
   echo "<form name='formti' action='m_gbsolotitular.php' method='POST'>";
...
// Fin insertado 2
   ...
   echo "</form>";
   echo "<form method='POST' action='adm_solotitular.php?vsol=$vsol&vmod=$vmod&vtex=$vtex&vder=$vder'>"; 
?>
   <!-- <p align='center'><I><b><font class='nota4'>Mostrando <?= $inicio + 1 ?>-<?= min($inicio + $cuanto, $total) ?> de <?= $total?> ocurrencias encontradas </font></b></I></p> -->
<div align="center">
   <?php
 ...
 if ($vmod=='Modificar') {
   ...
   echo "<form name='formtitular' action='m_gbsolotitular.php' method='post'>";
...
//Fin insertado 3
 ...
 if ($vmod=='Corregir') {
   ...
   echo "<form name='formtitular' action='m_gbsolotitular' method='post'>";
   echo "<input type='hidden' name='vtit' value='$vtit'>";
   echo "<input type='hidden' name='vsol' value='$vsol'>";
   echo "<input type='hidden' name='vmod' value='$vmod'>";
   echo "<input type='hidden' name='vder' value='$vder'>";
