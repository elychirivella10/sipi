...
require ("$include_lib/PDF_tablesbfweb.php");
...
$smarty->assign('subtitulo','Conversi&oacute;n de Archivos Busquedas Webpi Formato TXT a PDF');
...
$rutafinal="/var/www/apl/sipi/documentos/busquedas/webpi/fonetica";
$vopc=2;
$reciboant='';
$cont=1; 
$totalfile=0;
...
if ($vopc==2) {   
   ...
   foreach($salida as $line) { 
     ...
     $archivo=substr(ltrim(rtrim($line)),21,11);
     #Abrimos el fichero
     ...
     while ((!feof($Descriptor2)) AND $fin==0)  {
        ...
        //Obtener Encabezado
        $fincant=strpos($linearch,"** Total de Solicitudes Buscadas:");
        if ($fincant === false) { } 
        else { 
          $numpos = strpos($linearch,"** Total"); $numpos = $numpos + 35;
          $cant_sol=trim(substr($linearch,$numpos,10));       
        }
        ...
        if ($lineas==7) {  $numpos = strpos($linearch,"Pedido"); $numpos = $numpos + 8;
            $pedido=substr($linearch,$numpos,40);
            $nropedido=trim(substr($pedido,0,8));  
        }
        //echo "p=$pedido / $nropedido ";
        if (!empty($nropedido)) {
          //$npedidow = (int)$nropedido;
          $resbusweb=pg_exec("SELECT * FROM stmbusweb WHERE nro_pedido='$nropedido' AND tipo_busq='F'");
          if (!$resbusweb) { 
          ...
          if ($filas_web!=0) {
            ...
            $ntramite=trim($regweb[nro_tramite]);
            $tramite =str_pad($ntramite,8,"0",STR_PAD_LEFT);
          }  
        }
        ...
        if ($lineas==8) {  $numpos = strpos($linearch,"Recibo"); $numpos = $numpos + 8; 
            ...
            $recibo=$tramite." - ".$nrefbus; 
        } 
        //echo "$fecha_bus / $pedido - $nropedido / $recibo "; exit();

        if ($lineas==8) {  $denomi =substr($linearch,0,70);  
            $numpos = strpos($linearch,"Recibo"); 
            $numpos = utf8_decode($numpos) + 8;
            ...
            //echo "Renombrar ".$archivo." como ".$newname,"<br>"; exit(); 
            exec('cp '.$ruta.'/'.$archivo.' '.$ruta.'/'.$newname);
            //Agregado por Romulo Mendoza 25/01/2011 
            $tbname_1 = "stmbusweb";
            $update_str = "estado='2',user_carga='$login',fecha_proceso='$fechahoy',hora_proceso='$hh',user_proceso='$login'";
   ...
   //  $pdf->ln(6);      
	 ...
	 $restit = pg_fetch_array($result);
      ...
      // if (is_numeric($porc)) { 
	     ...
	     $titular='';
	     $pdf->Draw_Data($data);
        $pdf->MultiCell(196,4,utf8_decode('MARCA: '.$registro['nombre']),0,'J',0);
        $pdf->ln(1); 
//      $tit2 = $tit2 + 1;
        $cant = $cant + 1;
      // }
...
$pdf->Draw_Table_Border();
     ...
     $pdf->MultiCell(196,4,'** Total de Solicitudes Buscadas: '.$cant_sol,0,'J',0); 
     $pdf->ln(6); 
     ...
     $pdf->Output($rutafinal.'/'.trim($newname).'.pdf');
     $pdf->Close();
   ...
   exec('rm '.$ruta.'/d*');
   //echo "Total de archivos procesados: ".$totalfile,"<br>";
   Mensajenew("TOTAL ARCHIVOS PROCESADOS: ".$totalfile,"z_leabusq.php?vopc=1",'S');
