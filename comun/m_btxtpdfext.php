...
$smarty->assign('subtitulo','Conversi&oacute;n de Archivos Busquedas Externas TXT a Formato PDF');
...
//Variables
$linea="-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------";
$ruta="/home/fonetica/tintegral";
$rutapdf="/home/fonetica/pdfext/fonetica";
$rutapdf1="/var/www/apl/sipi/documentos/busquedas/pdfext/fonetica";
$vopc=2;
$reciboant='';
$cont=1; 
$totalfile=0;
...
if ($vopc==2) {   
   ...
   foreach($salida as $line) { 
     ...
     $archivo=trim(substr(ltrim(rtrim($line)),25,11));
     #Abrimos el fichero
     ...
     while ((!feof($Descriptor2)) AND $fin==0)  {
        ...
        if ($lineas==7) {  $numpos = strpos($linearch,"Pedido"); $numpos = $numpos + 8;
            $pedido=trim(substr($linearch,$numpos,40));  
            $nropedido=trim(substr($pedido,0,6));
        }
	...
	//	$header_type[$i]['TEXT'] = utf8_decode("%Par.");
     ...
     for($cont2=0;$cont2<$filas_resultado;$cont2++) { 
       ...
       // if (is_numeric($porc)) { 
	     ...
	     if (is_numeric($porc)) { $solbusq = $solbusq + 1; }  
...
//      $tit2 = $tit2 + 1;
     ...
     $pdf->ln(6);  
     $pdf->SetFont('Arial','B',8);  
     //$pdf->MultiCell(196,4,'** Total de Solicitudes Buscadas: '.$cant,0,'J',0); $solbusq = 
     //$pdf->MultiCell(196,4,'** Total de Solicitudes Buscadas: '.$solbusq,0,'J',0);  
     ...
     //$pdf->Output($rutapdf1.'/'.trim($newname).'.pdf');
      
     $pdf->Close();
     exec('rm '.$ruta.'/'.$newname);  
     exec('cp -f '.$ruta.'/'.trim($newname).'.pdf'.' '.$rutapdf1.'/'.trim($newname).'.pdf');  
     exec('mv -f '.$ruta.'/'.trim($newname).'.pdf'.' '.$rutapdf.'/'.trim($newname).'.pdf');  
     //exec('cp -f '.$ruta.'/'.trim($newname).'.pdf'.' '.$rutapdf1.'/'.trim($newname).'.pdf');  
   ...
   //echo "Total de archivos procesados: ".$totalfile,"<br>"; 
   exec('rm '.$ruta.'/d*');
   Mensajenew("TOTAL ARCHIVOS PROCESADOS: ".$totalfile,"m_btxtpdfext.php?vopc=1",'S');
...
$smarty->display('m_btxtpdfext.tpl');
