...
$smarty->assign('subtitulo','ENVIO DE ARCHIVOS BUSQUEDAS PDF WEBPI POR CORREO');
...
if ($vopc==2) {
 ...
 for($cont=0;$cont<$filas_found;$cont++) {
  ...
  //ejecuto el shell
  //$cmd="scp -P 22 /var/www/apl/pedidosexternos/pedidos_tu3.txt www-data@192.8.18.19:/home/taquilla/"; 
  //exec($cmd,$salida);
  //foreach($salida as $line) { 
  //echo "Holaa<br>";	
  //echo "$line<br>"; }

   //if ($tipo=="F") { $vruta="/var/www/apl/sipi/documentos/busquedas/webpi/fonetica/"; }   
   //if ($tipo=="G") { $vruta="/var/www/apl/sipi/documentos/busquedas/webpi/graficas/"; }

   if ((($usrant!=$cuenta) || ($tramant!=$tramite)) || ($indc==1)) {
     //echo " nueva cuenta = $cuenta, con $tramite y $refbusq $nombre ";
     $adjuntos = array();
     ...
     $adjuntos[$count+1]['filename']=$vruta.trim($nropedido).".pdf";
     ...
     $resveri = pg_exec("UPDATE stmbusweb SET estado='3',fecha_envio='$fechahoy',hora_envio='$horactual',user_envio='$usuario' WHERE usuario='$cuenta' AND nro_tramite=$tramite  AND tipo_busq='$tipo' AND ref_busq=$refbusq");
   ...
   if ($usrant!=trim($objs->usuario) || $tramant!=trim($objs->nro_tramite)) {  
     ...
     $vcant = 0;
     $obj_bus = $sql->query("SELECT count(*) AS veredo FROM stmbusweb WHERE estado IN ('1','2') AND nro_tramite='$tramite'");
     ...
     if ($vcant==0) {
       ...
       $actbusq    = $sql1->update1("stztramite","$update_str","nro_tramite='$tramite' AND estatus_tra='05'");
...
function correo($sql_mail,$vemail,$nombre,$tramite,$adjuntos) {
 ...
 $body .= "Anexo al presente, le hacemos llegar el(los) resultado(s) de la(s) b&uacute;squeda(s) fon&eacute;tica(s) y/o gr&aacute;fica(s) solicitada(s) por usted en el Webpi, y relacionada(s) al tramite n&uacute;mero <strong>".$tramite."</strong>.<br><br>";

 $body .= "<strong>Se le recuerda que, de acuerdo al Aviso Oficial de fecha 10/10/2017, las b&uacute;squedas Fon&eacute;ticas y Gr&aacute;ficas tendr&aacute;n una vigencia de 30 d&iacute;as continuos a partir de su fecha de solicitud por el WEBPI. De NO ser usada en dicho periodo, las mismas quedar&aacute;n sin efecto.</strong><br><br>";
 ...
 $body .= "As&iacute; mismo, le informamos que con los resultados de las b&uacute;squedas que reciben por correo, ustedes son los que determinan despu&eacute;s de un an&aacute;lisis de los mismos, si es factible o no seguir con el procedimiento marcario.<br><br>";
 ...
 if ($obj_filas>0) {
   $body .= "   <tr>";
   $body .= "     <td colspan='5' align='left'><b><H3><I>Relaci&oacute;n de B&uacute;squedas Solicitadas</I></H3></b></td>";
   $body .= "   </tr>";
   $body .= "   <tr>";
   $body .= "     <td width='12%' align='left'><b>Pedido Nro.</b></td>";
   $body .= "     <td width='12%' align='left'><b>Referencia</b></td>";
   $body .= "     <td width='58%' align='left'><b>Denominaci&oacute;n</b></td>";
   $body .= "     <td width='07%' align='left'><b>Clase</b></td>";
   ...
   for ($cont=0;$cont<$obj_filas;$cont++) {
     ...
     $body .= "     <td width='07%' align='left'>$objs->clase</td>";
