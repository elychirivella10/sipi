<script language="JavaScript">
 ...
 function browseconsulta(var1,var2,var3) {
   this.interesado=1;
   this.modalidad=1;
   open("z_consuser.php?vuser="+var1.value+"&vmod="+var2.value+"&nconex="+var3.value+"&vmodal="+this.modalidad+"&vtip="+this.interesado,"Ventana","scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }

</script>

<?php
// *************************************************************************************
// Programa: z_usuarios.php 
...
$smarty->assign('subtitulo','Administraci&oacute;n de Usuarios');
...
if (!empty($idcod)) {
  $filasfound = 0;
  $obj_query = $sql->query("SELECT * FROM $tbname_1 WHERE id='$idcod' and role=''");
  if ($obj_query) { 
    $filasfound = $sql->nums('',$obj_query); 
    if ($filasfound==1) {
      // Comienzo de Transaccion 
      pg_exec("BEGIN WORK");
      $result = $sql->del($tbname_1,"id='$idcod'");
      ...
      if ($result) {
        ...
        echo "<p align='right'><b><font size='1' color='#CC0000' face='Tahoma'>Unidad: $idcod, NO Eliminada</font></b></p>"; }
    } 
  }
}  
...
if ($total==0) {
  Mensajenew("La Tabla de Usuarios esta Vacia ...!!!","javascript:history.back();","N");
...
for($cont=0;$cont<$filas_found;$cont++) {
  ...
  $arr_user4[$cont] = $reg['cod_depto'];
  $arr_user5[$cont] = trim($reg['usuario']);
  $arr_user6[$cont] = trim($reg['role']);
  $arr_user7[$cont] = $reg['estatus'];
  $arr_user8[$cont] = $reg['fecha_ing']." - ".$reg['hora_ing'];
  $arr_user9[$cont] = $reg['fecha_elim']." - ".$reg['hora_elim'];
  $reg=pg_fetch_array($resultado);
}
...
$smarty->assign('arr_user9',$arr_user9); 
