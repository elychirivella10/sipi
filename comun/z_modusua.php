...
if ($conx==0) { 
  ...
  if ($conx==1) {
    $salir=1;
    $res_conex = insconex($login,$modulo,'M');
...
if ($vopc==2) {
  ...
  $req_fields = array("cedula","nombre","email","usuario","pass");
  $valores = array($cedula,$nombre,$email,$usuario,$passwd);
  ...
  if ($valemail==2) { 
    mensajenew("La direccion e-mail no es valido  ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); }

  //Verificación de la Cuenta del Usuario
  $ExisUser=Ex_user($cuenta);
  if ($ExisUser != 0) {
    mensajenew("Cuenta del Usuario YA existe en la Base de Datos ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  } 

  //Verificacion del Password
  if ($passwd != $rpasswd) { 
    mensajenew("Password o Clave Incorrecto ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); 
  }
  ...
  $clave = md5($passwd);
  $update_str = "cedula='$cedula',nombre='$nombre',usuario='$usuario',pass='$clave',estatus='$estado',cod_depto='$depto_id',email='$email'";
