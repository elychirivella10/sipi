<script language="JavaScript">
 ...
 function browseconsulta(var1,var2,var3) {
   this.interesado=1;
   this.modalidad=1;
   open("z_consrole.php?vrol="+var1.value+"&vmod="+var2.value+"&nconex="+var3.value+"&vmodal="+this.modalidad+"&vtip="+this.interesado,"Ventana","scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }

</script>

<?php
// *************************************************************************************
// Programa: z_roles.php 
...
$smarty->assign('subtitulo','Administraci&oacute;n de Roles');
...
if (!empty($idcod)) {
  ...
  if ($obj_query) { 
    ...
    if ($filasfound==0) {
      ...
      $horaelm  = hora(); 
      $fechahoy = hoy();
      $update_str = "estado='E',fecha_elim='$fechahoy',hora_elim='$horaelm'";
      $result     = $sql->update("$tbname_1","$update_str","role='$idcod'");
...
$resultado   = pg_exec("SELECT * FROM $tbname_1 WHERE estado='A' ORDER BY nombre OFFSET $inicio LIMIT $cuanto");
