...
<?php
// *************************************************************************************
// Programa: z_fileboletin_neg.php 
...
if ($vopc==2) {
 ...
 $ejecute1 = pg_exec("SELECT substr(trim(telefono1),2,3) as t1, substr(trim(telefono1),6,7) as telefono, concat('SAPI INFORMA: SU SOLICITUD ',solicitud,' FUE NEGADA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
...
UNION
SELECT substr(trim(telefono2),2,3) as t1, substr(trim(telefono2),6,7) as telefono, concat('SAPI INFORMA: SU SOLICITUD ',solicitud,' FUE NEGADA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
 FROM stzderec a, stzevtrd b, stzottid c, stzsolic d
	WHERE a.nro_derecho=b.nro_derecho and a.tipo_mp='M' and
              a.nro_derecho=c.nro_derecho and c.titular=d.titular and c.titular>0 and
              b.evento = 1062 and 
...
UNION
SELECT substr(trim(telefono1),2,3) as t1, substr(trim(telefono1),6,7) as telefono, concat('SAPI INFORMA: SU SOLICITUD ',solicitud,' FUE NEGADA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
...
//Crea el archivo
$vruta="../../sms/";
$txt=$vruta.'Boletin'.$boletin.'_sms_negadas.txt';
$open=fopen($vruta.'Boletin'.$boletin.'_sms_negadas.txt',"w+");
...
fclose($open);  
 ...
 $adjuntos[$count+1]['filename']=$vruta.'Boletin'.$boletin.'_sms_negadas.txt';
...
$smarty->display('z_fileboletin_neg.tpl');
$smarty->display('pie_pag.tpl');
...
// $mail->Subject = "Envio de Solicitud(es) de Busqueda(s) en Linea, Tramite WEBPI No. ".$tramite;
 ...
 $mail->Subject = "SAPI - Mensajes de Texto - Boletin ".$boletin." - NEGADAS de MARCAS";
...
//  $mail->AddAttachment($filetxt);
 ...
 $body .= "Anexo al presente, se hace entrega de archivo plano con los datos de las solicitudes de marcas publicadas como NEGADAS ";
