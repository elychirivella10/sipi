...
$encabezado= "Reporte de Acceso Eventos/Rol por Usuarios";
$linea="_________________________________________________________________________________________________";

//Pantalla Titulos
$smarty->assign('titulo','Sistema de Marcas');
$smarty->assign('subtitulo','Reporte de Acceso Eventos/Rol por Usuarios');
...
if (!empty($vus)) {
   $obj_query = $sql->query("SELECT * FROM $tbname1 WHERE usuario='$vus'"); }
if (!empty($vcd)) {
   $obj_query = $sql->query("SELECT * FROM $tbname1 WHERE cedula='$vcd'"); }
