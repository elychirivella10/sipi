...
WHERE stzsolic.identificacion = '$idsol' and stdobaut.nro_derecho = '$nderec' and stzsolic.titular = stdobaut.titular ");
	$regis = pg_fetch_array($resul_sol);
        $nombre= $regis['nombre']; $cedula=$regis['identificacion']; $domicilio=trim(utf8_decode($regis['domicilio'])).' / '.trim($regis['telefono1']);
...
WHERE stzsolic.identificacion = '$idsol' and stdobart.nro_derecho = '$nderec' and stzsolic.titular = stdobart.titular ");
	$regis = pg_fetch_array($resul_sol);
        $nombre= $regis['nombre']; $cedula=$regis['identificacion']; $domicilio=trim(utf8_decode($regis['domicilio'])).' / '.trim($regis['telefono1']);
	return array($cedula,$nombre,$domicilio);
 }
 ...
 if ($tipo == 'TITULAR'){
	$resul_sol=pg_exec("SELECT stzsolic.nombre, stzsolic.identificacion, stdobtit.domicilio, stzsolic.telefono1 FROM stzsolic,stdobtit WHERE stzsolic.identificacion = '$idsol' and stdobtit.nro_derecho = '$nderec' and stzsolic.titular = stdobtit.titular ");
	$regis = pg_fetch_array($resul_sol);
        $nombre= $regis['nombre']; $cedula=$regis['identificacion']; $domicilio=trim(utf8_decode($regis['domicilio'])).' / '.trim($regis['telefono1']);
       return array($cedula,$nombre,$domicilio);
 }
 ...
 if ($tipo == 'PRODUCTOR'){
	$resul_sol=pg_exec("SELECT stzsolic.nombre, stzsolic.identificacion, stdobpro.domicilio, stzsolic.telefono1   FROM stzsolic,stdobpro WHERE stzsolic.identificacion = '$idsol' and stdobpro.nro_derecho = '$nderec' and stzsolic.titular = stdobpro.titular  ");
	$regis = pg_fetch_array($resul_sol);
        $nombre= $regis['nombre']; $cedula=$regis['identificacion']; $domicilio=trim(utf8_decode($regis['domicilio'])).' / '.trim($regis['telefono1']);
...
if ($reg['tipo_obra']=='IE') {
    ...
    $pdf->Cell(30,8,'Nombre de la Agrupación:',0,0);
    $pdf->Cell(85,8,$resgru['nombre_grupo'],0,0);
    $res_gen=pg_exec("SELECT * FROM stdgenero WHERE cod_genero ='$resgru[tipo_agrupa]'");
    $resgen = pg_fetch_array($res_gen);
    $pdf->Cell(30,8,'Tipo de Agrupación:',0,0);
    ...
    $pdf->Cell(80,8,trim($regis['nombre']),0,0);
    $pdf->Cell(12,8,'Documento de Ident.:',0,0); 
    $pdf->Cell(8,8,$regis['identificacion'],0,1);
    ...
    $pdf->Cell(8,8,$regis['telefono1'],0,0);
    $pdf->Cell(12,8,'Fax:',0,0);
...
if ($reg['tipo_obra']=='PF' or $reg['tipo_obra']=='IE') {
    ...
    if ($reg['tipo_obra']=='IE'){
       ...
       if ($respf['tipo_fijacion']=='S') {$pdf->Cell(100,8,'Sonora',0,0);}
       if ($respf['tipo_fijacion']=='A') {$pdf->Cell(100,8,'Audiovisual',0,0);}}
    $pdf->Cell(15,8,utf8_decode('Año de Primera Publicación:'),0,0); 
    $pdf->Cell(100,8,$anno_1publica,0,0);
    $pdf->Cell(30,8,utf8_decode('Año de Fijación:'),0,0);
   $pdf->Cell(100,8,$respf['anno_fijacion'],0,1);
}
...
if ($reg['tipo_obra']== 'OM' ) {
    ...
    if ($resmusic['letra']=='V') {$pdf->Cell(30,8,'Con letra',0,0);}   
...
$pdf->Draw_Header();
	...
	$idsol= trim($resautor['doc_artista']);
	$info = consultar($idsol,$nderec,$tipo);
      $data = Array();
      $data[0]['TEXT'] = $info[0];
      $data[1]['TEXT'] = $info[1];   
...
//Caso para Obra Escenica, OE, AR, PF datos de los PRODUCTORES
if ($reg['tipo_obra']== 'OE' or $reg['tipo_obra']=='AR' or $reg['tipo_obra']=='PF') {
...
$pdf->Draw_Header();
    ...
    for($cont3=0;$cont3<$filas_found_regprod;$cont3++)  {
  	//Busqueda de los datos del Productor
        $idsol= trim($resprod['doc_productor']);
	$info = consultar($idsol,$nderec,$tipo);
      $data = Array();
      $data[0]['TEXT'] = $info[0];
      $data[1]['TEXT'] = $info[1];   
      $data[2]['TEXT'] = $info[2];
      $pdf->Draw_Data($data);
      $resautor = pg_fetch_array($res_prod);
    }   
}
 $pdf->Draw_Table_Border();
}
...
$header_type = Array();
		...
		$i=1;
		$header_type[$i] = $table_default_header_type;
		$header_type[$i]['TEXT'] = utf8_decode("Editor ");
		$header_type[$i]['WIDTH'] = 80;
		$i=2;
		$header_type[$i] = $table_default_header_type;
		$header_type[$i]['TEXT'] = utf8_decode("Domicilio");
...
$pdf->Draw_Header();
	...
	$idsol= trim($resedt['doc_edimpres']);
	$info = consultar($idsol,$nderec,$tipo);
      $data = Array();
      $data[0]['TEXT'] = $info[0];
      $data[1]['TEXT'] = $info[1];   
