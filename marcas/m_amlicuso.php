...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) { 
         $sql->disconnect();
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amlicuso.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amlicuso.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect();
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amlicuso.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='' || $vlicen=='') {
        $sql->disconnect();
        mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS ...!!!','m_amlicuso.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
	 }
      ...
      if ($esmayor==1) {
        $sql->disconnect();
        mensajenew('ERROR: No se puede cargar un evento previo a la Fecha de la Solicitud ...!!!','m_amlicuso.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
      }

	   if (empty($vfactura)) { 
         $sql->disconnect();
         mensajenew('ERROR: DEBE COLOCAR EL NUMERO DE LA FACTURA ...!!!','m_amlicuso.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	   }
...
// nrgg }
      ...
      $obj_filas = $sql->nums('',$obj_query);
      $cant_usada= $obj_filas;
      //if ($obj_filas!=0) {
      //  mensajenew('ERROR: Factura '.$nfactura.' YA usado anteriormente en otro Servicio ...!!!','m_amlicuso.php','N');
      ///  $smarty->display('pie_pag.tpl'); exit(); }
      if ($obj_filas!=0) {
// nrgg {
	     $objs = $sql->objects('',$obj_query);
...
// nrgg }
      ...
      if ($objfilas==0) {
        mensajenew('ERROR: Factura NO existe en la Base de Datos del SISFAC ...!!!','m_amlicuso.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
      ...
      $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('0106','0106E','0106E1','0106E2') AND fac_id=$fac_id"); 
      $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
      if ($objtotdtalle==0) {
        mensajenew('ERROR: Factura NO presenta ning&uacute;n Servicio de Licencia de Uso de Registro de Marcas ...!!!','m_amlicuso.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
      $objsdta = $mysql->objects_mysql('',$objdetalle);
      $codservi = $objsdta->ser_id;
      $cant_serv= $objsdta->dtalle1_cantidad_ser;
      if ($cant_usada>=$cant_serv) {
          mensajenew('ERROR: Factura '.$nfactura.' YA usada en '.$cant_usada.' registro(s) de marcas..!!!','m_amlicuso.php','N');
          $smarty->display('pie_pag.tpl'); exit(); }
...
// nrgg }
      ...
      $insert_str = "'$nfac','$fechafac',0,0,0,'$sede','L',$vcant_am";  
      ...
      if ($can_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','m_amlicuso.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           mensajenew("ERROR: Falla de Ingreso en la B.D. Transacciones Abortadas ...!!!", "m_amlicuso.php","N");
           $smarty->display('pie_pag.tpl'); exit();   }
