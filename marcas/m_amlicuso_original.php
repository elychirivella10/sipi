...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) { 
         $sql->disconnect();
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amlicuso_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amlicuso_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect();
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amlicuso_original.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='' || $vlicen=='') {
        $sql->disconnect();
        mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS ...!!!','m_amlicuso_original.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
	 }
      ...
      if ($esmayor==1) {
        $sql->disconnect();
        mensajenew('ERROR: No se puede cargar un evento previo a la Fecha de la Solicitud ...!!!','m_amlicuso_original.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
      }

	   ///if (empty($vfactura)) { 
      //   $sql->disconnect();
      //   mensajenew('ERROR: DEBE COLOCAR EL NUMERO DE LA FACTURA ...!!!','m_amlicuso_original.php','N');
	   //   $smarty->display('pie_pag.tpl'); exit(); 
	   //}
      ...
      //$obj_filas = $sql->nums('',$obj_query);
      //if ($obj_filas==0) {
      //  mensajenew('ERROR: Servicio debe ser cargado en otra pantalla con Factura '.$nfactura.' ...!!!','m_amlicuso_original.php','N');
      //  $smarty->display('pie_pag.tpl'); exit(); }
      //$objs = $sql->objects('',$obj_query);
      //$fechafac = $objs->fecha_factura; 
      ...
      $resule=pg_exec("select * from stzevtrd where nro_derecho='$vderh' and 
       ...
       if ($cantfil>0) {
         $sql->disconnect();
         mensajenew('ERROR: El Evento ya fue Cargado ...!!!','m_amlicuso_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($can_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','m_amlicuso_original.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           mensajenew("ERROR: Falla de Ingreso en la B.D. Transacciones Abortadas ...!!!", "m_amlicuso_original.php","N");
           $smarty->display('pie_pag.tpl'); exit();   }
   ...
   $smarty->display('m_amlicuso_original.tpl'); 
