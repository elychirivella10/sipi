...
function gestionvienap(var1,var2,var3,var4) {
  open("adm_bviena.php?vsol="+var1.value+"-"+var2.value+"&vtex="+var3.value+"&vmod="+var4.value,"Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
// Modificado y corregido II Semestre 2009 - por Romulo Mendoza el 07/09/2009   
// *************************************************************************************
...
if (!$obj_query) { 
  Mensage_Error("ERROR: Problema al intentar realizar la consulta en la tabla $tbname_3 ...!!!");
  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  
$filas_found=$sql->nums('',$obj_query);
if ($filas_found==0) {
  Mensage_Error("ERROR: Tabla de Viena vacia...!!!");
  $smarty->display('pie_pag.tpl');  $sql->disconnect(); exit(); } 
...
if ($vopc==1) {
    ...
    if (empty($vsol1) && empty($vsol2)) {
      mensajenew('ERROR: No introdujo ningún valor de Expediente ...!!!','m_solviena.php','N');
      $smarty->display('pie_pag.tpl'); exit(); }

    $resultado=pg_exec("SELECT * FROM $tbname_2 WHERE solicitud='$vsol' and solicitud!='' and tipo_mp='M' ");
    if (!$resultado) { 
      mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_solviena.php','N');
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
    }	 
    $filas_found=pg_numrows($resultado); 
    if ($filas_found==0) {
      mensajenew('AVISO: NO EXISTEN DATOS ASOCIADOS ...!!!','m_solviena.php','N');
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
    ...
    if (($modal!='G') && ($modal!='M')) {
      mensajenew('ERROR: SOLICITUD DEBE SER GRAFICA O MIXTA PARA CLASIFICARLA ...!!!','m_solviena.php','N');
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  ...
  if (file_exists($nameimage)) { }
    ...
    $sql->del("$tbname_9","solicitud='$vsol'");
    $obj_query = $sql->query("SELECT * FROM $tbname_5,$tbname_3 WHERE $tbname_5.nro_derecho='$vdere' AND $tbname_5.ccv = $tbname_3.ccv ORDER BY $tbname_5.ccv"); 
    $obj_filas = $sql->nums('',$obj_query);
    $objs = $sql->objects('',$obj_query);
    if ($obj_filas!=0) {
      for($i=0;$i<$obj_filas;$i++) { 
        ...
        $insccv = $sql->insert("$tbname_9","","$insert_str","");
        $objs = $sql->objects('',$obj_query); 
      }
    }
}
...
//	  pg_exec("BEGIN WORK");
  //        pg_exec("LOCK TABLE stmtmpcvs IN SHARE ROW EXCLUSIVE MODE");
   ...
   if ($vopc==5 && $accion=='Guardar') {
     ...
     $resultmp=pg_exec("SELECT * FROM stmtmpccv WHERE solicitud='$vsol'");
