...
if (empty($fecsold) and empty($fecsolh)) {
  mensajenew("Informacion Vacia ...!!!","javascript:history.back();","N");
 ...
 if ($filas_resultado ==0) {
   mensajenew("No Hay Informacion encontrada ...!!!","javascript:history.back();","N");
   $smarty->display('pie_pag.tpl'); exit(); 
 }
 ...
 if ($vopc==5) {
   ...
   for($cont=1;$cont<=$total;$cont++) { 
     ...
     $vdoc = $reg[documento];
     if ($vtmp=='on') {
       $insert_val = "$vorden,'$vsolh',$vdoc,'N'";
       $sql->insert("stmofdev","","$insert_val","");
     }
     $reg=pg_fetch_array($cantidad);
   }
   Mensaje3("DATOS GUARDADOS CORRECTAMENTE con la Orden $vorden ...!!!","$vurl","m_ofidevreg.php?orden=$vorden","m_rpt1binsr.php?vped=$vsol1");
      ...
      for($cont2=0;$cont2<$cantcampos;$cont2++) {
          ...
          switch ($namecampo) {
            case "Solicitud":
              ...
              $nameimagen="../imagenes/marcas/ef".$dirano."/".$vs1.$vs2.".jpg"; 
              break;
            case "Clase":
              $vclase = $reg['clase'];
              break;
