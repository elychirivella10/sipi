...
if ($vopc==0) {
  ...
  if ($letrafac!='F') {
    ...
    $objs = $sql->objects('',$obj_query);
    if ($obj_filas==0) {
     Mensajenew('Clasificaci&oacute;n Internacional de Viena Vacia(s) ...!!!','javascript:history.back();','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  } else {
    //$obj_query = $sql->query("SELECT * FROM $tbname_9 WHERE factura='$recibo' AND cod_planilla='$planilla'"); 
    $obj_query = $sql->query("SELECT * FROM $tbname_9 WHERE solicitud='$v1'"); 
    $obj_filas = $sql->nums('',$obj_query);
    $objs = $sql->objects('',$obj_query);
    if ($obj_filas==0) {
  ...
  $vsel = "";
  $vselect = "INSERT INTO $ntabla1 ";
  $operador='AND';
  // Se recorre y se trae todas aquellas solicitudes por cada Codigo de Viena que se introduce      
  for($i=0;$i<$obj_filas;$i++) { 
    $varsolc = $objs->ccv;
    if ($operador=='OR') { 
      if ($vsel!= '') { $vsel = $vsel." INTERSECT "; } }
    else {  
      if ($vsel!= '') { $vsel = $vsel." UNION "; } }
    ...
    $vsel = $vsel."SELECT '$fechahoy','$horahoy','$usuario','$v1',solicitud,clase,ind_claseni FROM viena WHERE ccv="."'".$varsolc."'";
    $objs = $sql->objects('',$obj_query); 
  }
  $vselect = $vselect.$vsel;
  //echo "select=$vselect";
  //$resultado=pg_exec($vselect);
  ...
  // Se filtra eliminando solicitudes repetidas 
  ...
  //Filtro por Clasificación Internacional de Niza $vaclas y a la vez por sus asociadas Clases Internaciones y Nacionales en $vclabus  
