...
$cantidad=pg_exec("SELECT stzofdev.orden, stzofdev.solicitud, stzofdev.documento, stzofdev.estatus, stzderec.nro_derecho FROM $tablas WHERE stzofdev.estatus='N' AND stzofdev.fecha_carga='$fechahoy' AND stzofdev.orden='$vorden' AND stzofdev.tipo_mp='M' AND stzofdev.solicitud = stzderec.solicitud");
...
// Imprimir Oficio PDF
 ...
 for($cont=1;$cont<=$total;$cont++) { 
   ...
   $obj_query = $sql->query("SELECT nro_derecho FROM stzderec WHERE solicitud='$v1' AND tipo_mp='M'"); 
   ...
   for ($j=0; $j<$filas_coded; $j++) {
    ...
    $res_otros=pg_exec("SELECT * FROM stzotrde WHERE nro_derecho = '$nderec' AND derecho = 'M' AND grupo = 'A' AND documento=$vdoc");
   ...
   $pdf->MultiCell(95,4,utf8_decode('  MARGARITA VILATIMO'),0,'L',0);
   $pdf->SetFont('Arial','',9);
   $pdf->MultiCell(100,4,'  Registradora de la Propiedad Industrial ',0,'L',0);

   $sol='';
   $update_str = "estatus='S'";
   $sql->update("stzofdev","$update_str","solicitud='$vsolh' AND documento='$vdoc' AND tipo_mp='M'");
//   $reg=pg_fetch_array($cantidad);

   if  ($cont!=$total) { $pdf->AddPage(); 
   $reg = pg_fetch_array($cantidad); }

  //   if ($cont+1<$total) {$pdf->AddPage();}
 }
//Eliminar datos temporales 
$sql->del("stmtmpam","usuario='$usuario'");
