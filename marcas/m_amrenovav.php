...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) { 
        $sql->disconnect();
        mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amrenovav.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amrenovav.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect();
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amrenovav.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='') {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS ...!!!','m_amrenovav.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
	 }
      ...
      if ($esmayor==1) {
         $sql->disconnect(); 
         mensajenew('ERROR: No se puede cargar un evento previo a la Fecha de la Solicitud ...!!!','m_amrenovav.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
         }

	   if (empty($vfactura)) { 
         $sql->disconnect();
         mensajenew('ERROR: DEBE COLOCAR EL NUMERO DE LA FACTURA ...!!!','m_amrenovav.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	   }
         
	   if (empty($fechafac)) { 
         $sql->disconnect();
         mensajenew('ERROR: DEBE COLOCAR LA FECHA DE LA FACTURA ...!!!','m_amrenovav.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	   }
      ...
      $obj_filas = $sql->nums('',$obj_query);
      if ($obj_filas!=0) {
        mensajenew('ERROR: Factura '.$nfactura.' YA usado anteriormente en otro Servicio ...!!!','m_amrenovav.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
      ...
      $insert_str = "'$nfac','$fechafac',0,0,0,'$sede','R'";  
      ...
      if ($can_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_amrenovav.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           mensajenew("ERROR: Falla de Ingreso en la B.D. Transacciones Abortadas ...!!!","m_amrenovav.php","N");
           $smarty->display('pie_pag.tpl'); exit();   }
   }
   ...
   $smarty->display('m_amrenovav.tpl'); 
