  ...
  if ($vfil==0) 
   { // Good Nuevo 
       ...
       for($cont=0;$cont<$vfil;$cont++) {
         ...
         if ($vb[$cont]=="on") {
           $resulcon=pg_exec("select * from stztmpage where solicitud='$vsol' and agente='$vtit[$cont]'");
           if (pg_numrows($resulcon)==0) {
             $col_campos = "solicitud,agente,nombre,domicilio,tipo_mp";
             $insert_str = "'$vsol','$vtit[$cont]','$vnom[$cont]','$vdom[$cont]','M'";
...
if ($vmod=='Buscar/Eliminar')
   {
    for($cont=0;$cont<$vfil;$cont++) 
       ...
       if ($vb[$cont]=="on")
          {$resultado=pg_exec("DELETE FROM stztmpage WHERE solicitud='$vsol' AND agente='$ti[$cont]' AND nombre='$no[$cont]' AND domicilio='$do[$cont]'"); 
          } 
       }
   }
