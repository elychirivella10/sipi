...
*/
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
         $sql->disconnect();
         mensaje('ERROR: PROBLEMA  AL PROCESAR LA BUSQUEDA ...!!!','m_cambtitu_original.php');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($filas_found==0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensaje('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_cambtitu_original.php');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      //               Corresponde a una Anotacion Marginal','m_cambtitu.php','N');
   ...
   if ($vopc==3) {     
      ...
      if ($objfilas==0) 
      {  $sql->disconnect();
         mensajenew('AVISO: Debe cargar el nuevo titular. Transaccion abortada ..!!!','javascript:history.back();','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      } else {
      ...
      $fechahoy = hoy();
       ...
       if ($cantfil>0) {
         $sql->disconnect();
         mensajenew('AVISO: El Evento ya fue Cargado ...!!!','m_cambtitu_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
       }
       ...
       // Segundo evento: 234
       $resule=pg_exec("select * from stzevtrd where nro_derecho='$vderh' and evento=1234 and 
                       documento='$vdoc'");
       $rege = pg_fetch_array($resule);
       $cantfil = pg_numrows($resule);
       if ($cantfil>0) {
      ...
      for ($contobj=0;$contobj<$numo;$contobj++) {
          ...
          {  pg_exec("ROLLBACK WORK");
             $sql->disconnect();
             $smarty->display('encabezado1.tpl');
             mensajenew('AVISO: Domicilio del Titular Vacio.  Transaccion Abortada ...!!!','m_cambtitu_original.php','N');
             $smarty->display('pie_pag.tpl'); exit();
          }
      ...
      if ($cant_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_cambtitu_original.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
   ...
   $smarty->display('m_cambtitu_original.tpl'); 
