...
$smarty ->assign('subtitulo','Publicaciones de Renovacion/Cesion/Fusion/Licencia de Uso/Cambio de Titular, Domicilio'); 
...
$smarty ->assign('fechahoy',$fecha); 
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) { 
         ...
         mensajenew('ERROR AL PROCESAR LA BUSQUEDA...!!!','m_marginal.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $smarty->display('encabezado1.tpl');
         mensajenew('NO EXISTEN DATOS ASOCIADOS...!!!','m_marginal.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $smarty->display('encabezado1.tpl');
         mensajenew('Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_marginal.php','N');
	      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vtipo=='N' || $vtipo=='L' || $vtipo=='C' || $vtipo=='F') {
        $restitu=pg_exec("SELECT * FROM $tbname_9 where solicitud='$vsolh'");
        $filas_titular=pg_numrows($restitu); 
        if ($filas_titular==0) {
          Mensajenew("Expediente $vsolh sin ningun Titular Nuevo o Cesionario u Otorgado ...!!!","javascript:history.back();","N");
      ...
      if ($vtipo=='D') {
        if ($vcomenta=='') {
          $smarty->display('encabezado1.tpl');
          Mensajenew('NO selecciono el Titular Inicial o anterior o actual ...!!!','javascript:history.back();','N');
	       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      }
      ...
      if ($vtipo=='R') {
        if (empty($vfecv) || empty($vclasint) || $vclasint==0) {
          $smarty->display('encabezado1.tpl'); 
          Mensajenew("No ha ingresado datos de Vencimiento o Clase Internacional para la Renovacion ...!!!","javascript:history.back();","N");
          $smarty->display('pie_pag.tpl'); exit();  }
      } else {
        $vclasint=0;
      }
      ...
      if ($vseldoc=='S') {
        if ($vdoc=='') { 
          $smarty->display('encabezado1.tpl');
          mensajenew('NO ha indicado el N&uacute;mero de Documento ...!!!','javascript:history.back();','N');
	       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
	   } else { $vdoc=0; } 
      
      if ($vpub=='') { 
        $smarty->display('encabezado1.tpl');
        mensajenew('NO selecciono si ya fue o va hacer Publicado ...!!!','javascript:history.back();','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
	   }
      ...
      if ($vpub=='S') {
        if ($vbol=='' || $vbol==0) { 
          $smarty->display('encabezado1.tpl');
          Mensajenew('NO ha indicado el Bolet&iacute;n donde fue Publicado ...!!!','javascript:history.back();','N');
	       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
	   }

      if ($vbol=='' || $vbol==0) { 
        $smarty->display('encabezado1.tpl');
        Mensajenew('NO ha indicado el Bolet&iacute;n donde fue o sera Publicado ...!!!','javascript:history.back();','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
      ...
      { 
       ...
       if (empty($vfecv)) {
         $insert_campos= "nro_derecho,solicitud,documento,nro_docum,tramitante,agente,codtit1,titular1,codtit2,titular2,domicilio_ant,domicilio,pais,tipo_tramite,publicado,boletin,tipo_mp,usuario,fecha_trans,hora,verificado";
         $insert_valores= "'$vder','$vsolh','$vseldoc',$vdoc,'$vtraf',$vagef,$vcodtit,'$vcomenta',$act_titular,'$regtitu[nombre]','$vdomant','$regtitu[domicilio]','$regtitu[nacionalidad]','$vtipo','$vpub',$vbol,'M','$usuario','$vfec','$hh','$vok'";
       } 
       else {
         $insert_campos= "nro_derecho,solicitud,documento,nro_docum,vencimiento,tramitante,agente,codtit1,titular1,codtit2,titular2,domicilio_ant,domicilio,pais,tipo_tramite,publicado,boletin,tipo_mp,usuario,fecha_trans,hora,verificado,claseint";
         $insert_valores= "'$vder','$vsolh','$vseldoc',$vdoc,'$vfecv','$vtraf',$vagef,$vcodtit,'$vcomenta',$act_titular,'$regtitu[nombre]','$vdomant','$regtitu[domicilio]','$regtitu[nacionalidad]','$vtipo','$vpub',$vbol,'M','$usuario','$vfec','$hh','$vok','$vclasint'";
       }
      ...
      if ($numerror==0) {
        ...
        $smarty->display('encabezado1.tpl');
        mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_marginal.php','S');
        $smarty->display('pie_pag.tpl'); exit(); } 
      else {
   ...
   $smarty ->assign('lpropietario','Licenciatario, Sobreviviente, Cesionario, Titular Nuevo, Cambio de Domicilio,Renovacion'); 
