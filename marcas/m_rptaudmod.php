...
$smarty->assign('subtitulo','Reporte de Auditorias de Solicitudes Modificadas');
$smarty->assign('login',$login);
$smarty->assign('fechahoy',$fecha);

//PDF Encabezados
$encab_principal= "Sistema de Marcas";
$encabezado= "Auditoria de Solicitudes Modificadas";
...
if(!empty($fecsold) and !empty($fecsolh)) { 
	if(!empty($where)) {
	   $where = $where." and"." ((stzaudmod.fecha >= '$fecsold') and (stzaudmod.fecha <='$fecsolh'))";
	   $titulo= $titulo." Desde:"." $fecsold"." Hasta:"." $fecsolh";
	}
	else { 
		$where = $where." ((stzaudmod.fecha >= '$fecsold') and (stzaudmod.fecha <='$fecsolh'))";
...
if(!empty($usuario)) { 
	if(!empty($where)) {
	   $where = $where." and"." (trim(stzaudmod.usuario)='$usuario')";
  	   $titulo= $titulo." Usuario:"."$usuario";  
	}
	else { 
		$where = $where." (trim(stzaudmod.usuario)='$usuario')";
 	   $titulo= $titulo." Usuario:"."$usuario";
	}
}
...
$where = $where." and stzderec.solicitud=stzaudmod.solicitud";
...
stzderec.poder,stmmarce.modalidad,stzsolic.nombre as ntitular,stzottid.domicilio,stzottid.nacionalidad,stzaudmod.fecha,stzaudmod.hora,stzaudmod.usuario
 FROM  stmmarce, stzottid, stzsolic, stzderec, stzaudmod
 WHERE $where  ORDER BY 1"); 

echo "SELECT stzderec.solicitud, stzderec.fecha_solic,stzderec.nombre,stmmarce.clase,stmmarce.ind_claseni,stzderec.tipo_derecho,stzderec.tramitante,stzderec.agente,
stzderec.poder,stmmarce.modalidad,stzsolic.nombre as ntitular,stzottid.domicilio,stzottid.nacionalidad,stzaudmod.fecha,stzaudmod.hora,stzaudmod.usuario
 FROM  stmmarce, stzottid, stzsolic, stzderec, stzaudmod
 WHERE $where  ORDER BY 1";
...
if (!$resultado)    { 
     $smarty->display('encabezado1.tpl');
     mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_rptpaudmod.php','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
$filas_found=pg_numrows($resultado); 
if ($filas_found==0)    {
     $smarty->display('encabezado1.tpl');
     mensajenew('ERROR: No existen Datos Asociados ...!!!','m_rptpaudmod.php','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 

$registro = pg_fetch_array($resultado);
$filas_resultado=pg_numrows($resultado); 

$cantidad=pg_exec("SELECT DISTINCT ON (stzderec.solicitud) stzderec.fecha_solic,stzderec.nombre,stmmarce.clase,stmmarce.ind_claseni,stzderec.tipo_derecho,stzderec.tramitante,stzderec.agente,stzderec.poder,stmmarce.modalidad,stzsolic.nombre as ntitular,stzottid.domicilio,stzottid.nacionalidad,stzaudmod.fecha,stzaudmod.hora,stzaudmod.usuario
 FROM  stmmarce, stzottid, stzsolic, stzderec, stzaudmod
 WHERE $where "); 
...
echo "total=$filas_resultado-$cantidad";
//Inicio del Pdf
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado;$cont++) { 
   ...
   $pais_nombre=pais($registro['nacionalidad']);
	...
	$tiempo = $registro['fecha']." - ".$registro['hora'];
	$data[8]['TEXT'] = $tiempo;
	$data[9]['TEXT'] = trim(utf8_decode($registro['usuario']));	
	$registro = pg_fetch_array($resultado);
	$pdf->Draw_Data($data);
  }
