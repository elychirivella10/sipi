...
$resultado=pg_exec("SELECT DISTINCT ON(stzderec.solicitud) stzderec.solicitud, stzderec.fecha_solic, stzderec.nombre, stmmarce.modalidad,stmmarce.clase,stmmarce.ind_claseni, stzderec.estatus, stzsolic.nombre as titular, telefono1,telefono2,email,identificacion, idtramitante,agente,tramitante
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado;$cont++) { 
	...
	$data[2]['TEXT'] = $registro['clase'].'-'.$registro['ind_claseni'];
        $data[3]['TEXT'] = trim(utf8_decode($registro['titular']));
   $datostitular='';
   $tel=trim($registro['telefono1']).' / '.trim($registro['telefono2']);
   $ema=trim($registro['email']);
	$data[4]['TEXT'] = $ema;
	$data[5]['TEXT'] = $tel;
	$data[6]['TEXT'] = $agentram_nom;
	$data[7]['TEXT'] = $agentram_tel;
	$registro = pg_fetch_array($resultado);
	$pdf->Draw_Data($data);
  }
