...
$smarty->assign('subtitulo','Notificacion de Cancelaciones del Bol. 600 a Anular');
...
if ($vopc==2) 
{
  ...
  if (!$obj_query) { 
    mensajenew('Problema al intentar realizar la consulta en la tabla $tbname_2 ...!!!','m_nulcan600.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  ...
  $lcwhere = " WHERE a.tipo_mp='M' AND a.estatus=1842 AND (a.nro_derecho=b.nro_derecho) AND b.evento IN (1312) AND documento='600' AND fecha_trans='27/02/2020' AND fecha_event='07/02/2020' AND a.nro_derecho IN (SELECT nro_derecho FROM stzevtrd WHERE evento=1196 AND nro_derecho IN (SELECT nro_derecho FROM stzevtrd WHERE evento=1195 AND documento='588')) ORDER BY a.solicitud";
  ...
  if (!$resultado) { 
    mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_nulcan600.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 

  if ($filas_found==0) {
    mensajenew('ERROR: NO existen DATOS Asociados ...!!!','m_nulcan600.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else
  {
    $numerror  = 0;
    ...
    for($cont=0;$cont<$filas_found;$cont++) 
    {
      ...
      if (!$obj_query2) { 
        mensajenew('ERROR: Problema al intentar realizar la consulta en la tabla $tbname_3 ...!!!','m_nulcan600.php','N');
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  ...
  if ($numerror==0) {
    ...
    Mensajenew("'$filas_found' SOLICITUDES ACTUALIZADAS CORRECTAMENTE ...!!!","m_nulcan600.php","S");
    $smarty->display('pie_pag.tpl'); exit();
  }
  else {
...
$smarty->display('m_nulcan600.tpl');
