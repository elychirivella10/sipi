...
if ($vopc==1) {
  ...
  //if (($estatus==1001) || ($estatus==1002) || ($estatus==1004) || ($estatus==1006) || ($estatus==1025) || ($estatus==1030)) { }  
  //else { 
  //  //Mensajenew("Solo se puede modificar Expedientes en Estatus 1-SOLICITUD PRESENTADA ...!!!","m_manteni.php?vopc=4&vaccion=2&vauxnum=0","N");
  //  Mensajenew("Solicitud en estatus NO Modificable ...!!!","m_manteni.php?vopc=4&vaccion=2&vauxnum=0","N");
  //  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  //}
  ...
  for ($contobj=0;$contobj<$obj_filas;$contobj++) {
     ...
     $insert_str = "'$varsol','$vcod','$nomb','$domi','$pais','$indo','M','$paisdom'";
...
if (($vopc==2) || ($vopc==6)) {
  ...
  if ($accion==1) {
    ...
    if ($numtitu==0 AND $numccv==0 AND $inslema AND $inslogo AND 
      ...
      Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","m_actualiza.php?vopc=4","S");
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
...
$smarty->display('m_actualiza.tpl');
