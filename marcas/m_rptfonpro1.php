...
$smarty->assign('subtitulo','Auditor&iacute;a Externa de B&uacute;squedas Procesadas');
...
if(!empty($desde) && !empty($hasta)) {
  ...
  if(empty($where)) {
    $where = $where." (stmbusqueda.f_proceso >='$desde' AND stmbusqueda.f_proceso <='$hasta')";
...
function Header()
{
   ...
   $this->Cell(0,6,'Busqueda Fonetica',0,1,'C');
   //$this->Image("../imagenes/milco1.jpg",160,5,40,10,'JPG');
   $this->SetFont('Arial','BU',10);
   $this->Cell(0,7,'Auditoria de Busquedas Procesadas a Entregar',0,1,'C'); 
   ...
   $this->Cell(16,8,'F. Recibo',1,0,'C'); 
   $this->Cell(18,8,'Hora',1,0,'C'); 
   $this->Cell(12,8,'Recibo',1,0,'C'); 
   $this->Cell(11,8,'Pedido',1,0,'C'); 
   $this->Cell(40,8,'Solicitante',1,0,'C');
   $this->Cell(53,8,'Denominacion',1,0,'C');
   $this->Cell(8,8,'Clase',1,0,'C'); 
   $this->Cell(16,8,'Usuario',1,0,'C'); 
   $this->Cell(9,8,'Prior.',1,0,'C'); 
   $this->Cell(17,8,'Procesada',1,1,'C'); 
...
$pdf->AliasNbPages();
  ...
  $res_pedido=pg_exec("SELECT f_pedido,hora_c,nro_pedido,nro_recibo,solicitante,tipobusq,identificacion,indole,telefono,usuario,denominacion,clase,f_proceso FROM stmbusqueda WHERE $where");
  $filasfound=pg_numrows($res_pedido);
  $total=$filasfound;
  //echo "total= $total ";
  if ($filasfound==0)    {
     $smarty->display('encabezado1.tpl');
     mensajenew('AVISO: No existen Datos Asociados ...!!!','m_rptfonpro.php','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  ...
  for($cont=0;$cont<$filasfound;$cont++)   { 
     ...
     $fechapro   =$regef['f_proceso'];

     if ($tipoprio=="L") { $tipo="Hab."; } else { $tipo="Nor."; }
     ...
     //    break;
     //}
     $solicitante = substr($solicitante,0,20);             
     $pdf->SetFont('Arial','',8);
     $pdf->ln(2);
     $pdf->Cell(16,8,$fecharec,0,0); 
     $pdf->Cell(18,8,$hora,0,0);
     $pdf->Cell(12,8,$recibo,0,0);
     $pdf->Cell(11,8,$npedido,0,0);
     $pdf->Cell(40,8,utf8_decode($solicitante));     
     $pdf->Cell(55,8,utf8_decode(substr($vmarca,0,50)),0,0); 
     $pdf->Cell(7,8,$vclase,0,0); 
     $pdf->Cell(16,8,$vusuario,0,0);
     $pdf->Cell(8,8,$tipo,0,0); 
     $pdf->Cell(10,8,$fechapro,0,1); 
  ...
  $pdf->Cell(25,8,"Total Entregadas: ".$total,0,1);
