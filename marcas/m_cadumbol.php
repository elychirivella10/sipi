...
if ($vopc==2) 
{
  ...
  $lcwhere = " WHERE a.tipo_mp='M' AND a.estatus=1400 AND (a.nro_derecho=b.nro_derecho) AND b.evento IN (1060,1097,1122,1181,1182,1956,1989,1928) AND b.estat_ant IN (1101,1390,1027,1399,1127,1102,1395,1008,1191)";
  ...
  if (!$resultado) { 
    mensajenew('Error al Procesar la Busqueda ...!!!','m_cadubol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 
  if ($filas_found==0) {
    mensajenew('NO existen DATOS Asociados ...!!!','m_cadubol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else 
  { 
    $numerror = 0;
    ...
    for($cont=0;$cont<$filas_found;$cont++) 
    {
      ...
      if (!empty($update_str))
        { $act_dere = $sql->update("$tbname_4","$update_str","nro_derecho='$vder' AND estatus=1400"); }
  ...
  if ($numerror==0) {
    ...
    Mensajenew("'$filas_found' SOLICITUDES ACTUALIZADAS CORRECTAMENTE ...!!!","m_cadubol.php","S");
