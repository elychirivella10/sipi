...
function limpiar()	{
	...
	function checkear(f)	{
				function no_prever() {
					alert("El archivo buscado y seleccionado no es válido, el formato de la imagen debe ser PNG ...!!!");
					limpiar();
				}
				function prever() {
				...
				(/\.(png)$/i.test(f.value)) ? prever() : no_prever();
			}
...
$fecha     = fechahoy();
$subtitulo = "Solicitud(es) de B&uacute;squeda(s) Fonetica";
$sql = new mod_db();
$vfac=$_GET['vfac'];
$vmod=$_GET['vmod'];
$vfon=$_GET['vfon'];
...
echo "    <td>";
echo "     <div align='center'>";
echo "     <font class='titulo1' size='1'><b>Solicitud(es) de B&uacute;squeda(s) Fonetica</b></font><br />";
echo "     </div>";
echo "    </td>";
echo "  </tr>";
echo "</table>";
   ...
   if ($filas_resultado==0){
      ...
      echo "</h5>";
       ...
       ?>
       <form action="../marcas/m_gbclases.php" name="formclase" enctype="multipart/form-data" method="POST" >
       <?php
       ...
       if ($vtip=="D") {
         ...
         echo "<font face='Arial' color='#800000' size='2'>La imagen a usar debe estar en formato PNG (Portable Network Graphics) de tamaño 4x4cms.</font></p>"; }
  ...
  procesamiento de datos y ordenadores; extintores.' class='inputbox' />&nbsp;";
       ...
       echo "<input type='text' name='planilla47' size='6' maxlength='7' onkeyup='number_sindec(this)' disabled>&nbsp;&nbsp;<br /></p>";
...
?>
 ...
 if ($vmod=='Buscar/Eliminar' || $vmod=='Eliminar') {
   ...
   echo "</h5>";
   ?>
   <form action="../marcas/m_gbclases.php" name="formtitular" method="post"> 
   <?php 
   echo "<input type='hidden' name='vfac' value='$vfac'>";
   echo "<input type='hidden' name='vmod' value='$vmod'>";
