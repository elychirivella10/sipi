...
$smarty->assign('subtitulo','Perencion de Procedimiento Recursos/Acciones - Bol. 588 III Parte/Estatus 801');
...
{
  if (empty($boletin)) {
     mensajenew('AVISO: Error en el N&uacute;mero de Bolet&iacute;n o esta vacio ...!!!','m_peren588_801.php','N');
  ...
  if (!$obj_query) { 
    mensajenew('ERROR: Problema al intentar realizar la consulta en la tabla $tbname_2 ...!!!','m_peren588_801.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  $filas_found=$sql->nums('',$obj_query);
  if ($filas_found==0) {
    mensajenew('ERROR: Tabla de Eventos Vacia ...!!!','m_peren588_801.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  ...
  $lcwhere = " WHERE a.tipo_mp='M' AND a.estatus=1801 AND (a.nro_derecho=b.nro_derecho) AND b.evento IN (1195) AND documento='588' AND fecha_trans='16/11/2018' AND fecha_event='12/11/2018' AND a.nro_derecho NOT IN (SELECT nro_derecho FROM stzevtrd WHERE evento=1196) AND a.nro_derecho IN (SELECT nro_derecho FROM stzevtrd WHERE evento=1866 AND documento='1' AND fecha_trans='09/01/2020' AND fecha_event='20/12/2019') ORDER BY a.solicitud";
  ...
  if (!$resultado) { 
    mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_peren588_801.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 

  if ($filas_found==0) {
    mensajenew('ERROR: NO existen DATOS Asociados ...!!!','m_peren588_801.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else
  {
    $numerror  = 0;
    $documento ="";
    $comentario= "RECURSO DE RECONSIDERACION A NEGADA.";
    ...
    for($cont=0;$cont<$filas_found;$cont++) 
    {
      ...
      if (!$obj_query2) { 
        mensajenew('ERROR: Problema al intentar realizar la consulta en la tabla $tbname_3 ...!!!','m_peren588_801.php','N');
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      ...
      if ($filas_197==0) {
        ...
        if (!empty($update_str))
          { $act_dere = $sql->update("$tbname_4","$update_str","nro_derecho='$vder' AND estatus=1801"); 
        }
  ...
  if ($numerror==0) {
    ...
    Mensajenew("'$filas_found' SOLICITUDES ACTUALIZADAS CORRECTAMENTE ...!!!","m_peren588_801.php","S");
    $smarty->display('pie_pag.tpl'); exit();
  }
  else {
    ...
    //Desconexion de la Base de Datos
    $sql->disconnect();
    Mensajenew("ERROR: Falla de Actualizaci&oacute;n / Inserci&oacute;n de Datos en la BD ...!!!","m_peren588_801.php","N");
    $smarty->display('pie_pag.tpl'); exit();
  }
...
$smarty->display('m_peren588_801.tpl');
