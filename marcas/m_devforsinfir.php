...
$smarty->assign('subtitulo','Generaci&oacute;n de Oficios de Devoluci&oacute;n x Bolet&iacute;n sin Firma Digital');
...
if ($vopc==2) {
 ...
 if ((empty($boletin)) && (empty($vsol1)) && (empty($vsol2)) && (empty($vsol3)) && (empty($vsol4))) {
   mensajenew('ERROR: Debe introducir un Criterio de B&uacute;squeda ...!!!','m_devforsinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 }

 if ($boletin < 602) {
   mensajenew('ERROR: El Boletin a generar y firmar NO puede ser menor al 602 ...!!!','m_devforsinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 }

 if ((!empty($vsol1)) && (!empty($vsol2)) && ((empty($vsol3)) || (empty($vsol4)))) {
   mensajenew('ERROR: Debe introducir el rango final de Solicitud para proseguir ...!!!','m_devforsinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 } 
 ...
 if ((!empty($vsol1)) && (!empty($vsol2)) && (!empty($vsol3)) && (!empty($vsol4))) {
   if (empty($boletin)) {
     ...
     if ($vsoli > $vsolf) {  
       $smarty->display('encabezado1.tpl');
       mensajenew('ERROR: Rango de solicitudes erroneo ...!!!','m_devforsinfir.php?vopc=1','N');    
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
*/
   ...
   */
     ...
     if (!$resultado) { 
       mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_devforsinfir.php?vopc=1','N');   
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
$smarty->display('m_devforsinfir.tpl');
$smarty->display('pie_pag.tpl');
}
$sql->disconnect();
?>
