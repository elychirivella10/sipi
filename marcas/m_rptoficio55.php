...
$smarty->assign('subtitulo','Impresi&oacute;n de Oficios de Devoluci&oacute;n Forma Version Vieja - Ley 55 ');
...
if(!empty($vsold) and !empty($vsolh) and ($vsold!='0000-000000') and ($vsolh!='0000-000000')) { 
  ...
  if ($tipoest=="D") { 
   ...
   $resultado=pg_exec("SELECT b.solicitud, b.tramitante, b.nombre, b.agente,b.nro_derecho,b.registro,b.estatus, b.poder, a.clase, a.modalidad    			FROM stmmarce a, stzderec b, stzevtrd c 
		...
		AND tipo_mp='M' 
                AND c.fecha_trans <= '30/10/2020'
   		AND b.estatus in (1200,1113,1115,1116,1118,1300,1025,1600,1002,1004,1005,1006,1008)
   		AND c.evento IN (1500) ".$queryusuario." ORDER BY b.solicitud");
 }
}
...
//   if ($nagen==0) {$nagen='';}
   ...
   $nameimage = "../graficos/marcas/ef".$varsol1."/".$varsol1.$varsol2.".jpg";
    ...
    //$pdf->MultiCell(0,4,'MINISTERIO DEL PODER POPULAR DE INDUSTRIAS Y PRODUCCION NACIONAL',0,1,'C');
    //$pdf->MultiCell(0,4,'SERVICIO AUTONOMO DE LA PROPIEDAD INTELECTUAL',0,1,'C');
    //$pdf->MultiCell(90,7,utf8_decode('OFICIO DE DEVOLUCION A SOLICITUD N°: '),0,1,'C');
    //$pdf->SetFont('Arial','B',10);
    //$pdf->Cell(50,8,$reg['solicitud'],0,0);
    //$pdf->SetFont('Arial','',10);
    ...
    $y = $pdf->Gety();
 	 $pdf->SetXY(10,$y+14);
    //$pdf->ln(2);
    //$pdf->Cell(90,9,'CIUDADANO(A): ',0,0);
    //$pdf->Cell(59,9,utf8_decode('OFICIO DE DEVOLUCION A SOLICITUD N°: '),0,0,'R');
    //$pdf->Cell(209,9,$reg['solicitud'],0,0);
    //$tram = agente_tramp($nagen,trim($reg['tramitante']),$n_poder);
    ////$tram=$nagen.$reg['tramitante'].$n_poder;
...
//    $pdf->ln(14);
//    $pdf->MultiCell(198,4,'   El Funcionario                                                                  Retirado por: ',0,'J',0);
