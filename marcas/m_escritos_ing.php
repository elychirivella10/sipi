...
$vuser=$usuario;  
   ...
   $smarty ->assign('varfocus','formarcas1.vesc1'); 
   $smarty ->assign('vmodo','');
   ...
   if ($vopc==1) {
      if ($vesc=='' || $vesc==0) { 
         ...
         mensajenew('ERROR! Introduzca el N&uacute;mero del Escrito que desea ingresar...','m_escritos_ing.php','N');
   ...
   if ($vopc==2) {
      $resultado=pg_exec("SELECT * FROM stzderec,stmmarce WHERE stzderec.nro_derecho=stmmarce.nro_derecho and solicitud='$vsol' and tipo_mp='M'");
   }

   if ($vopc==3) {
      $resultado=pg_exec("SELECT * FROM stzderec,stmmarce WHERE stzderec.nro_derecho=stmmarce.nro_derecho and registro='$vreg' and registro!='' and tipo_mp='M'");
   }
   ...
   if ($vopc==1) { 
      ...
      if ($res_filas>0) { 
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR! El Escrito ya existe...','m_escritos_ing.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
   ...
   if ($vopc==2 || $vopc==3)  {   
      ...
      if ($filas_found==0 && $vopc<>4) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('No Existen Datos Asociados... Verifique!','m_escritos_ing.php?vopc=1&vesc='.$vesc,'N');
         $smarty->display('pie_pag.tpl'); exit(); 
      }	 
   ...
   if ($vopc==4)  {   
      ...
      $smarty ->assign('vmodo2',''); 
      $smarty ->assign('vmodo3','readonly'); 
      $smarty ->assign('vmodo3',''); $smarty ->assign('varfocus','formarcas5.vnom');
   ...
   if ($vopc==5) {
	 ...
	 $smarty->display('pie_pag.tpl'); exit(); 
      ...
      $can_error=0;
      $resule=pg_exec("select * from stzescri where nro_escrito='$vesc' and tipo_mp='M'");
       $rege = pg_fetch_array($resule);
       $cantfil = pg_numrows($resule);
       if ($cantfil>0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR - El Escrito ya fue cargado anteriormente.','m_escritos_ing.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }    
      pg_exec("BEGIN WORK");           
      $insert_campos="nro_escrito,fecha,solicitud,registro,nombre,ind_claseni,clase,tipo_escrito,nro_boletin,comentario,
                     codigo_agente,tramitante,tipo_mp,motivo,otro_motivo,usuario,fecha_trans,hora,anno";
      $insert_valores ="'$vesc','$vfecesc','$vsol','$vreg','$vnom','$vindcla','$vcla','$vtipesc','$vdoc','$vcom',
                        '$vcodagen','$vtra','M','$vmotivos','$vomot','$vuser','$vfecha','$hh','$vanno'";
      $valido=$sql->insert("stzescri","$insert_campos","$insert_valores","");
      ...
      if ($can_error==0) {
           ...
           $smarty->display('encabezado1.tpl');
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_escritos_ing.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
   ...
   $smarty->display('encabezado1.tpl');
   $smarty->display('m_escritos_ing.tpl'); 
