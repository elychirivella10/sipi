...
if ($vopc==2) {
  ...
  //if (empty($fecharec) || empty($prioridad) || empty($recibo) || empty($solicitant) || empty($vsol1) || empty($indole)) {
  if (empty($clase) || empty($planilla) || empty($vsede)) {
     if ($accion==1) {
  ...
  // Modificar Solicitud
    ...
    $envio = trim($_POST['vplus']);
    if($envio=='V') { 
      mensajenew('ERROR: Debe seleccionar si desea enviar por Correo los Resultados ...!!!','m_bexfigu.php?vopc=4','S');
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
    }
    if($envio=='N') { $email = ''; }
    if($envio=='S') { 
      if(empty($email)) {
        mensajenew('ERROR: Debe indicar el correo donde se le enviar&aacute; los resultados de la b&uacute;squeda al usuario ...!!!','m_bexfigu.php?vopc=4','S');
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }    
    }
    ...
    //if ($envio=="N") {
    //  $update_str = "clase=$clase,sede='$vsede'"; }
    //else {  
      $update_str = "clase=$clase,sede='$vsede',email='$email',envio='$envio'"; 
    //}
    //echo "cambio=$update_str, $planilla1, $planilla  ";
...
if (($vopc!=1) && ($vopc!=2) && ($vopc!=3) && ($vopc!=4)) {
  ...
  $smarty->assign('modo5','disabled');  
  $smarty->assign('modo6','disabled');
...
$smarty->assign('campo2','Fecha Factura:');
...
$smarty->assign('campo11','N&uacute;mero Control de Planilla Blanca:');
$smarty->assign('campo12','Clase:');
$smarty->assign('campo13','Sede:');
