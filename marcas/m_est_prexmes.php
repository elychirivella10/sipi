...
else {$pquery3=" and estatus-1000='".$vesta."'"; $ptit=$ptit.' EN ESTATUS:'.$vesta;}
...
if ($vindo=='T' or $vindo=='T ' or $vindo==' T' or $vindo=='') { $pquery6=''; }
else {$pquery6=" and (position(indole in upper('".$vindo."')) > 0)"; 
      $ptit=$ptit.' INDOLE:'.$vindo;}

// Query 
if ($vano1=='' and $vano2=='') {
    $pquery0='';
} else {
    $pquery0=" and EXTRACT(YEAR FROM fecha_solic) between '$vano1' and '$vano2'";}

   $resultadosum=pg_exec("select count(*) from stzderec a, stmmarce b
   where tipo_mp='M' and a.nro_derecho=b.nro_derecho ".$pquery0.$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.
  $pquery5.$pquery6);
...
if ($vano1==$vano2 and $vano1!='') {
   $resultado=pg_exec("select EXTRACT(MONTH FROM fecha_solic) as mes, count(*) as cant 
            from stzderec a, stmmarce b
           where tipo_mp='M' and a.nro_derecho=b.nro_derecho ".$pquery0.$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.$pquery5.$pquery6.
  ...
  for($cont=$vano1;$cont<=$vano2;$cont++)   {
   $resultado=pg_exec("select count(*) as cant from stzderec a, stmmarce b
           where EXTRACT(YEAR FROM fecha_solic)= '$cont' and 
                 tipo_mp='M' and a.nro_derecho=b.nro_derecho ".$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.$pquery5.$pquery6);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0]; 
   if ($cont==$vano1) {
...
// Setup the titles
if ($vano1==$vano2 and $vano1!='') {$graph->title->Set("MARCAS PRESENTADAS EN EL ANO ".$vano1.$ptit."\n \n ");}
if ($vano1==$vano2 and $vano1=='') {$graph->title->Set("MARCAS PRESENTADAS ".$ptit."\n \n ");}
if ($vano1<>$vano2) {$graph->title->Set("MARCAS PRESENTADAS POR ANO ".$ptit."\n \n ");}
...
<tr><td class="cnt"><a href="m_est_prexmes.php?vopc=0"><img src="../imagenes/salir_f2.png" border="0"></a>  Salir  </td></tr></table><?php
...
$dol_val=array('T','C','G','N','O','P');
$dol_des=array('--- TODAS LAS INDOLES','Colectiva','Gubernamental','Persona Natural','Cooperativa','Privada');
// Vector Paises
...
$smarty->display('m_est_prexmes.tpl');
$smarty->display('pie_pag.tpl');
}
