...
if ($vopc==2) 
{
  ...
  if (!$resultado) { 
    mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_reversarnegbol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado);
  //echo "$condicion$lcwhere - $filas_found"; //exit(); 
  if ($filas_found==0) {
    mensajenew('AVISO: NO existen DATOS Asociados ...!!!','m_reversarnegbol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else 
  { 
    $numerror = 0;
    ...
    { 
      ...
      $del_datos = $sql->del("$tbname_2","stzevtrd.nro_derecho='$vder' AND stzevtrd.evento in (1062) AND stzevtrd.estat_ant IN (1102) AND stzevtrd.usuario='ehernandez' AND stzevtrd.fecha_trans='22/08/2019' AND documento=596");
      //echo " $vder , $regsol[solicitud] "; exit();
      $regestfin = 1102;
      $update_str = "estatus='$regestfin'";
      //Se actualiza Maestra Principal de Derecho 
      if (!empty($update_str))
        { $act_dere = $sql->update("$tbname_1","$update_str","nro_derecho='$vder' AND estatus=1500 AND tipo_mp='M'"); } 
  ...
  if ($numerror==0) {
    ...
    Mensajenew("'$filas_found' REGISTROS ACTUALIZADOS CORRECTAMENTE ...!!!","m_reversarnegbol.php","S");
