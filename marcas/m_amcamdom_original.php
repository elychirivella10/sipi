...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) { 
         $sql->disconnect();
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amcamdom_original.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amcamdom_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect();
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amcamdom_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='') {
        $sql->disconnect();
        $smarty->display('encabezado1.tpl');
        mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS ...!!!','m_amcamdom_original.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
	 }
      ...
      if ($cantfil>0) {
         $sql->disconnect();
         mensajenew('ERROR - El Evento ya fue Cargado ...!!!','m_amcamdom_original.php','N');
         $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      // Verificacion del Cambio de domicilio a alguno de los titulares
       ...
       if ($vcomen=='') {
         ...
         $sql->disconnect();
         mensajenew('ERROR: No realizo ningun Cambio de Domicilio ...!!!','m_amcamdom_original.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
       } 
      ...
      if ($cant_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_amcamdom_original.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           mensajenew("ERROR: Falla de Ingreso en la B.D. Transacciones Abortadas...!!!","m_amcamdom_original.php","N");
           $smarty->display('pie_pag.tpl'); exit();   } 
   }
   ...
   $smarty->display('m_amcamdom_original.tpl'); 
