...
$vuser = $usuario;
   ...
   if (($login=='rmendoza') || ($login=='ngonzalez')) { }	
     ...
     Mensajenew("ERROR: Usuario NO tiene Permiso para esta pantalla, hay un nuevo formato ...!!!","../index1.php","N");
   ...
   $manphp = vman_php("m_devoluci56.php");
   if ($manphp==1) {
     $sql->disconnect(); 
     //$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
         $sql->disconnect(); 
         //$smarty->display('encabezado1.tpl');
         mensajenew('ERROR AL PROCESAR LA BUSQUEDA','m_devoluci56.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($filas_found==0) {
         $sql->disconnect();
         //$smarty->display('encabezado1.tpl');
         mensajenew('NO EXISTEN DATOS ASOCIADOS','m_devoluci56.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
	 ...
	 $smarty ->assign('lcausadev','Causales de Devoluci&oacute;n - Examen de Forma:'); 
         ...
         $sql->disconnect();
         //$smarty->display('encabezado1.tpl');
         mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 1','m_devoluci56.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
   }
    ...
    if($vcausa25=='on'){$vc25='X';}
	 ...
	 $smarty->display('pie_pag.tpl'); exit();  
      ...
      if ($vest-1000==1) { //Esta bien
         ...
         $sql->disconnect(); 
         //$smarty->display('encabezado1.tpl');
         mensajenew($vest.'ERROR AL INTENTAR GRABAR - La solicitud ha sido modificada por otro usuario','m_devoluci56.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($inscaus==0) { 
        ...
        //$smarty->display('encabezado1.tpl');
        //mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','m_devoluci56.php','S');
        Mensaje2("DATOS GUARDADOS CORRECTAMENTE ...!!!","m_devoluci56.php","m_rptdevfor.php?vsol=$vsold&vusr=$usuario");
        $smarty->display('pie_pag.tpl'); exit();   
      }
      else {
   ...
   //$smarty->display('encabezado1.tpl');
   $smarty ->display('m_devoluci56.tpl'); 
