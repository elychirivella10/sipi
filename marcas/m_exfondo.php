...
$smarty->assign('subtitulo','Examen de Fondo a Expediente'); 
...
if ($vopc==1) {
  ...
  if (!$resultado) { 
      mensaje('ERROR AL PROCESAR LA BUSQUEDA ...!!!','m_exfondo.php');
	   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }	 
  $filas_found=pg_numrows($resultado); 
  if ($filas_found==0) {
      mensaje('NO EXISTEN DATOS ASOCIADOS ...!!!','m_exfondo.php');
	   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }	 
  ...
  if (($estatus!=1008) && ($estatus!=1104) && ($estatus!=1027) && ($estatus!=1029) && ($estatus!=1305)) {
    mensaje('Solo se pueden examinar Expedientes en Estatus 8, 27, 29, 104 o 305...!!!','m_exfondo.php');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }
  ...
  // Obtencion del Nombre Agente
  $nagente='';
  $obj_query = $sql->query("SELECT * FROM $tbname_2 WHERE agente ='$vcodage'");
  $obj_filas = $sql->nums('',$obj_query);
  if ($obj_filas!=0) {
    $objs = $sql->objects('',$obj_query);
    $nagente = trim($objs->nombre); }

  // Obtencion del Nombre Pais
  $npais='';
  $pais_resid = pais($vcodpais);

  // Obtencion de Titulares
  $resultit=pg_exec("SELECT nro_derecho,titular,nacionalidad,domicilio FROM stzottid WHERE stzottid.nro_derecho='$vder'");
  $filastitu=pg_numrows($resultit);
  $reg = pg_fetch_array($resultit);
  for($cont=0;$cont<$filastitu;$cont++) 
    {
      $resul2=pg_exec("SELECT titular,nombre FROM stzsolic WHERE titular='$reg[titular]'");    
      $reg2 = pg_fetch_array($resul2);
      $arraytit1[$cont]=$reg2[titular];
      $arraytit2[$cont]=trim($reg2[nombre]);
      $res_pais=pg_exec("SELECT * FROM stzpaisr WHERE pais='$reg[nacionalidad]' and pais!=''");
      $respai = pg_fetch_array($res_pais);
      $arraytit3[$cont]=$reg[nacionalidad]." - ".trim($respai[nombre]);
      $arraytit4[$cont]=$reg[domicilio];
      $reg = pg_fetch_array($resultit);
    }
  $smarty->assign('arr_tit1',$arraytit1); 
  $smarty->assign('arr_tit2',$arraytit2); 
  $smarty->assign('arr_tit3',$arraytit3); 
  $smarty->assign('arr_tit4',$arraytit4); 
  $smarty->assign('vnumtitu',$filastitu);
  ...
  switch ($opcion) {
    case "Conceder":
      ...
      if (($estatus==1008) || ($estatus==1104) || ($estatus==1305)) { 
         ...
         mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 8, 104, 305 ...!!!','m_exfondo.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
      if ($fevento=='') {
	 ...
	 $smarty->display('pie_pag.tpl'); exit();  
      ...
      if (($vest==1008) || ($vest==1104) || ($vest==1305)) { //Esta bien
         ...
         Mensajenew('ERROR: La solicitud ha sido modificada por otro usuario','m_exfondo.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
      ...
      if ($actprim and $instram) {
        ...
        Mensajenew("DATOS GUARDADOS CORRECTAMENTE...!","m_exfondo.php?nconex=$n_conex&salir=1&conx=0",'S');
        $smarty->display('pie_pag.tpl'); exit();
      }
      else {
    ...
    case "Devolver":
      ...
      $allcausas = $vc1.$vc2.$vc3.$vc4.$vc5.$vc6.$vc7.$vc8.$vc9.$vc10.$vc11.$vc12.$vc13.$vc14.$vc15.$vc16.$vc17;
      ...
      $regsol = pg_fetch_array($resulsol);
      $vest   = $regsol[estatus];
      if ($vest==1103 || $vest==1123 || $vest==1008) { //Esta bien 
      } else {
         mensajenew('ERROR: La solicitud ha sido modificada por otro usuario ...!!!','m_exfondo.php','N');
	      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
      ...
      if ($vcausa1 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',1,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa2 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',2,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa3 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',3,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa4 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',4,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa5 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',5,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa6 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',6,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa7 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',7,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa8 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',8,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa9 =='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',9,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa10=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',10,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa11=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',11,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa12=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',12,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa13=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',13,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa14=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',14,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa15=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',15,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa16=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',16,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }
      if ($vcausa17=='on') {
        $ins_de=$sql->insert("stzcaded","$col_campos","'$vder',17,'M','M','M','$fechahoy'","");
        if (!$ins_de) { $inscaus = $inscaus + 1; } }

      $votro = trim($votro);
      if ($votro<>'') {
        $col_campos = "nro_derecho,otros,derecho,grupo,tipo_dev,fecha_dev";
        $ins_otros = $sql->insert("stzotrde","$col_campos","'$vder','$votro','M','M','M','$fechahoy'",""); 
      ...
      //Carga del Evento 53
      $insert_campos="nro_derecho,evento,fecha_event,secuencial,estat_ant,documento,fecha_trans,usuario,desc_evento,hora";
      $insert_valores ="'$vder',1053,'$fevento1',nextval('stzevtrd_secuencial_seq'),'$vest',0,'$fechahoy','$usuario','$vdese','$horactual'";
      $instra1 = $sql->insert("stzevtrd","$insert_campos","$insert_valores","");	 
      //Carga del Evento 500
      $insert_campos="nro_derecho,evento,fecha_event,secuencial,estat_ant,documento,fecha_trans,usuario,desc_evento,hora";
      $insert_valores ="'$vder',1500,'$fevento1',nextval('stzevtrd_secuencial_seq'),1103,0,'$fechahoy','$usuario','$vdese','$horactual'";
      $instra2 = $sql->insert("stzevtrd","$insert_campos","$insert_valores","");	 

      // Verificacion y actualizacion real de los Datos en BD 
      if ($actprim AND $instra1 AND $instra2 AND $inscaus==0) {
        pg_exec("COMMIT WORK");
        //Desconexion de la Base de Datos
        $sql->disconnect();
        ...
        if (!$actprim)   { $error_pri  = " - Maestra Principal"; } 
        if (!$instra1)   { $error_tra  = " - Tr&aacute;mite "; }
        if (!$instra2)   { $error_tra  = " - Tr&aacute;mite "; }
        if ($inscaus!=0) { $error_cau  = " - Causales de Devoluci&oacute;n "; }
        Mensajenew("Falla de Actualizaci&oacute;n / Inserci&oacute;n de Datos en la BD $error_pri $error_tra $error_cau ...!!!","javascript:history.back();","N");
        $smarty->display('pie_pag.tpl'); exit();
      }
      break;
    case "Negar":
      $evento = "1225";
      $estatus= $estatus + 1000;
      if ($estatus==1008 || $estatus==1027 || $estatus==1029 || $estatus==1104 || $estatus==1129 || $estatus==1305) {  
        //Estatus correctos
      } 
      ELSE {
        Mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 8, 27, 29, 104, 129 o 305...!!!','m_exfondo.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      if ($vart=='') {
        Mensajenew('ERROR: DATOS INCORRECTOS o VACIOS ...!!!','m_exfondo.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      ...
      ($vreg5!='' && ($vreg5==$vreg6))) {
        Mensajenew('ERROR: Existen Registros Duplicados ...!!!','m_exfondo.php','N');
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      $noexiste=0;
      if ($vreg1!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg1' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($vreg2!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg2' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($vreg3!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg3' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($vreg4!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg4' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($vreg5!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg5' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($vreg6!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg6' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($noexiste==1) {
        Mensajenew('ERROR: Algún Registro no fué Encontrado ...!!!','m_exfondo.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      ...
      if (($vart=='135' && strpos('abcdefghijklmnop',$vlit1)===false) || ($vart=='136' && strpos('abcdefgh',$vlit1)===false) || ($vart=='194' && strpos('abcd',$vlit1)===false) || ($vlit1!='' && !strpos('137177178192',$vart)===false)) {
        $smarty->display('encabezado.tpl');
        mensajenew('ERROR: Articulo y Literal no se corresponden ...!!!','m_exfondo.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
         }
      if ($vlit2!='' && (($vart=='135' && strpos('abcdefghijklmnop',$vlit2)===false) || ($vart=='136' && strpos('abcdefgh',$vlit2)===false) || ($vart=='194' && strpos('abcd',$vlit2)===false) || (!strpos('137177178192',$vart)===false))) {
        $smarty->display('encabezado.tpl');
        mensajenew('ERROR: Articulo y Literal no se corresponden','m_exfondo.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	  
      ...
      $insert_valores ="'$vder',1225,'$fevento2',nextval('stzevtrd_secuencial_seq'),'$vest',0,'$fechahoy','$usuario','$vdese','$comenta1','$horactual'";
      $instram = $sql->insert("stzevtrd","$insert_campos","$insert_valores","");
      ...
      $insert_campos="nro_derecho,articulo,literal,reg_base"; $insert_valores="'$vder','$vart','$vlit1','$vreg1'";
      $insnega = $sql->insert("stmliaor","$insert_campos","$insert_valores","");
      if (!$insnega) { $negadas = $negadas + 1; }
      if ($vreg2!='') {
        $insert_campos="nro_derecho,articulo,literal,reg_base"; $insert_valores="'$vder','$vart','$vlit1','$vreg2'";
        $insnega = $sql->insert("stmliaor","$insert_campos","$insert_valores",""); 
        if (!$insnega) { $negadas = $negadas + 1; } }      
      if ($vreg3!='') {
        $insert_campos="nro_derecho,articulo,literal,reg_base"; $insert_valores="'$vder','$vart','$vlit1','$vreg3'";
        $insnega = $sql->insert("stmliaor","$insert_campos","$insert_valores",""); 
        if (!$insnega) { $negadas = $negadas + 1; } }
      if ($vlit2!='' && $vreg4=='') {
        $insert_campos="nro_derecho,articulo,literal,reg_base"; $insert_valores="'$vder','$vart','$vlit2','$vreg4'";
        $insnega = $sql->insert("stmliaor","$insert_campos","$insert_valores",""); 
        if (!$insnega) { $negadas = $negadas + 1; } }
      ...
      if ($vreg4!='') {
        ...
        if (!$insnega) { $negadas = $negadas + 1; } }
      if ($vreg5!='') {
        $insert_campos="nro_derecho,articulo,literal,reg_base"; $insert_valores="'$vder','$vart','$vlit2','$vreg5'";
        $insnega = $sql->insert("stmliaor","$insert_campos","$insert_valores",""); 
        if (!$insnega) { $negadas = $negadas + 1; } }
      if ($vreg6!='') {
        $insert_campos="nro_derecho,articulo,literal,reg_base"; $insert_valores="'$vder','$vart','$vlit2','$vreg6'";
        $insnega = $sql->insert("stmliaor","$insert_campos","$insert_valores",""); 
        if (!$insnega) { $negadas = $negadas + 1; } }
      ...
      // Verificacion y actualizacion real de los Datos en BD 
      if ($actprim AND $instram AND $negadas==0) {
        pg_exec("COMMIT WORK");
        //Desconexion de la Base de Datos
        $sql->disconnect();
        ...
        if (!$actprim)   { $error_pri  = " - Maestra Principal"; } 
        if (!$instram)   { $error_tra  = " - Tr&aacute;mite "; }
        if ($negadas!=0) { $error_neg  = " - Literales "; }
        Mensajenew("Falla de Actualizaci&oacute;n / Inserci&oacute;n de Datos en la BD $error_pri $error_tra $error_neg ...!!!","javascript:history.back();","N");
        $smarty->display('pie_pag.tpl'); exit();
      }
      break;
    case "Detener":
      ...
      if ($estatus==1008  || $estatus==1027 || $estatus==1129 || $estatus==1104 || $estatus==1305) { 
         ...
         mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 8, 27, 104, 129 o 305...!!!','m_exfondo.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
