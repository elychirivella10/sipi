...
$smarty->assign('subtitulo','Actualizacion de Datos en Lotes del Boletin Rezagado'); 
  ...
  if (($usuario!='rmendoza') AND ($usuario!='ngonzalez')) {
   ...
   if ($vopc==1) {
      ...
      if ($can_error==0) {
           ...
           //$smarty->display('encabezado1.tpl');
           mensajenew("AVISO: Se Actualizaron '$cantreg' Solicitudes",'m_bolactloe.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else { 
   ...
   //$smarty->display('encabezado1.tpl');
   $smarty ->display('m_bolactloe.tpl'); 
