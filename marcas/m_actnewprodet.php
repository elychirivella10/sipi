...
$smarty->assign('subtitulo','Actualizaci&oacute;n a Detenidas por Nuevo Procedimiento de Forma/Fondo por Bolet&iacute;n');
...
if ($vopc==2) 
{
  ...
  if (!$obj_query) { 
    mensajenew('Problema al intentar realizar la consulta en la tabla $tbname_2 ...!!!','m_actdetnewpro.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  $filas_found=$sql->nums('',$obj_query);
  if ($filas_found==0) {
    mensajenew('Tabla de Eventos Vacia ...!!!','m_actdetnewpro.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
...
//                   AND stzderec.nro_derecho in (SELECT nro_derecho FROM stmforfon WHERE evento=1054)";
  ...
  if (!$resultado) { 
    mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_actdetnewpro.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 
  //echo "son $filas_found "; exit();
  if ($filas_found==0) {
    mensajenew('ERROR: NO existen DATOS Asociados ...!!!','m_actdetnewpro.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else 
  { 
    $numerror = 0;
    ...
    for($cont=0;$cont<$filas_found;$cont++) 
    {
      ...
      $obj_query = $sql->query("SELECT $tbname_5.solicitud,$tbname_5.usuario,$tbname_6.comentario FROM $tbname_5,$tbname_6 
                                 WHERE $tbname_5.nro_derecho='$vder' AND $tbname_5.evento='1054' 
                                  AND  $tbname_5.nro_derecho=$tbname_6.nro_derecho");
      $objs = $sql->objects('',$obj_query);
      $usuario_examinador=$objs->usuario;
      $comentadet = $objs->comentario;
      ...
      $insert_str = "'$vder','$evento','$fechahoy',nextval('stzevtrd_secuencial_seq'),'$estatus',0,'$fechahoy','$usuario_examinador','$mensa_automatico','$comentadet','$horactual'";
      ...
      $update_str1 = "estado='D'";
      $act_forfon = $sql->update("$tbname_5","$update_str1","nro_derecho='$vder' AND evento='1054'");
      $act_dtforfon = $sql->update("$tbname_6","$update_str1","nro_derecho='$vder' AND evento='1054'");

      if ($ins_tram AND $act_dere AND $act_forfon AND $act_dtforfon) { }
  ...
  if ($numerror==0) {
    ...
    Mensajenew("'$filas_found' SOLICITUDES ACTUALIZADAS CORRECTAMENTE ...!!!","m_actdetnewpro.php","S");
