...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) {
         $sql->disconnect(); 
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect(); 
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='' || $vtipo=='') {
         $sql->disconnect();
         mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS...','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	 }
	 
	   if (empty($vfactura)) { 
         $sql->disconnect();
         mensajenew('ERROR: DEBE COLOCAR EL NUMERO DE LA FACTURA ...!!!','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	   }

	   if (empty($fechafac)) { 
         $sql->disconnect();
         mensajenew('ERROR: DEBE COLOCAR LA FECHA DE LA FACTURA ...!!!','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	   }
      ...
      if ($esmayor==1) {
         $sql->disconnect();
         mensajenew('ERROR: No se puede cargar un evento previo a la Fecha de la Solicitud','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($numtmp==0) {
         $sql->disconnect();
         mensajenew('ERROR: No existe(n) Titular(es) Final(es). Transaccion Abortada...','m_amcesionv.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
       
      if ($vtipo==1) {$vcodeven=1205; $tipoanota = "C"; $vdeseven='SOLICITUD DE CESION';}
      if ($vtipo==2) {$vcodeven=1206; $tipoanota = "F"; $vdeseven='SOLICITUD DE FUSION';}
      if ($vtipo==3) {$vcodeven=1209; $tipoanota = "N"; $vdeseven='SOLICITUD DE CAMBIO DEL TITULAR';}
      ...
      if ($nfac!='FA394227') {
        ...
        $obj_filas = $sql->nums('',$obj_query);
        if ($obj_filas!=0) {
          mensajenew('ERROR: Factura '.$nfactura.' YA usado anteriormente en otro Servicio ...!!!','m_amcesionv.php','N');
          $smarty->display('pie_pag.tpl'); exit(); }
      }
      ...
      if ($cantfil>0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR - El Evento ya fue Cargado','m_amcesionv.php','N');
         $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      {  $temptit=$regco[titular];
         ...
         if ($tempdom=='' or $tempnac=='') {
            $sql->disconnect();
            $smarty->display('encabezado1.tpl');
            mensajenew('ERROR: Falto ingresar Nacionalidad o Domicilio del Titular Final...','m_amcesionv.php','N');
            $smarty->display('pie_pag.tpl'); exit(); 
         }
      ...
      if ($cant_error==0) {
           ...
           $smarty->display('encabezado1.tpl');
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_amcesionv.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           $smarty->display('encabezado1.tpl');
           mensajenew("Falla de Ingreso en la B.D. Transacciones Abortadas...!!!","m_amcesionv.php","N");
           $smarty->display('pie_pag.tpl'); exit();   } 
   }
   ...
   //$smarty->display('encabezado1.tpl');
   //$smarty ->assign('modo1','readonly'); 
   $smarty->display('m_amcesionv.tpl'); 
