...
if ($vopc==1) {
  ...
  $estatus = $reg[estatus];
  if ($estatus!=1026) {
    mensajenew("Solo se pueden modificar Marcas en Estatus 26 ...!","m_mdsolerr.php?vopc=4&vaccion=2&vauxnum=0","N");
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }
  ...
  $valores_fields = array($nombre,$fecha_solic,$tipo_marca,$vclase,$clase_id,$modalidad,$vcodpais); 
  $campos = "nombre|fecha_solic|tipo_derecho|clase|ind_claseni|modalidad|pais_resid";
...
if (($vopc==2) || ($vopc==6)) {
  ...
  if ($accion==1) {
    ...
    if ($numtitu==0 AND $numccv==0 AND $inslema AND $inslogo AND 
      ...
      Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","m_mdsolerr.php?vopc=4&nconexion=".$nconexion,"S");
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
