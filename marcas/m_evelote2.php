...
{
  $indcond = 0;
  $vsola=sprintf("%04d",$vsol1)."-".sprintf("%06d",$vsol2);
  $vsolb=sprintf("%04d",$vsol3)."-".sprintf("%06d",$vsol4);
  ...
  $lcwhere = " WHERE (stzderec.solicitud>='$vsola' and stzderec.solicitud<='$vsolb') and stzderec.tipo_mp='M' ";

  if ((!empty($est_id1)) and ($est_id1!=0)) {
     $est_id1 = $est_id1+1000;
  ...
  if ($vsola=='0000-000000' and $vsolb=='0000-000000') {
     mensajenew('ERROR AL INTENTAR PROCESAR - RANGO DE DATOS INCORRECTOS O VACIOS ...!!!','javascript:history.back();','N');
  ...
  if (!$resultado) { 
    mensajenew('ERROR AL PROCESAR LA BUSQUEDA ...!!!','m_evelote.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 
  if ($filas_found==0) {
    mensajenew('NO EXISTEN DATOS ASOCIADOS ...!!!','m_evelote.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else 
  { 
    $errorgrabar = 0; 
    $evento2_id = $evento2_id + 1000;
      ...
      if (!empty($evento2_id) || ($evento2_id!=0)) { 
        ...
        if ($esmayor==0) {
          if ($tipo_evento == "M") {
            ...
            $regestfin=estatus_final($evento2_id,$estatus,"M");
            if (!empty($regestfin)) {
          ...
          //Se obtiene el proximo valor para el secuencial a guardar en stzevtrd a partir de stzsistem
  ...
  if ($errorgrabar == 0) {
    ...
    $sql->disconnect();
    Mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_evelote.php','S');
    $smarty->display('pie_pag.tpl'); exit();
  }
  else {
