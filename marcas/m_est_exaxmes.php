...
$smarty->assign('subtitulo','Estadisticas - Examinadas Fondo/Forma');
...
if ($dif>11) {
    $sql->disconnect(); 
    mensajenew('El rango no puede ser mayor a 11 a&ntilde;os...!!!',
...
if ($vano1=='' and $vano2=='') {
   ...
   $resultadosum=pg_exec("select count(*) from stzderec a, stmmarce b, stzevtrd c
   where tipo_mp='M' and a.nro_derecho=c.nro_derecho and evento in (1051,1052,1053,1054,1071) 
         and a.nro_derecho=b.nro_derecho ".$pquery0.$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.
  $pquery5);
...
if ($vano1==$vano2 and $vano1!='') {
   $resultado=pg_exec("select EXTRACT(MONTH FROM fecha_trans) as mes, count(*) as cant 
           ...
           where tipo_mp='M' and a.nro_derecho=c.nro_derecho and evento in (1051,1052,1053,1054,1071) 
                 and a.nro_derecho=b.nro_derecho ".$pquery0.$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.$pquery5.
  ...
  for($cont=$vano1;$cont<=$vano2;$cont++)   {
   $resultado=pg_exec("select count(*) as cant from stzderec a, stmmarce b, stzevtrd c
           where EXTRACT(YEAR FROM fecha_trans)= '$cont' and 
                 tipo_mp='M' and and a.nro_derecho=c.nro_derecho and evento in (1051,1052,1053,1054,1071) 
                 and a.nro_derecho=b.nro_derecho ".$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.$pquery5);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0]; 
   if ($cont==$vano1) {
      $datay=array($cant_reg);
      $datax=array("ANO ".$vano1); 
      $cant_total=$cant_reg;
      $cant_bol=1;
   } else {
     array_push($datay,$cant_reg); 
     array_push($datax,"ANO ".$cont); 
    ...
    $resultado=pg_exec("select count(*) as cant from stzderec a, stmmarce b, stzevtrd c
           where tipo_mp='M' and and a.nro_derecho=c.nro_derecho and evento in (1051,1052,1053,1054,1071) 
                 and a.nro_derecho=b.nro_derecho ".$pquery0.$pquery1.$pquery11.$pquery2.$pquery3.$pquery4.$pquery5);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0];    
   $datay=array($cant_reg);
...
// Setup the titles
if ($vano1==$vano2 and $vano1!='') {$graph->title->Set("MARCAS EXAMINADAS FONDO/FORMA EN EL ANO ".$vano1.$ptit."\n \n ");}
if ($vano1==$vano2 and $vano1=='') {$graph->title->Set("MARCAS EXAMINADAS FONDO/FORMA ".$ptit."\n \n ");}
if ($vano1<>$vano2) {$graph->title->Set("MARCAS EXAMINADAS FONDO/FORMA POR ANO ".$ptit."\n \n ");}
...
<tr><td class="cnt"><a href="m_est_exaxmes.php?vopc=0"><img src="../imagenes/salir_f2.png" border="0"></a>  Salir  </td></tr></table><?php
...
$smarty->display('m_est_exaxmes.tpl');
$smarty->display('pie_pag.tpl');
}
