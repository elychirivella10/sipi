...
$smarty->assign('subtitulo','Reverso de Actualizacion mal ejecutada de Estatus 6 a 5');
...
if ($vopc==2) 
{
  ...
  $lcwhere = " WHERE tipo_mp='M' AND stzderec.estatus=1006 AND (stzderec.nro_derecho=stzevtrd.nro_derecho) AND stzevtrd.evento in (1030) AND stzevtrd.estat_ant in (1005) AND stzevtrd.usuario='lcrodriguez' AND stzevtrd.fecha_event='13/10/2020' AND stzevtrd.fecha_trans='13/10/2020'";
  $orderby = " ORDER BY stzderec.solicitud";
  ...
  { 
    ...
    { 
      ...
      $del_datos = $sql->del("$tbname_2","stzevtrd.nro_derecho='$vder' AND stzevtrd.evento in (1030) AND stzevtrd.estat_ant IN (1005) AND stzevtrd.usuario='lcrodriguez' AND stzevtrd.fecha_event='13/10/2020' AND stzevtrd.fecha_trans='13/10/2020'");
      //echo " $vder , $regsol[solicitud] "; //exit();
      $regestfin = 1005;
      $update_str = "estatus='$regestfin'";
      //Se actualiza Maestra Principal de Derecho 
      if (!empty($update_str)) { 
        $act_dere = $sql->update("$tbname_1","$update_str","nro_derecho='$vder' AND estatus=1006 AND tipo_mp='M'"); 
      } 
...
$smarty->display('m_reversarev30.tpl');
