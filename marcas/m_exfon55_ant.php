...
if ($vopc==2) {
  ...
  switch ($opcion) {
    ...
    case "Negar":
      ...
      ELSE {
        Mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 8, 27, 29, 104, 129 o 305...!!!','m_exfon55.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      if ($vart=='') {
        Mensajenew('ERROR: DATOS INCORRECTOS o VACIOS ...!!!','m_exfon55.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      ...
      ($vreg5!='' && ($vreg5==$vreg6))) {
        Mensajenew('ERROR: Existen Registros Duplicados ...!!!','m_exfon55.php','N');
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      $noexiste=0;
      ...
      if ($vreg6!='') {$resul=pg_exec("SELECT * FROM stzderec WHERE registro='$vreg6' AND tipo_mp='M'");
                      $filasr=pg_numrows($resul); if ($filasr==0) {$noexiste=1;} }
      if ($noexiste==1) {
        Mensajenew('ERROR: Algún Registro no fué Encontrado ...!!!','m_exfon55.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
      ...
      if (($vart=='135' && strpos('abcdefghijklmnop',$vlit1)===false) || ($vart=='136' && strpos('abcdefgh',$vlit1)===false) || ($vart=='194' && strpos('abcd',$vlit1)===false) || ($vlit1!='' && !strpos('137177178192',$vart)===false)) {
        ...
        mensajenew('ERROR: Articulo y Literal no se corresponden ...!!!','m_exfon55.php','N');
	     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
         }
    ...
    case "Detener":
      ...
      if ($estatus==1008  || $estatus==1027 || $estatus==1129 || $estatus==1104 || $estatus==1305) { 
         ...
         mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 8, 27, 104, 129 o 305...!!!','m_exfon55.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
