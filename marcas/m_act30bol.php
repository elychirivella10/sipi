...
$smarty->assign('subtitulo','Aplicaci&oacute;n de Evento 30 en Lote por Bolet&iacute;n');
...
if ($vopc==2) 
{
  ...
  if (!$obj_query) { 
    mensajenew('Problema al intentar realizar la consulta en la tabla $tbname_2 ...!!!','m_cadubol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  $filas_found=$sql->nums('',$obj_query);
  if ($filas_found==0) {
    mensajenew('Tabla de Eventos Vacia ...!!!','m_ev30bol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  ...
  $lcwhere = " WHERE a.tipo_mp='M' AND a.estatus=1005 AND (a.nro_derecho=b.nro_derecho) AND b.evento IN (1022) AND a.nro_derecho IN (SELECT nro_derecho FROM stzevtrd WHERE evento IN (1201,1089) AND documento='$boletin' AND estat_ant IN (1002,1027)) ";
  ...
  if (!$resultado) { 
    mensajenew('Error al Procesar la Busqueda ...!!!','m_ev30bol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 

  if ($filas_found==0) {
    mensajenew('ERROR: NO existen DATOS Asociados ...!!!','m_ev30bol.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  else
  {
    $numerror = 0;
    ...
    for($cont=0;$cont<$filas_found;$cont++) 
    {
      ...
      if (!empty($update_str))
        { $act_dere = $sql->update("$tbname_4","$update_str","nro_derecho='$vder' AND estatus=1005"); }
  ...
  if ($numerror==0) {
    ...
    Mensajenew("'$filas_found' SOLICITUDES ACTUALIZADAS CORRECTAMENTE ...!!!","m_ev30bol.php","S");
