...
$smarty->assign('subtitulo','Listado de Marcas Devueltas Publicadas No Contestadas');
...
//$resultado=pg_exec("SELECT DISTINCT ON(stzderec.solicitud) stzderec.solicitud, stzderec.fecha_solic, stzderec.nombre, stmmarce.modalidad,stmmarce.clase,stmmarce.ind_claseni,stzderec.tipo_mp,stzsolic.nombre as titular
//						FROM  stmmarce, stzottid, stzsolic, stzderec
//						WHERE $where 
//						AND stmmarce.nro_derecho=stzderec.nro_derecho
//						AND stmmarce.nro_derecho=stzottid.nro_derecho
//						AND stzsolic.titular = stzottid.titular
//                 AND stzderec.nro_derecho in (select distinct nro_derecho from stzevtrd where evento=201 and documento=$boletin)
//						ORDER BY stzderec.solicitud ");	
	 
$resultado=pg_exec("SELECT DISTINCT ON(stzderec.solicitud) stzderec.solicitud,stzderec.fecha_solic,stzderec.nombre,stmmarce.modalidad,stmmarce.clase,stmmarce.ind_claseni,stzderec.tipo_derecho,stzsolic.nombre as titular,stzottid.nacionalidad,stzottid.domicilio 
...
$encabezado= utf8_decode('Listado de Devueltas Publicadas NO Contestadas');
...
//Cargando los datos en la Tabla
 for($cont=0;$cont<$filas_resultado;$cont++) {
   ...
   $nbtitular   = trim($registro['titular']).'.'.trim($registro['domicilio']).','.trim($pais_nombre); 
	...
	$data[6]['TEXT'] = utf8_decode($nbtitular);
	$registro = pg_fetch_array($resultado);
	$pdf->Draw_Data($data);
  }
