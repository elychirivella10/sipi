...
if ($vopc==1) {
    ...
    //pg_exec("CREATE TEMPORARY TABLE $ntabla1 (solicitud char(11)); CREATE INDEX figurativa_sol ON figurativa USING btree (solicitud)");
    pg_exec("CREATE TEMPORARY TABLE $ntabla1 (solicitud char(11))");
    //pg_exec("CREATE TEMPORARY TABLE $ntabla1 (control character(11),solicitud char(11),clase int2,ind_claseni char(1))");
    ...
    $obj_query = $sql->query("SELECT * FROM $tbname_10 WHERE solicitud='$v1'");
    $obj_filas = $sql->nums('',$obj_query);
    $objs = $sql->objects('',$obj_query);
    if ($obj_filas==0) {
     Mensajenew('ERROR: Clasificacion Internacional de Viena Vacia(s) ...!!!','javascript:history.back();','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
    for($i=0;$i<$obj_filas;$i++) { 
      $varsolc = $objs->ccv;
      if ($operador=='OR') { 
      ...
      $vsel = $vsel."SELECT '$v1',solicitud,clase,ind_claseni FROM viena WHERE ccv="."'".$varsolc."'";
      $objs = $sql->objects('',$obj_query); 
    }
    ...
    if (!$obj_query) { 
      mensajenew('ERROR: Problema al intentar realizar la consulta en la tabla $ntabla2 ...!!!','m_pbinfigu.php','N');
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
