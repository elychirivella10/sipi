...
$smarty->assign('subtitulo','Modificaci&oacute;n de Expediente'); 
if ($vopc==3) {
  $smarty->assign('subtitulo','Modificaci&oacute;n de Expediente'); }
if ($vopc==4) {
  $smarty->assign('subtitulo','Modificaci&oacute;n de Expediente'.$nveces); }
...
if ($vopc==1) {
  ...
  //Tabla Principal 
  $valores_fields = array($nombre,$fecha_solic,$tipo_marca,$agen_id,$vpoder,$tramitante,$vcodpais);
  $campos = "nombre|fecha_solic|tipo_derecho|agente|poder|tramitante|pais_resid";
  ...
  //Tabla Secundaria  
  $valores_fields = array($vclase,$clase_id,$modalidad,$distingue);
  $campos1  = "clase|ind_claseni|modalidad|distingue";
  $vstring1 = bitacora_fields();
  $smarty->assign('campos1',$campos1);
  $smarty->assign('vstring1',$vstring1);
  
  //Obtención de la Descripción del Estatus 
  $obj_query = $sql->query("SELECT * FROM $tbname_21 WHERE estatus='$estatus'");
  $objs = $sql->objects('',$obj_query);
  ...
  if ($obj_filas > 0) {  
    ...
    for ($contobj=0;$contobj<$obj_filas;$contobj++) {
      ...
      $col_campos = "solicitud,agente,nombre,domicilio,tipo_mp";
      $obj_ag_query = $sql->query("SELECT * FROM $tbname_18 WHERE solicitud='$varsol' AND agente=$vcod AND tipo_mp='M'");
...
if (($vopc==2) || ($vopc==6)) {
  ...
  if (!empty($vreg1) && !empty($vreg2)) { 
    $registro=$vreg1.sprintf("%06d",$vreg2); } 
  ...
  if ($accion==1) {
    ...
    $ncontrol   = grabar_sys("nbitaco",$sys_actual);
    pg_exec("COMMIT WORK");
    ...
    //Derecho 
    $insert_str = "'$ncontrol','$fechahoy','$horactual','$usuario','M','$vder','$varsol','M','$tbname_6','$campos','$vstring'";
    $insbit1 = $sql->insert("$tbname_12","","$insert_str","");
    //Marcas 
    $insert_str = "'$ncontrol','$fechahoy','$horactual','$usuario','M','$vder','$varsol','M','$tbname_4','$campos1','$vstring1'";
    $insbit2 = $sql->insert("$tbname_12","","$insert_str","");

    if ($modalidad=="D") {
      $sql->del("$tbname_8","nro_derecho='$vder'"); 
      $sql->del("$tbname_17","nro_derecho='$vder'"); }

    echo "pais= $vcodpais,$input2,$pais_resid."; 
    ...
    if (!empty($registro)) {
      $update_str = $update_str.",fecha_regis='$fecha_regis',fecha_venc='$fecha_venc'"; }
    echo "$update_str ";
    $updderecho = $sql->update("$tbname_6","$update_str","nro_derecho='$vder'");
    //echo "pasa 1 gb $updderecho ";    
    ...
    //echo "pasa 2 gb $updmarce ";    

    // Tabla de Batfon 
    $insert_str = "'$varsol'";
    $insfon = $sql->insert("$tbname_22","","$insert_str","");
    ...
    if (($modalidad=="G") || ($modalidad=="M")) {
      ...
      $obj_query = $sql->query("SELECT * FROM stmlogos WHERE nro_derecho =$vder");
      $obj_filas = $sql->nums('',$obj_query);
      $objs = $sql->objects('',$obj_query);
      //$obj_filas = $sql->nums('',$obj_query);
      //echo "son etiquetas= $obj_filas";
      if ($obj_filas==0) {
    ...
    if ($numtitu==0 AND $numccv==0 AND $inslema AND $inslogo AND 
        $numprio==0 AND $numagen==0 AND $updmarce AND $updderecho AND $insbit1 AND $insbit2) { 
      pg_exec("COMMIT WORK");
      //Desconexion de la Base de Datos
      $sql->disconnect();

      Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","m_modifica.php?vopc=4&nconexion=".$nconexion,"S");
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
...
$smarty->assign('vcodpais',$vcodpais);
$smarty->assign('vcodest',($estatus-1000));
$smarty->assign('vestatus',$descripcion);
