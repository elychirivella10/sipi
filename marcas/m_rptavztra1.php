...
if(!empty($desdet) and !empty($hastat)) { 
	if(!empty($where)) {
	   $where = $where." and"." ((stzevtrd.fecha_trans >= '$desdet') and (stzevtrd.fecha_trans <='$hastat'))";
	   $titulo= $titulo." Fecha Trans:"."$desdet"." al: "."$hastat";
	}
	else { 
		$where = $where." ((stzevtrd.fecha_trans >= '$desdet') and (stzevtrd.fecha_trans <='$hastat'))";
      $titulo= $titulo." Fecha Trans:"."$desdet"." al: "."$hastat";
	}
}
...
if(!empty($desde) and !empty($hasta)) { 
	if(!empty($where)) {
	   $where = $where." and"." ((stzevtrd.fecha_event >= '$desde') and (stzevtrd.fecha_event <='$hasta'))";
	   $titulo= $titulo." Fec Evento:"."$desde"." al: "."$hasta";
	}
	else { 
		$where = $where." ((stzevtrd.fecha_event >= '$desde') and (stzevtrd.fecha_event <='$hasta'))";
      $titulo= $titulo." Fec Evento:"."$desde"." al: "."$hasta";
	}
}
if(!empty($evento)) { 
...
$resultado=pg_exec("SELECT stzderec.solicitud, stzderec.nombre,stmmarce.modalidad,stzevtrd.evento,stzevtrd.estat_ant, stzevtrd.fecha_event,stmmarce.clase, stmmarce.ind_claseni,stzevtrd.fecha_trans, stzderec.fecha_venc, stzderec.estatus,stzevtrd.documento
			FROM  stmmarce, stzevtrd, stzderec 
			WHERE $where 
			AND (stzderec.registro ='' OR stzderec.registro is NULL)
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado;$cont++) { 
	...
	$data[8]['TEXT'] = $registro['fecha_trans'];
