...
if(!empty($usuario)) { 
	   $where = $where."( stzevtrd.usuario = '$usuario')"." and ";	}

// Armando el query segun las opciones

$resultado=pg_exec("SELECT stzderec.nro_derecho,stzderec.solicitud, stzderec.nombre, stmmarce.clase, stzderec.registro, stzsolic.nombre as titular, stzderec.agente, trim(tramitante) as tramitante
			FROM  stmmarce, stzottid, stzsolic, stzderec, stzevtrd	
			WHERE $where stzevtrd.evento = '1069' 
			...
			AND stzderec.nro_derecho in (select distinct nro_derecho from stzevtrd where evento=1502)
...
//generacion del TXT 
 ...
 $open = fopen($via.$via1.'reindvforam'.'_'.$fecha.'.txt',"w+") or die ("Error de lectura");
 fputs($open, "$archivo");
 fclose($open);
