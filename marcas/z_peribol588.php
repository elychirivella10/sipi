...
$smarty->assign('subtitulo','Boletin Extraordinario $boletin : Perencion de Recursos y Acciones por NO Ratificacion por Publicar, Cont. Bol. 588');
...
if ($vopc==2) 
{
  ...
  if ($boletin!=1) {
    mensajenew("ERROR: Bolet&iacute;n NO Corresponde a este Extraordinario a Generar ...!!!","z_prebol588.php","N");
    $smarty->display('pie_pag.tpl'); exit();           
  }
	
  $obj_query = $sql->query("SELECT max(nro_boletin) FROM stzboletin WHERE generado='S' AND tipo_boletin='E'");
    ...
    if ($filas_found!=0) {
      ...
      for($cont=0;$cont<$filas_found;$cont++) 
      {
        ...
        if ($evento_found==0) {
          ...
          $insert_valores ="$vder,'$vsol',$boletin,$estatus,'M','$usuario','$fechahoy','$horahoy',0,'$tipbol'";
    ...
    if ($filas_found!=0) {
      ...
      for($cont=0;$cont<$filas_found;$cont++) 
      {
        ...
        if ($evento_found==0) {
          ...
          $insert_campos="nro_derecho,solicitud,boletin,estatus,tipo,usuario,
                          fecha_carga,hora_carga,nanota,tipo_boletin";
          $insert_valores ="$vder,'$vsol',$boletin,$estatus,'P','$usuario','$fechahoy','$horahoy',0,'$tipbol'";
          //No grabar cuando la solicitud exista en el temporal
          $resulfound=pg_exec("SELECT solicitud FROM stztmpbo WHERE solicitud='$vsol' and 
