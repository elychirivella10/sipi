...
$smarty->assign('subtitulo','B&uacute;squeda Gr&aacute;fica / Actualizaci&oacute;n de Estatus para Proceso de Re-Env&iacute;o de Resultados'); 
...
if ($vopc==1) {
  ...
  if (empty($vsol1) && empty($recibo)) {
     mensajenew('ERROR: No introdujo ningún valor de No Factura o Pedido ...!!!','m_actenvgra.php?vopc=4','N');
     $smarty->display('pie_pag3.tpl'); exit(); }

  //Validacion del Numero de Recibo
  if (!empty($recibo)) {
  	 $vfiltro = "1"; 
    $resultado=pg_exec("SELECT * FROM $tbname_1 WHERE recibo='$recibo'");
    if (!$resultado) { 
      mensajenew('ERROR: No se pudo accesar la Base da Datos ...!!!','m_actenvgra.php?vopc=4&vauxnum=0','N');
      $smarty->display('pie_pag3.tpl'); $sql->disconnect(); exit(); }	 
    $filas_found=pg_numrows($resultado); 
    if ($filas_found==0) {
      mensajenew('AVISO: NO EXISTEN DATOS ASOCIADOS ...!!!','m_actenvgra.php?vopc=4&vauxnum=0','N');
...
if ($vopc==2) {
  ...
  // Actualizo en Maestra de Eventos
    ...
    if($vfiltro=="1") {    
      $actbusq = $sql->update("$tbname_1","$update_str","recibo='$recibo'"); }
    else {   
    ...
    if ($actbusq) {
      ...
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE ...!!!','m_actenvgra.php?vopc=4','S');
      $smarty->display('pie_pag3.tpl'); exit();
    }
    else {
...
$smarty->display('m_actenvgra.tpl');
