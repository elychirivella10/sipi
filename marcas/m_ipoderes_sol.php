...
$sql->connection($usuario);
  ...
  if ($vopc==1 and $vaccion=='Incluir') {
   ...
   if ($vopc==1 and $vaccion=='Buscar' and $vsol1!='' and $vsol2!='') {
      ...
      $sql->disconnect();
      $smarty->display('encabezado1.tpl');
      mensajenew('EL PODER que intenta consultar NO EXISTE!!!','m_ipoderes_sol.php?vopc=12','N');
      $smarty->display('pie_pag.tpl'); exit(); 
      } else { 
      // Elimina posibles registros existentes en el temporal
      $sql->del("temptitu","solicitud='$vsol'");
   ...
   if ($vopc==5 and $vaccion=='Incluir') {
      ...
      if ($filas_found>0) {
            $sql->disconnect();
            $smarty->display('encabezado1.tpl');
            mensajenew('Este PODER ya fue cargado por otro Usuario!!!','m_ipoderes_sol.php','N');
            $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($valins==0) { 
        ...
        $smarty->display('encabezado1.tpl');
        mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','m_ipoderes_sol.php?vopc=11','S');
        $smarty->display('pie_pag.tpl'); exit();   
      }
      else {
