...
$smarty->assign('subtitulo','Asignacion de Numero de Registro a Expediente');
...
if ($vopc==1) {
  ...
  if (empty($vsol1) & empty($vsol2))
   {
    mensajenew('ERROR: No introdujo ningún valor de Expediente ...!!!','m_regsolm.php','N');
  ...
  if (!$resultado) { 
    mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_regsolm.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }	 
  $filas_found=pg_numrows($resultado); 
  if ($filas_found==0) {
    mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }	 
  ...
  $vest1     =$reg[estatus];
  if ($vest1!=1410) {
    Mensajenew('ERROR: Evento NO aplica para este Estatus o no esta definido la Migracion ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }
...
if ($vopc==2) {
  ...
  if (($pago=='') || ($pago==0)) { 
    mensajenew('ERROR: El pagp NO pude estar en blanco o ser Cero ...!!!','javascript:history.back();','N');
    $smarty->display('pie_pag.tpl'); exit(); 
  }
  ...
  if (!$obj_query) { 
    Mensajenew('ERROR: Problema al intentar consultar la tabla Maestra $tbname_6 ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  $filas_found=$sql->nums('',$obj_query);
  $objs = $sql->objects('',$obj_query);
  $numbereg = $objs->registro;
  if (!empty($numbereg)) {
    mensajenew('AVISO: Expediente ya le fue asignado Numero de Registro ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  if (empty($vfecvi)) { 
    mensajenew('ERROR: La Fecha de Vigencia del Registro esta vacia ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); exit(); }
  ...
  if ($esmayor==1) {
    ...
    mensajenew('ERROR: Fecha de Registro NO puede ser mayor a la fecha actual ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); exit(); } 
  $esmayor=0;
  $esmayor=compara_fechas($solfecha,$vfecvi);
  if ($esmayor==1) {
    $smarty->display('encabezado.tpl');
    Mensajenew('ERROR: Fecha de Registro NO puede ser menor a la Fecha de la Solicitud ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); exit(); } 
  $esmayor=0;
  ...
  if (!$obj_query) { 
    Mensajenew('ERROR: Problema al intentar consultar la tabla $tbname_4 ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
  $filas_found=$sql->nums('',$obj_query);
  if ($filas_found==0) {
    //Mensajenew('EXPEDIENTE NO TIENE CARGADO EVENTO DE PUBLICACION DE CONCESION ...!!!','m_regsolm.php','N');
    //$smarty->display('pie_pag.tpl'); $sql->disconnect(); exit();
    $esmayor=0;
    $fechahoy = "12/09/2008";
  ...
  if ($fil_found_reg>0) {
    Mensajenew('ERROR: NUMERO DE REGISTRO YA EXISTE EN LA BASE DE DATOS ...!!!','m_regsolm.php','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }	 
  ...
  if ($actprim AND $instram) {
    ...
    Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","m_regsolm.php?nconexion=".$nconexion."&nveces=".$nveces,"S");
    $smarty->display('pie_pag.tpl'); exit();
  }
  else {
...
if ($vopc==1) 
 { $smarty->assign('varfocus','formarcas2.vfecvi'); 
   $smarty->assign('vmodo',''); $smarty->assign('modo',''); }
else 
