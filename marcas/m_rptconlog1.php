...
$smarty->assign('subtitulo','Auditor&iacute;a de Planillas B&uacute;squedas Gr&aacute;ficas Cargadas');
...
if ($prioridad!='T') { 
  if(!empty($where)) {
     $where = $where." AND"." (stmcntrl.tipobusq='$prioridad')";
     $titulo= $titulo." Prioridad: "."$prioridad";  }
}

if ($vplus=='I') {
  if(!empty($where)) {
     $where = $where." AND"." (stmcntrl.envio='N')";
     $titulo= $titulo." Envio por: Impresora";  }
}

if ($vplus=='C') {
  if(!empty($where)) {
     $where = $where." AND"." (stmcntrl.envio='S')";
...
if(!empty($planilla)) { 
  if(!empty($where)) {
     $from = ' stmcntrl,stmbusplan ';
     $where = $where." AND"." stmcntrl.pedido=stmbusplan.nro_pedido AND (stmbusplan.cod_planilla='$planilla')";
...
$res_pedido=pg_exec("SELECT fecharec,hora,stmcntrl.pedido,recibo,fechaing,solicitant,prioridad,identificacion,indole,telefono,usuario,sede,clase,envio,email FROM $from WHERE $where ");
$filasfound=pg_numrows($res_pedido);

//policia
//  $smarty->display('encabezado1.tpl');
//  Mensajenew('QUERY: SELECT fecharec,hora,stmcntrl.pedido,recibo,fechaing,solicitant,prioridad,identificacion,indole,telefono,usuario,sede,clase,envio,email FROM '.$from.' WHERE '. $where,'m_rptconlog.php','N');
//  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit();
//
...
$encabezado = "Busqueda Elementos Figurativos";
$titulo = "Auditoria de Planillas Graficas Cargadas";
...
$header_type = Array();
		...
		$i=6;
		$header_type[$i] = $table_default_header_type;
		$header_type[$i]['TEXT'] = utf8_decode("Telefono");
...
//Dibujando la Tabla para el pdf
 ...
 for($cont=0;$cont<$filasfound;$cont++) {
   ...
   $nplanilla = 0;
   $obj_query = $sql->query("SELECT cod_planilla FROM stmbusplan WHERE nro_pedido='$npedido' AND tipo_busq='G'");
...
//if($vplus=="I") { $totcorreos=$totcorreos-1; }
