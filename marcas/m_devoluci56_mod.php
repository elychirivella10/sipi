...
$smarty ->assign('subtitulo','Devoluci&oacute;n (L.P.I. 10/12/1956) Modificaci&oacute;n'); 
...
$vuser = $usuario;
   ...
   if (($login=='rmendoza') || ($login=='ngonzalez')) { }	
   else { 
     Mensajenew("ERROR: Modulo Bloqueado por Desuso, Usuario NO tiene Permiso ...!!!","../index1.php","N");
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
         $sql->disconnect(); 
         //$smarty->display('encabezado1.tpl');
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_devoluci56_mod.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($filas_found==0) {
         $sql->disconnect();
         //$smarty->display('encabezado1.tpl');
         mensajenew('AVISO: NO EXISTEN DATOS ASOCIADOS ...!!!','m_devoluci56_mod.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      //Verificacion del usuario de la devolucion
      if (($login=='rmendoza') || ($login=='ngonzalez')) { }	
      else { 
        ...
        if ($obj_ftram==0) {
          $sql->disconnect();
          //$smarty->display('encabezado1.tpl');
          mensajenew('AVISO: El Usuario que aplic&oacute; la Devolucion NO corresponde con '.$usuario.' ...!!!','m_devoluci56_mod.php','N');
  	  $smarty->display('pie_pag.tpl'); exit(); 
        }
      }
      ...
      if ($vest-1000==200) {  
         ...
         $sql->disconnect();
         //$smarty->display('encabezado1.tpl');
         mensajenew('AVISO: Solo Aplica en Solicitudes que tengan Estatus: 200','m_devoluci56_mod.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }

      //Inicializa variables para modificar
      //Stzcaded
      $obj_query = $sql->query("SELECT * FROM stzcaded where nro_derecho='$nderec' ORDER BY cod_causa");
      $obj_filas = $sql->nums('',$obj_query);
      $contobj = 0;
      $objs = $sql->objects('',$obj_query);
   ...
   if ($vopc==3) {
    ...
    if($vcausa25=='on'){$vc25='X';}
      ...
      if ($vsolh=='-' || $vfevh=='' || $allcausas=='') {
         $sql->disconnect();
         //$smarty->display('encabezado1.tpl');
         mensajenew('ERROR: PROBLEMAS AL INTENTAR GRABAR - DATOS INCORRECTOS ...!!!','javascript:history.back();','N');
	 $smarty->display('pie_pag.tpl'); exit();  
      }
      ...
      if ($vest-1000==200) { //Esta bien
         ...
         $sql->disconnect(); 
         //$smarty->display('encabezado1.tpl');
         mensajenew($vest.'ERROR AL INTENTAR GRABAR - La solicitud ha sido modificada por otro usuario','m_devoluci56_mod.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
	 ...
	 $smarty->display('pie_pag.tpl'); exit(); 
      ...
      if ($votro<>'') {
        ...
        $ins_otros = $sql->insert("stzotrde","$col_campos","'$nderec','$votro','M','M','M','$vfec','$hh'",""); 
...
//      if (!$ins_otros) { $inscaus = $inscaus + 1; }
      ...
      if ($inscaus==0) { 
        ...
        Mensaje2("DATOS GUARDADOS CORRECTAMENTE ...!!!","m_devoluci56_mod.php","m_rptdevfor.php?vsol=$vsold&vusr=$usuario");
        $smarty->display('pie_pag.tpl'); exit();   
      }
      else {
   ...
   //$smarty->display('encabezado1.tpl');
   $smarty ->display('m_devoluci56_mod.tpl'); 
