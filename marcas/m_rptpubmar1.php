...
$smarty->assign('subtitulo','Auditor&iacute;a de Publicaciones de Marcas en Prensa Digital SAPI');
...
if(!empty($hastac)) {
  if(empty($where)) {
    $where = $where." (stmpagopren.fecha_publi='$hastac')";
    $titulo= $titulo." Con Fecha de Publicacion el: "."$hastac"; }
  else {
    $where = $where." AND (stmpagopren.fecha_publi='$hastac')";
    $titulo= $titulo." Con Fecha de Publicacion el: "."$hastac"; }
}
...
if ($filasfound==0)  {
  $smarty->display('encabezado1.tpl');
  Mensajenew('AVISO: No existen Datos Asociados en la fecha indicada: '.$hastac.' ...!!!','m_rptpubmar.php','N');
