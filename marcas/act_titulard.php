...
$hiddenvars['total']=$total;
 ...
 if ($vmod=='Buscar/Incluir' || $vmod=='Buscar')
  {$resultado=pg_exec("SELECT * FROM stzsolic WHERE nombre like '$vtex%' order by nombre OFFSET $inicio LIMIT $cuanto");
   ...
   if ($filas_resultado==0){
       ...
       echo " <td width='77%' class='der-color'>";
       echo "   &nbsp;<input type='text' name='vdom' size='68' maxlength='200'></font></td>";
       echo "</tr>";
       ...
       echo " <td width='77%' class='der-color'>";
       echo "   &nbsp;<input type='text' name='vtl1' size='15' maxlength='15' onKeyPress='return acceptChar(event,9,this)' onKeyup='checkLength(event,this,14,document.forobfie.vfax)'>&nbsp;&nbsp;<small>Formato: (9999) 9999999</small></td>";
       echo "</tr>";
       echo "<tr>";
       echo " <td width='23%' class='izq-color'><small><b>Tel&eacute;fono 2:</b><small></td>";
       echo " <td width='77%' class='der-color'>";
       echo "   &nbsp;<input type='text' name='vtl2' size='15' maxlength='15' onKeyPress='return acceptChar(event,9,this)' onKeyup='checkLength(event,this,14,document.forobfie.vfax)'>&nbsp;&nbsp;<small>Formato: (9999) 9999999</small></td>";
       echo "</tr>";
       echo "<tr>";
       echo "  <td width='23%' class='izq-color'><small><b>Fax:</b></small></td>";
       echo "  <td width='77%' class='der-color'>";
       echo "    &nbsp;<input type='text' name='vfax' size='15' maxlength='15' onKeyPress='return acceptChar(event,9,this)' onKeyup='checkLength(event,this,14,document.forobfie.vfax)'>&nbsp;&nbsp;<small>Formato: (9999) 9999999</small></td>";
       echo "</tr>";
       echo "<tr>";
       echo "  <td width='23%' class='izq-color'><small><b>E-Mail:</b></small></td>";
       echo "  <td width='77%' class='der-color'>";
       echo "    &nbsp;<input type='text' name='vema' size='68' maxlength='80'></font></td>";
       echo "</tr>";
   ...
   echo " <td width='10%' class='celda4'>NAC.</td>";
   echo " <td width='30%' class='celda4'>DOMICILIO</td>";
   echo "</tr>";
   ...
   for($cont=0;$cont<$filas_resultado;$cont++) {
     ...
     echo " <td width='10%' class='celda3'>$reg[titular]</td>";
     echo " <td width='40%' class='celda3'>$reg[nombre]</td>";
     echo " <td width='10%' class='celda3'><select size='1' name='vnac$cont' value='$reg[nacionalidad]' size='2' onkeydown='codigotecla(document.formti.vdom$cont)'>"; 
     ...
     echo "</select></td>";
     echo " <td width='30%' class='celda3'><input type='text' name='vdom$cont' value='$reg[domicilio]' onkeydown='codigotecla(document.formti.vpai$cont)'></td>";
     echo "</tr>";
 ...
 if ($vmod=='Buscar/Eliminar'  || $vmod=='Eliminar')
  {$resultado=pg_exec("SELECT * FROM temptitu WHERE solicitud='$vsol'");
   ...
   echo " <td width='10%' class='celda4'>CODIGO</td>";
   echo " <td width='40%' class='celda4'>NOMBRE</td>";
   echo " <td width='10%' class='celda4'>NACIONALIDAD</td>";
   echo " <td width='30%' class='celda4'>DOMICILIO</td>";
   echo "</tr>";
   for($cont=0;$cont<$filas_found;$cont++) {
     ...
     $regpais = pg_fetch_array($res_pais);
     echo " <td width='10%' class='celda3'>$reg[nacionalidad]-$regpais[nombre]</td>";
     echo " <td width='30%' class='celda3'>$reg[domicilio]</td>";
...
if ($vmod=='Cambiar Domicilio')
  {$resultado=pg_exec("SELECT * FROM temptitu WHERE solicitud='$vsol' order by titular");
   $reg=pg_fetch_array($resultado);
   $filas_found=pg_numrows($resultado);
   ?>
   <form action="updatitularp.php" name="formtitular" method="post"> 
   ...
   for($cont=0;$cont<$filas_found;$cont++) {
     ...
     echo " <td width='10%' class='celda3'>$reg[titular]</td>";
     echo " <td width='40%' class='celda3'>$reg[nombre]</td>";
     echo " <td width='10%' class='celda3'><select size='1' name='vnac$cont' value='$reg[nacionalidad]' size='2'>"; 
     $res_pais=pg_exec("SELECT pais,substr(nombre,1,15) as nombre FROM stzpaisr ORDER BY pais");
     $filas_res_pais=pg_numrows($res_pais); 
     $regpais = pg_fetch_array($res_pais);
     for($i=0;$i<$filas_res_pais;$i++) 
     ...
     echo "</select></font></td>";
     echo " <td width='30%' class='celda3'><input type='text' name='vdom$cont' value=''></td>";
 ...
 if ($vmod=='Ver')
  {$resultado=pg_exec("SELECT * FROM temptitu WHERE solicitud='$vsol' and tipo_mp='$vtip'");
   $reg=pg_fetch_array($resultado);
   $filas_found=pg_numrows($resultado);
   echo "<p align='center'><font class='nota3'><b>TITULARES ASOCIADOS A LA SOLICITUD $vsol:</b></font></p>";
   echo "<table border='1' cellpadding='0' cellspacing='0' width='100%'>";
   echo "<tr>";
   echo " <td width='10%' class='celda4'>CODIGO</td>";
   echo " <td width='40%' class='celda4'>NOMBRE</td>";
   ...
   for($cont=0;$cont<$filas_found;$cont++) {
     ...
     echo " <td width='10%' class='celda3'>$reg[titular]</td>";
     echo " <td width='40%' class='celda3'>$reg[nombre]</td>";
     echo " <td width='10%' class='celda3'>$reg[nacionalidad]</td>";
     echo " <td width='30%' class='celda3'>$reg[domicilio]</td>";
     echo "</tr>";
     $reg = pg_fetch_array($resultado);
     }
   ...
   if ($filas_found==0) { echo "<p align='center'><font class='nota3'>NINGUN TITULAR ASOCIADO</font></p>"; }
   echo "<p align='center'>";
   echo "  <input type='button' class='boton_blue' value='Aceptar' name='aceptar' onclick='cerrarwindows2()'></p>";
   exit;
  }
