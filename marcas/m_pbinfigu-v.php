...
if ($vopc==1) {
  ...
  if (!file_exists($nameimage)) {
    Mensajenew("La Imagen $nameimage NO ha sido Encontrada, debe ser Scaneada para proseguir ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); }     
  ...
  $objs = $sql->objects('',$obj_query);
  if ($obj_filas==0) {
    Mensajenew("Expediente '$vsol' NO presenta Clasificacion Internacional de Viena(s) ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
