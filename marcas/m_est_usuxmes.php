...
else {$pquery3=" and evento-1000='".$veven."'"; $ptit=$ptit.' - EVENTO:'.$veven;}
...
if ($vano1!='') {
   $resultado=pg_exec("select evento-1000 as evento, count(*) as cant 
...
<tr><td class="cnt"><a href="m_est_usuxmes.php?vopc=0"><img src="../imagenes/salir_f2.png" border="0"></a>  Salir  </td></tr></table><?php
...
// Vector Eventos
$obj_query = $sql->query("SELECT * FROM stzevder WHERE tipo_mp='M' ORDER BY evento");
$obj_filas = $sql->nums('',$obj_query);
$est_val[0] = ' '; $est_des[0] = '--- TODOS LOS EVENTOS ';
$objs = $sql->objects('',$obj_query);
for ($contobj=1;$contobj-1<$obj_filas;$contobj++) {
    $est_val[$contobj] = $objs->evento-1000;
...
$smarty->display('m_est_usuxmes.tpl');
$smarty->display('pie_pag.tpl');
}
