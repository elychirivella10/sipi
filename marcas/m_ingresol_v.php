...
if ($vopc==3) {
  $smarty->assign('subtitulo','Mantenimiento de Expediente CAN-486/ Ingreso'); }
...
if ($vopc==2) {
  ...
  //Verificacion de si la solicitud existe en la base de datos ... 
  //$obj_query = $sql->query("SELECT * FROM $tbname_6 WHERE solicitud='$varsol' AND tipo_mp ='M'");
  //$obj_filas = $sql->nums('',$obj_query);
  //if ($obj_filas!=0) {
  //  Mensajenew("Solicitud ya cargada en la Base de Datos ...!!!","javascript:history.back();","N"); 
  //  $smarty->display('pie_pag.tpl'); exit(); }
  ...
  if ($accion==2) {
    ...
    if ($numtitu==0 AND $numccv==0 AND $inslema AND $instram AND $inslogo AND 
      ...
      Mensajenew("Falla de Actualizaci&oacute;n / Inserci&oacute;n de Datos en la BD $error_lem, $error_tra, $error_log, $error_pri, $error_age, $error_mar, $error_der, $error_tit, $error_ccv, $error_cla ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); exit();
    }
