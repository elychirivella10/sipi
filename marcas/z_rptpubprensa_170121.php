...
$smarty->assign('subtitulo','Solicitudes a Publicar en Prensa SAPI');
...
if ($filas_resultado==0) { $mensaje= $mensaje.'  - No se genero Solicitadas '; } 
   ...
   for($cont=0;$cont<$filas_resultado;$cont++) { 
     ...
     $update_str = "fecha_gene='$fechahoy',hora_gene='$horactual',estatus='G',publicada='S'";
     $updpagoprensa = $sql->update("stppagopren","$update_str","nro_derecho='$nderec' AND fecha_publi='$fecsolh'");
...
if ($filastotalpren!=0) {
  ...
  $pdf->Output();
    ...
    $smarty->display('encabezado1.tpl');
    mensajenew('AVISO: NO Hay solicitudes de Marcas y Patentes a Publicar en Prensa ...!!!','javascript:history.back();','N');    
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
}  
