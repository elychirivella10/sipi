...
$smarty ->assign('subtitulo','Anotaciones Marginales Cesion/Fusion/Cambio de Titular'); 
...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) {
         $sql->disconnect(); 
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect(); 
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='' || $vtipo=='') {
         $sql->disconnect();
         mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS...','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	 }
	 
	   if (empty($vfactura)) { 
         $sql->disconnect();
         mensajenew('ERROR: DEBE COLOCAR EL NUMERO DE LA FACTURA ...!!!','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
	   }
      ...
      if ($esmayor==1) {
         $sql->disconnect();
         mensajenew('ERROR: No se puede cargar un evento previo a la Fecha de la Solicitud','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($numtmp==0) {
         $sql->disconnect();
         mensajenew('ERROR: No existe(n) Titular(es) Final(es). Transaccion Abortada...','m_amcesion.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
...
//nrgg
      ...
      if ($objfilas==0) {
        mensajenew('ERROR: Factura NO existe en la Base de Datos del SISFAC ...!!!','m_amcesion.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
      ...
      switch ($vtipo) {
       case 1:
         $tipoanota = "C";
         //Datos del Detalle Cesion
         $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('0102','0102E','0102E1','0102E2','0102E3') AND fac_id=$fac_id"); 
         $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
         if ($objtotdtalle==0) {
           mensajenew('ERROR: Factura NO presenta ning&uacute;n Servicio de Cesi&oacute;n de Registro de Marcas ...!!!','m_amcesion.php','N');
           $smarty->display('pie_pag.tpl'); exit(); }
...
//nrgg
         ...
         if ($vcant_am>=$canservi) {
            mensajenew('ERROR: Factura '.$nfactura.' YA asociada a otra(s) solicitud(es)...!!!','m_amcesion.php','N');
            $smarty->display('pie_pag.tpl'); exit();
         } 
//nrgg
         if($codservi=='0102') { $totalpag= 'Total Bolivares: '; }
         else { $totalpag= 'Total US$ : '; }
         break;
       case 2:
         $tipoanota = "F";
         //Datos del Detalle Fusion 
         $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('0108','0108E','0108E1','0108E2','0108E3') AND fac_id=$fac_id"); 
         $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
         if ($objtotdtalle==0) {
           mensajenew('ERROR: Factura NO presenta ning&uacute;n Servicio de Fusi&oacute;n de Registro de Marcas ...!!!','m_amcesion.php','N');
           $smarty->display('pie_pag.tpl'); exit(); }
         $objsdta = $mysql->objects_mysql('',$objdetalle);
         $codservi = $objsdta->ser_id;
//nrgg
...
//nrgg
         ...
         if($codservi=='0108') { $totalpag= 'Total Bolivares: '; }
         else { $totalpag= 'Total US$ : '; }
         break;
       case 3:
         $tipoanota = "N";
         //Datos del Detalle Cambio del Titular 
         $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('0104','0104E','0104E1','0104E2','0104E3') AND fac_id=$fac_id"); 
         $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
         if ($objtotdtalle==0) {
           mensajenew('ERROR: Factura NO presenta ning&uacute;n Servicio de Cambio de Titular de Registro de Marcas ...!!!','m_amcesion.php','N');
           $smarty->display('pie_pag.tpl'); exit(); }
         $objsdta = $mysql->objects_mysql('',$objdetalle);
         $codservi = $objsdta->ser_id;
//nrgg
...
//nrgg
         if($codservi=='0104') { $totalpag= 'Total Bolivares: '; }
         else { $totalpag= 'Total US$ : '; }
         break;
      }
      ...
      if ($cantfil>0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR - El Evento ya fue Cargado','m_amcamdom.php','N');
         $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      {  $temptit=$regco[titular];
         ...
         if ($tempdom=='' or $tempnac=='') {
            $sql->disconnect();
            $smarty->display('encabezado1.tpl');
            mensajenew('ERROR - Falto ingresar Nacionalidad o Domicilio del Titular Final...','m_amcesion.php','N');
            $smarty->display('pie_pag.tpl'); exit(); 
         }
...
//nrgg 
      ...
      $insert_str = "'$nfac','$fechafac',0,0,0,'$sede','$tipoanota',$vcant_am";
      ...
      if ($cant_error==0) {
           ...
           $smarty->display('encabezado1.tpl');
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_amcesion.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           $smarty->display('encabezado1.tpl');
           mensajenew("Falla de Ingreso en la B.D. Transacciones Abortadas...!!!","m_amcesion.php","N");
           $smarty->display('pie_pag.tpl'); exit();   } 
   }
