...
if ($vopc==4) {
  ...
  $obj_query = $sql->query("SELECT * FROM $tbname_6 WHERE solicitud='$vresol' AND tipo_mp ='M'");
...
if (($vopc==1) || ($vopc==3) || ($vopc==4)) {
  ...
  for ($contobji=1;$contobji<=$objfilas;$contobji++) {
     ...
     //$vnomclase[$contobji] = trim(sprintf("%02d",$objs->clase_nacion)." ".trim($objs->descripcion)); 
...
if ($vopc==2) {
  ...
  if ($modalidad=="D" || $modalidad=="M") { 
     if (empty($nombre)) {
       mensajenew("Nombre de la Marca no puede estar Vacia por ser Denominativa o Mixta ...!!!","javascript:history.back();","N");
       $smarty->display('pie_pag.tpl'); exit(); } 
  }
  ...
  //Verificacion de si la solicitud existe o no en la base de datos...  
  ...
  //Verificacion de la modalidad y descripcion de la imagen 
  if ($modalidad=="N") {
  ...
  if ($accion==2) {
    ...
    $insert_str = "'$prox_derecho','$vclase','N','$modalidad','$vdis'";
    ...
    if ($numtitu==0 AND $numccv==0 AND $inslema AND $instram AND $inslogo AND 
        $numprio==0 AND $numagen==0 AND $insmarce AND $insderecho) {
      pg_exec("COMMIT WORK");
      //Desconexion de la Base de Datos
      $sql->disconnect();
      
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','m_inglpi55.php?vopc=3&nconex=$n_conex&salir=1&conx=0','S');
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
      ...
      Mensajenew("Falla de Actualizaci&oacute;n / Inserci&oacute;n de Datos en la BD $error_lem, $error_tra, $error_log, $error_pri, $error_age, $error_mar, $error_der, $error_tit, $error_ccv ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); exit();
    }
  }
}
