...
$obj_query = $sql->query("SELECT * FROM $tbname_3 WHERE tipo_mp = 'M' order by estatus");
if (!$obj_query) { 
  mensajenew('Problema al intentar realizar la consulta en la tabla $tbname_3 ...!!!','m_evelote.php','N');
  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
$obj_query = $sql->query("SELECT * FROM $tbname_2 WHERE tipo_mp = 'M' order by evento");
if (!$obj_query) { 
  mensajenew('Problema al intentar realizar la consulta en la tabla $tbname_2 ...!!!','m_evelote.php','N');
  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
$filas_found=$sql->nums('',$obj_query);
if ($filas_found==0) {
  mensajenew('La Tabla de Eventos esta Vacia ...!!!','m_evelote.php','N');
  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
