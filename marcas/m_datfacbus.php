...
$fecha   = fechahoy();
$vopc    = $_GET['vopc'];
$subtitulo = "Control de Facturas B&uacute;squeda(s) Foneticas/Graficas Diarias";
$tbname_1  = "stmbusfac";
...
if (empty($factura) || empty($fecharec)) {
    Mensajenew("ERROR: NO ingreso el Nro de la Factura o la Fecha de Recepcion en Informacion ...!!!","javascript:history.back();","N");
...
if ($vopc==1) {
  ...
  if ($factura!='') { 
    ...
    $objbusdia = $sql->query("SELECT * FROM $tbname_5 WHERE nro_factura='$nfac'");
    $objfildia = $sql->nums('',$objbusdia);
    if ($objfildia!=0) { 
      mensajenew("ERROR: NO. DE FACTURA ".$nfac." YA PRESENTADA ANTERIORMENTE ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit();
      //$objsreg  = $sql->objects('',$objbusdia);
    }
    ...
    //if ($filas_found!=0) {
    //  $fact_usa = "S";
    //  mensajenew("ERROR: NO. DE FACTURA ".$factura." USADO ANTERIORMENTE CON ".$son." BUSQUEDA(S) FONETICA...!!!","javascript:history.back();","N");
    //  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
    if ($fact_usa=="N") {
      ...
      if ($filas_found!=0) {
        $fact_usa = "S";
        mensajenew("ERROR: NO. DE FACTURA ".$nfac." USADO ANTERIORMENTE CON ".$son." BUSQUEDA(S) FONETICA...!!!","javascript:history.back();","N");
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
    }
    ...
    //if ($filas_found!=0) {
    //  $fact_usa = "S";
    //  mensajenew("ERROR: NO. DE FACTURA ".$factura." USADO ANTERIORMENTE CON ".$son." BUSQUEDA(S) GRAFICA...!!!","javascript:history.back();","N");
    //  $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
    if ($fact_usa=="N") {
      ...
      if ($filas_found!=0) {
        $fact_usa = "S";
        mensajenew("ERROR: NO. DE FACTURA ".$nfac." USADO ANTERIORMENTE CON ".$son." BUSQUEDA(S) GRAFICA...!!!","javascript:history.back();","N");
        $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
    }
    ...
    $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('001','002','003','004') AND fac_id=$fac_id"); 
    $totalserv  = $mysql->nums_mysql('',$objdetalle);
    if ($totalserv==0) {
      mensajenew('ERROR: Factura NO presenta ningun servicio de busqueda Fon&eacute;tica o Gr&aacute;fica asociado ...!!!','javascript:history.back();','N');
      $smarty->display('pie_pag.tpl'); exit(); }

    //Datos del Detalle 
    $nbusfon = 0;
    $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('001','002') AND fac_id=$fac_id"); 
    ...
    $nbusgra = 0;
    $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('003','004') AND fac_id=$fac_id"); 
    $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
    $objsdta = $mysql->objects_mysql('',$objdetalle);
    $serid = $objsdta->ser_id;
    if ($objtotdtalle==0) { $nbusgra = 0; }
    else { 
      $nbusgra = $objsdta->dtalle1_cantidad_ser; 
      if ($serid=='003') { $prioridad='B'; $prioridad1='B'; }
      else  { $prioridad='A'; $prioridad1='A'; }
    }
  ...
  if ($clave!='') {
    if ($clave!='12345') { 
      mensajenew("ERROR: La Clave es INVALIDA ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); exit(); 
    } 	
...
$smarty->assign('campo13','Fecha Recibida en Informaci&oacute;n:');
...
$smarty->display('m_datfacbus.tpl');
