...
// Desarrollado Año: 2020 II Semestre
// *************************************************************************************
...
$smarty->assign('subtitulo','Generaci&oacute;n de Oficios de Devoluci&oacute;n x Bolet&iacute;n sin Firma Digital Expediente');
...
if ($vopc==2) {
 ...
 $vsol3 = $_POST["vsol1h"];
 $vsol4 = $_POST["vsol2h"];
 $rutafinal = $ruta_devolucion."/marcas/forma/boletin";
 
 if ((empty($boletin)) && (empty($vsol1)) && (empty($vsol2)) && (empty($vsol3)) && (empty($vsol4))) {
   mensajenew('ERROR: Debe introducir un Criterio de B&uacute;squeda ...!!!','m_devforesinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 }

 if ($boletin < 602) {
   mensajenew('ERROR: El Boletin a generar y firmar NO puede ser menor al 602 ...!!!','m_devforesinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 }

 if ((!empty($vsol1)) && (!empty($vsol2)) && ((empty($vsol3)) || (empty($vsol4)))) {
   mensajenew('ERROR: Debe introducir el rango final de Solicitud para proseguir ...!!!','m_devforesinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 } 

 if ((!empty($vsol1)) && (!empty($vsol2)) && (!empty($vsol3)) && (!empty($vsol4))) {
   if (empty($boletin)) {
     mensajenew('ERROR: Debe introducir el Numero de Boletin para proseguir ...!!!','m_devforesinfir.php?vopc=1','N');   
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
   } else {
     ...
     if ($vsoli > $vsolf) {  
       $smarty->display('encabezado1.tpl');
       mensajenew('ERROR: Rango de solicitudes erroneo ...!!!','m_devforesinfir.php?vopc=1','N');    
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
*/
   ...
   $resultado=pg_exec("SELECT b.solicitud, b.tramitante, b.nombre, b.agente,b.nro_derecho,b.registro,b.estatus, b.poder, a.clase, a.modalidad  FROM stmmarce a, stzderec b, stzevtrd c 
		...
		AND tipo_mp='M' 
                AND ".$queryfecha."
   		AND b.estatus in (1200)
   		AND c.evento IN (1500) ORDER BY b.solicitud");
   */
   
     //verificando los resultados
     if (!$resultado) { 
       mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','m_devforesinfir.php?vopc=1','N');   
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
*/
 ...
 if ($filas_found>0) {
  ...
  $codesDir = "../documentos/devueltas/marcas/imagen_qr/"; 
  ...
  for($cont=0;$cont<$filas_found;$cont++)   {
    ...
    $content = "http://webpi.sapi.gob.ve/devolucion/verificadocwpi.php?s=$varsol&b=$boletin&c=$codseg2";
    ...
    //$pdf->MultiCell(198,4,' Atentamente,');
    //$y = $pdf->Gety();
    ...
    //$pdf->Image('../documentos/devueltas/marcas/imagen_qr/'.$filenameqr,170,$y+5,35,35,'PNG');
...
/*
   ...
   echo "   </tr>";
   echo "</table>";
   echo "<p align='center'><a href='../marcas/m_devforsinfir.php?vopc=1'><input type='image' name='continuar' value='Continuar' onclick='window.close()' src='../imagenes/apply_f2.png' alt='Continuar' align='middle' border='0' />Continuar</a></p>";
   echo "<br>";
...
if ($vopc==1) {
 ...
 $smarty->display('m_devforesinfir.tpl');
 $smarty->display('pie_pag.tpl');
}
