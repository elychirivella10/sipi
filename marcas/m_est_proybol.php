...
$smarty->assign('subtitulo','Estadisticas - Proyeccion Estatus/Boletin');
...
$resultado=pg_exec("SELECT estatus,count(*) as total FROM stzderec 
	WHERE tipo_mp='M' AND estatus IN (1001,1002,1004,1005,1006,1008,1200,1300,1101,1102,1025) GROUP BY 1 ORDER BY 1"); 

/*

if ($chekvar=='') {
$resultado=pg_exec("SELECT estatus,count(*) as total FROM stzderec b
	WHERE tipo_mp='M' AND estatus IN (1001,1002,1004,1005,1006,1008,1200,1300,1101,1102,1025) group by 1"); 
} else {
$resultado=pg_exec("SELECT estatus,count(*) as total FROM stzderec b
	WHERE tipo_mp='M' AND estatus IN (1001,1002,1004,1005,1006,1008,1200,1300,1101,1102,1025)".$pquery1.$pquery11.$pquery2.$pquery3.$pquery5." group by 1"); 
$resultado=pg_exec("SELECT estatus,count(*) as total FROM stmmarce a, stzderec b
	WHERE a.nro_derecho=b.nro_derecho AND estatus IN (1001,1002,1004,1005,1006,1008,1200,1300,1101,1102,1025) AND tipo_mp='M'".$pquery1.$pquery11.$pquery2.$pquery3.$pquery5." group by 1"); 
...
for ($cant=0;$cant<$cant_fil;$cant++) {
    if ($registro['estatus']=='1001') {$cant_1001=$registro[total];}
    if ($registro['estatus']=='1002') {$cant_1002=$registro[total];}
    if ($registro['estatus']=='1004') {$cant_1004=$registro[total];}
    if ($registro['estatus']=='1005') {$cant_1005=$registro[total];}
    if ($registro['estatus']=='1006') {$cant_1006=$registro[total];}
    if ($registro['estatus']=='1008') {$cant_1008=$registro[total];}
    if ($registro['estatus']=='1200') {$cant_1200=$registro[total];}
    if ($registro['estatus']=='1300') {$cant_1300=$registro[total];}
    if ($registro['estatus']=='1101') {$cant_1101=$registro[total];}
    if ($registro['estatus']=='1102') {$cant_1102=$registro[total];}
    $registro=pg_fetch_array($resultado);
}

$cant_total=$cant_1001+$cant_1002+$cant_1004+$cant_1005+$cant_1006+$cant_1008+$cant_1200+$cant_1300+$cant_1101+$cant_1102;
$datay=array($cant_1001,$cant_1002,$cant_1004,$cant_1005,$cant_1006,$cant_1008,$cant_1200,$cant_1300,$cant_1101,$cant_1102); 
$datax=array("1","2","4","5","6","8","200","300","101","102"); 
...
// Setup the titles
$graph->title->Set("TOTAL SOLICITUDES X ESTATUS ACTUAL MARCAS".$vtit0.$ptit."\n \n ");
...
$smarty->display('m_est_proybol.tpl');
$smarty->display('pie_pag.tpl');
}
