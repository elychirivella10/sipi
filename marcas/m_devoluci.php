...
$smarty ->assign('fechahoy',$fecha); 
   ...
   $manphp = vman_php("m_devoluci.php");
   if ($manphp==1) {
   ...
   if ($vopc==1) {
      $resultado=pg_exec("SELECT * FROM stmmarce WHERE solicitud='$vsol' and solicitud!=''");
   }
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR AL PROCESAR LA BUSQUEDA','m_devoluci.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $smarty->display('encabezado1.tpl');
         mensajenew('NO EXISTEN DATOS ASOCIADOS','m_devoluci.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }	 
      ...
      if ($vest==1 || $vest==103 || $vest==123 || $vest==8) { 
	    ...
	    $smarty ->assign('lcausadev','Causales de Devoluci&oacute;n - Examen de Fondo:'); 
         ...
         $smarty->display('encabezado1.tpl');
         mensajenew('Solo Aplica en Solicitudes que tengan Estatus: 1, 103, 123 u 8','m_devoluci.php','N');
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      }
   }
   ...
   if ($vopc==3) {
	 ...
	 $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
      ...
      // Mensaje final
      $smarty->display('encabezado1.tpl');
      mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_devoluci.php','S');
      $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit();   
