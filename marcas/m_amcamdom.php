...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) { 
         $sql->disconnect();
         mensajenew('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!','m_amcamdom.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensajenew('ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!','m_amcamdom.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect();
         mensajenew('ERROR: Solo aplica a MARCAS con REGISTRO ASIGNADO ...!!!','m_amcamdom.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
      ...
      if ($vsolh=='-' || $vregh=='' || $vcomenta=='' || $vdoc=='' || $vfevh=='') {
        $sql->disconnect();
        $smarty->display('encabezado1.tpl');
        mensajenew('ERROR: PROBLEMA AL INTENTAR GRABAR - DATOS INCORRECTOS O VACIOS ...!!!','m_amcamdom.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
	 }
	 
	   if (empty($vfactura)) { 
        $sql->disconnect();
        mensajenew('ERROR: DEBE COLOCAR EL NUMERO DE LA FACTURA ...!!!','m_amcamdom.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
	   }
      ...
      if ($esmayor==1) {
        $sql->disconnect();
        mensajenew('ERROR: No se puede cargar un evento previo a la Fecha de la Solicitud ...!!!','m_amcamdom.php','N');
	     $smarty->display('pie_pag.tpl'); exit(); 
      }
//nrgg
      $vcant_am=0;
...
//nrgg
        ...
        //mensajenew('ERROR: Factura '.$nfactura.' YA usado anteriormente en otro Servicio ...!!!','m_amcamdom.php','N');
        //$smarty->display('pie_pag.tpl'); exit(); 
//nrgg
      } 
      ...
      if ($objfilas==0) {
        mensajenew('ERROR: Factura NO existe en la Base de Datos del SISFAC ...!!!','m_amcamdom.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
      ...
      //Datos del Detalle Cambio de Domicilio
      $objdetalle = $mysql->query_mysql("SELECT ser_id,dtalle1_cantidad_ser FROM sfa_dtalles_1 WHERE ser_id IN ('0105','0105E','0105E1','0105E2','0105E3') AND fac_id=$fac_id"); 
      $objtotdtalle = $mysql->nums_mysql('',$objdetalle);
      if ($objtotdtalle==0) {
        mensajenew('ERROR: Factura NO presenta ning&uacute;n Servicio de Cambio de Domicilio de Registro de Marcas ...!!!','m_amcamdom.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
...
//nrgg
      ...
      if ($vcant_am>=$canservi) {
         mensajenew('ERROR: Factura '.$nfactura.' YA asociada a otra(s) solicitud(es)...!!!','m_amcamdom.php','N');
         $smarty->display('pie_pag.tpl'); exit();
      }
//nrgg
      if($codservi=='0105') { $totalpag= 'Total Bolivares: '; }
      ...
      // Verificacion del Cambio de domicilio a alguno de los titulares
       ...
       if ($vcomen=='') {
         ...
         $sql->disconnect();
         mensajenew('ERROR: No realizo ningun Cambio de Domicilio ...!!!','m_amcamdom.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
       } 
...
//nrgg
      ...
      $insert_str = "'$nfac','$fechafac',0,0,0,'$sede','D',$vcant_am";
      ...
      if ($cant_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','m_amcamdom.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
           pg_exec("ROLLBACK WORK"); $sql->disconnect();
           mensajenew("ERROR: Falla de Ingreso en la B.D. Transacciones Abortadas...!!!","m_amcamdom.php","N");
           $smarty->display('pie_pag.tpl'); exit();   } 
   }
