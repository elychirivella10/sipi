...
$smarty ->assign('subtitulo','Actualizaci&oacute;n de Fechas Disposiciones Administrativas Eventos (97, 94) en Bolet&iacute;n'); 
...
$smarty->display('encabezado1.tpl');
   ...
   if ($vopc==2) {
      ...
      if ($vbol=='') {
         mensajenew('ERROR: DATOS INCORRECTOS O VACIOS','m_actdispadm.php','N');
      ...
      if ($bol_found==0) {
        mensajenew("ERROR: Bolet&iacute;n '$vbol' NO Existe o NO Generado ...!!!","m_actdispadm.php","N");
        $smarty->display('pie_pag.tpl'); exit();           
      }
      ...
      if ($cantren <= 0) {
        $sql->disconnect();
        mensajenew('ERROR: PROBLEMA AL INTENTAR PROCESAR - No existen Datos para Actualizar','m_actdispadm.php','N');
        $smarty->display('pie_pag.tpl'); exit(); 
      }
	  ...
	  $gestor = trim($regv['tramitante']);
          ...
          if (!$obj_query2) { 
            mensajenew('ERROR: Problema al intentar realizar la consulta en la tabla stzevtrd ...!!!','m_actdispadm.php','N');
      ...
      if ($can_error==0) {
           pg_exec("COMMIT WORK"); 
           $sql->disconnect();
           mensajenew("AVISO: Se Actualizaron '$cantren' Solicitudes",'m_actdispadm.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
   ...
   $smarty ->display('m_actdispadm.tpl'); 
