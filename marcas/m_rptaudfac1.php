...
// *************************************************************************************
// Programa: m_rptaudfac1.php 
// Realizado por el Analista de Sistema Romulo Mendoza 
// Coordinación de Informática / Dirección de Soporte Administrativo / SAPI / MICO
// Creado Año: 2011 Semestre 
// *************************************************************************************
ob_start();
...
//echo "$desde, $hasta, $desdec, $hastac "; exit();
...
if(!empty($desde) && !empty($hasta)) {
  ...
  if(empty($where)) {
    $where = $where." (stmbusfac.f_recibo >='$desde' AND stmbusfac.f_recibo <='$hasta')";
...
if(!empty($desdec) && !empty($hastac)) {
  ...
  if(empty($where)) {
    $where = $where." (stmbusfac.f_carga>='$desdec' AND stmbusfac.f_carga<='$hastac')";
    $titulo= $titulo." Cargado el: "."$desdec"." Hasta: "."$hastac"; }
  else {
    $where = $where." AND (stmbusfac.f_carga>='$desdec' AND stmbusfac.f_carga<='$hastac')";
    $titulo= $titulo." Cargado el: "."$desdec"." Hasta: "."$hastac"; }
...
if(!empty($usuario)) { 
  if(!empty($where)) {
     $where = $where." AND"." (stmbusfac.usuario='$usuario')";
     $titulo= $titulo." por el Usuario: "."$usuario";  }
}

if(!empty($busqueda)) { 
  if(!empty($where)) {
     $where = $where." AND"." (stmbusfac.busqueda='$busqueda')";
     $titulo= $titulo." Tipo Busqueda: "."$busqueda";  }
}

if(!empty($vsede)) { 
  if(!empty($where)) {
     $where = $where." AND"." (stmbusfac.sede='$vsede')";
...
function Header()
{
   ...
   if ($tipbus=='F') { $this->Cell(0,6,'Busqueda Fonetica',0,1,'C'); }
   if ($tipbus=='G') { $this->Cell(0,6,'Busqueda Grafica',0,1,'C'); }
   //$this->Image("../imagenes/milco1.jpg",160,5,40,10,'JPG');
   $this->SetFont('Arial','BU',10);
   $this->Cell(0,7,'Auditoria de Facturas Busquedas Cargadas',0,1,'C'); 
   ...
   $this->Cell(18,8,'F. Recibo',1,0,'C'); 
   $this->Cell(18,8,'Hora',1,0,'C'); 
   $this->Cell(12,8,'Recibo',1,0,'C'); 
   $this->Cell(12,8,'Tipo',1,0,'C'); 
   $this->Cell(12,8,'Cantidad',1,0,'C'); 
   $this->Cell(70,8,'Solicitante',1,0,'C');
   $this->Cell(12,8,'Indole',1,0,'C');
   $this->Cell(9,8,'Sede',1,0,'C'); 
   $this->Cell(18,8,'Usuario',1,0,'C'); 
   $this->Cell(8,8,'Prior.',1,1,'C'); 
...
$pdf->AliasNbPages();
  ...
  $res_pedido=pg_exec("SELECT * FROM stmbusfac WHERE $where");
  $filasfound=pg_numrows($res_pedido);
  $total=$filasfound;
  if ($filasfound==0)    {
     $smarty->display('encabezado1.tpl');
     mensajenew('AVISO: No existen Datos Asociados ...!!!','m_rptaudfac.php','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); } 
  $total_nor = 0;
  $total_hab = 0;
  $total_fon = 0;
  ...
  for($cont=0;$cont<$filasfound;$cont++)   { 
     ...
     //    break;
     //}
     $solicitante = substr($solicitante,0,40);
     $pdf->ln(2);
     $pdf->Cell(18,8,$fecharec,0,0); 
     $pdf->Cell(18,8,$hora,0,0);
     $pdf->Cell(12,8,$recibo,0,0);
     $pdf->Cell(12,8,$busqueda,0,0);
     $pdf->Cell(12,8,$cantidad,0,0);
     $pdf->Cell(70,8,utf8_decode($solicitante));     
     $pdf->Cell(12,8,$tipindole,0,0);
     $pdf->Cell(9,8,$sede,0,0);
     $pdf->Cell(18,8,$vusuario,0,0);
     $pdf->Cell(8,8,$tipo,0,1); 
  ...
  $pdf->Cell(25,8,"Total de Facturas Cargadas: ".$total."               donde la Cantidad de Foneticas: ".$total_fon."               Cantidad de Graficas: ".$total_gra."               Cantidad de Peticionarios: ".$total_pet,0,1); 
  $pdf->ln(1);
  $pdf->Cell(25,8,"Total Habilitadas Cargadas: ".$total_hab,0,1);
  $pdf->Cell(25,8,"Total Normal Cargadas: ".$total_nor,0,1);
  $pdf->ln(8);
  $raya   = "___________________                     __________________               _____________________";
  $firmas = "Firma de Información                            Firma de Finanza                         Firma de Búsqueda";
  $fecha  = "Fecha de Consignación: ______________________";
  $pdf->Cell(25,8,utf8_decode($raya),0,1); 
  $pdf->Cell(25,8,utf8_decode($firmas),0,1); 
  $pdf->ln(2);
  $pdf->Cell(25,8,utf8_decode($fecha),0,1); 
