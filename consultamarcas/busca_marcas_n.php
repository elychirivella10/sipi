...
$varsol2=$_POST["vsol2"];
$varreg1=$_POST["vreg"];
if (empty($varreg1)) { $varreg1=$_GET["vreg"];}
$varsel=$_POST["vsel"];
$vartex1=$_POST["vtex"];
...
if (!$resultado) 
   { echo "<hr>";     
     echo "<p align='center'><b>ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA ...!!!$varsol</b>"; 
     echo "<hr>";
     //pg_close($conexdb);
...
if ($filas_found==0) 
   {
   echo "<hr>";     
   echo "<p align='center'><b>ERROR: NO EXISTEN DATOS ASOCIADOS ...!!!$varsol</b>\n"; 
   echo "<hr>";
   // Desconexion de la Base de datos 
   $sql->disconnect();
   //pg_close($conexdb); 
...
$filtro ="";
$fechacon = hoy();
$sys_actual = control_auditor($login,$fechacon,$varsol,$filtro);
...
$solwebpi = trim($reg['tramitewebpi']);
$nagen=$reg[agente];
...
if ( file($nameimage))
   { echo "<td width='22%' rowspan='9' align='center' style='background-color: #FFFFFF; border: 1 solid #C6DEF2'><a href='$nameimage' target='_blank'><img border='0' src=$nameimage width='210' height='190'></td></a>"; 
   } else {
     if ( file($nameimage_may))
     { echo "<td width='22%' rowspan='9' align='center' style='background-color: #FFFFFF; border: 1 solid #C6DEF2'><a href='$nameimage_may' target='_blank'><img border='0' src=$nameimage_may width='210' height='190'></td></a>"; 
     }
   }
?>
  </tr>
...
$resultado = pg_exec("SELECT stzottid.titular, stzsolic.nombre, stzottid.nacionalidad, stzottid.domicilio, stzottid.pais_domicilio, stzsolic.indole, stzsolic.identificacion
   ...
   for($cont=0;$cont<$filas_found;$cont++) 
     {
      ...
      $res_pais=pg_exec("SELECT * FROM stzpaisr WHERE pais='$reg[pais_domicilio]' and pais!=''");
      ...
      echo "<td width='20%' class='celda2'>$reg[pais_domicilio] - $respaidom[nombre]</td>";
  ...
  <tr>
    ...
    <?php
     if (($vtipuser==1) || ($vtipuser==5) || ($vtipuser==6)) { 
       ...
       else { echo "<td width='10%' class='columna-titulo'>Usuario</td>"; }
  ...
  <?php
   ...
   for($cont=0;$cont<$filas_found;$cont++) 
	...
	$boletin = $reg[documento];
     ...
     echo "<tr>";
     echo "<td width='10%' class='celda8'><b>$reg[fecha_event]</b></td>";
     ...
     $conpdf=0;
     //if (($evento==122) AND (($estatus_ant==200) || ($estatus_ant==116)) AND ($boletin>=602)) { 
     if (($evento==122) AND (($estatus_ant==200) ) AND ($boletin>=602)) { 
       if ($estatus_ant==200) { 
...
//Evento 20 - Escritos de Contestacion a Oficios de Devolucion
     ...
     if ($evento==20 AND $boletin>=609 AND $usrcarga=='admwebpi' AND ($estatus_ant==118 or $estatus_ant==113 or $estatus_ant==115)) { 
       if ($estatus_ant==113 or $estatus_ant==115) { 
          $archivodev = "http://172.16.0.7/documentos/cdevolucion/marcas/forma/boletin".$boletin."/ecd_".$varsol1.$varsol2.".pdf";
       }  
       else {
          $archivodev = "http://172.16.0.7/documentos/cdevolucion/marcas/fondo/boletin".$boletin."/ecd_".$varsol1.$varsol2.".pdf";
       }         
       echo "<td width='10%' class='celda8'><a href='$archivodev' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }

//Evento 28 - Prorroga de Contestacion a Oficios de Devolucion

     if ($evento==28 AND $boletin>=609 AND $usrcarga=='admwebpi' AND ($estatus_ant==113)) { 
        $archivodev = "http://172.16.0.7/documentos/prorrogas/marcas/boletin".$boletin."/epd_".$varsol1.$varsol2.".pdf";
        echo "<td width='10%' class='celda8'><a href='$archivodev' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }
...
//Evento 40 - Escrito de Oposicion
     ...
     if ($evento==40 AND $boletin>=609 and $usrcarga=='admwebpi' and ($estatus_ant==8 or $estatus_ant==3 or $estatus_ant==125 or $estatus_ant==130)) { 

       $resulesc = pg_exec("SELECT * FROM stzdocumento WHERE nro_derecho='$nderec' AND evento =1040 AND secuencial = $controlsec AND tipo_mp='M'");   
       $filas_esc=pg_numrows($resulesc);
       $regesc = pg_fetch_array($resulesc);
       $archivoesc = trim($regesc['documento']);
 
       $archivoesc = "http://172.16.0.7/documentos/oposiciones/marcas/boletin".$boletin."/".$archivoesc;
...
//Evento 43 - Escrito de Oposicion por Mejor Derecho
     ...
     if ($evento==43 AND $boletin>=609 and $usrcarga=='admwebpi') { 
       ...
       $resulesc = pg_exec("SELECT * FROM stzdocumento WHERE nro_derecho='$nderec' AND evento =1043 AND secuencial = $controlsec AND tipo_mp='M'");   
       $filas_esc=pg_numrows($resulesc);
       $regesc = pg_fetch_array($resulesc);
       $archivoesc = trim($regesc['documento']);
 
       $archivoesc = "http://172.16.0.7/documentos/opmejorderecho/marcas/boletin".$boletin."/".$archivoesc;
       echo "<td width='10%' class='celda8'><a href='$archivoesc' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }
...
//Evento 48 - Escrito de Contestacion a Oposicion
     ...
     if ($evento==48 AND $boletin>=609 and $usrcarga=='admwebpi' and ($estatus_ant==120)) { 

       $resulesc = pg_exec("SELECT * FROM stzdocumento WHERE nro_derecho='$nderec' AND evento =1048 AND secuencial = $controlsec AND tipo_mp='M'");   
       $filas_esc=pg_numrows($resulesc);
       $regesc = pg_fetch_array($resulesc);
       $archivoesc = trim($regesc['documento']);
 
       $archivoesc = "http://172.16.0.7/documentos/coposicion/marcas/boletin".$boletin."/".$archivoesc;
       echo "<td width='10%' class='celda8'><a href='$archivoesc' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }

//Evento 310 - Escrito de Reconsideracion a negada

     if ($evento==310 AND $boletin>=615 and $usrcarga=='admwebpi' and (($estatus_ant==500) OR ($estatus_ant==128))) { 

       $resulesc = pg_exec("SELECT * FROM stzdocumento WHERE nro_derecho='$nderec' AND evento =1310 AND secuencial = $controlsec AND tipo_mp='M'");   
       $filas_esc=pg_numrows($resulesc);
       $regesc = pg_fetch_array($resulesc);
       $archivoesc = trim($regesc['documento']);
 
       $archivoesc = "http://172.16.0.7/documentos/recursos/marcas/negadas/boletin".$boletin."/".$archivoesc;
       echo "<td width='10%' class='celda8'><a href='$archivoesc' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }
...
//Evento 310 - Escrito de Reconsideracion a Prioridad Extinguida
     ...
     if ($evento==310 AND $boletin>=616 and $usrcarga=='admwebpi' and (($estatus_ant==600))) { 
       ...
       $archivoesc = "http://172.16.0.7/documentos/recursos/marcas/prioridadex/boletin".$boletin."/".$archivoesc;
       echo "<td width='10%' class='celda8'><a href='$archivoesc' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }

//Evento 310 - Escrito de Reconsideracion a Caducas

     if ($evento==310 AND $boletin>=616 and $usrcarga=='admwebpi' and (($estatus_ant==751))) { 
       ...
       $archivoesc = "http://172.16.0.7/documentos/recursos/marcas/caducas/boletin".$boletin."/".$archivoesc;
       echo "<td width='10%' class='celda8'><a href='$archivoesc' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }
...
//Evento 310 - Escrito de Reconsideracion a Perencion de Procedimiento Prensa
     ...
     if ($evento==310 AND $boletin>=616 and $usrcarga=='admwebpi' and (($estatus_ant==651))) { 
       ...
       $archivoesc = "http://172.16.0.7/documentos/recursos/marcas/ppprensa/boletin".$boletin."/".$archivoesc;
       echo "<td width='10%' class='celda8'><a href='$archivoesc' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
       $conpdf=1;
     }


//Insertado por nrgg el 04/06/2021 para visualizar devoluciones de fondo a partir del boletin 608
     if (($evento==122) AND (($estatus_ant==116) ) AND ($boletin>=608)) { 
        $archivodev = "../documentos/devueltas/marcas/fondo/boletin".$boletin."/".$varsol1.$varsol2.".pdf"; 
        echo "<td width='10%' class='celda8'><a href='$archivodev' target='_blank'><img border='1' src='$imagenresultado' width='40' height='40'></a></td>";
        $conpdf=1;
     }
