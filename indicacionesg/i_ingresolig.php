...
function browserepre(var1,var2,var3,var4) {
  this.derecho='I';
  open("../comun/adm_repre.php?vsol="+var1.value+"-"+var2.value+"&vtex="+var3.value+"&vmod="+var4.value+"&vtip="+this.derecho,"Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }
...
$modalidad=$_POST['tipo_marca'];
$fecha_solic=$_POST['fecha_solic'];
$tipo_marca='I';
$nombre=$_POST['nombreig'];
$psoli=$_POST['psoli'];
$vpod1=$_POST['vpod1'];
$vpod2=$_POST['vpod2'];
...
//
$productosig=$_POST['productosig'];
$zonag=$_POST['zonag'];
$tratado=$_POST['tratado'];
$deszonag=$_POST['deszonag'];
...
if ($vopc==4) {
  ...
  $resulm = pg_exec("SELECT * FROM stzderec WHERE solicitud='$vresol' AND tipo_mp='I'");
  ...
  if ($nfil!=0) { 
    ...
    $fhoy = hoy();
    $sql->del("stztmptit","fecha_carga <'$fhoy'"); 
    $sql->del("stztmptit","solicitud='$vresol' AND tipo_mp='I'");
    $sql->del("stztmprepre","fecha_carga <'$fhoy'"); 
    $sql->del("stztmprepre","solicitud='$vresol' AND tipo_mp='I'");
 }  
}   

$smarty->assign('arraytipom',array(I,R));
$smarty->assign('arraynotip',array('Declaraci&oacute;n de Indicaci&oacute;n Geogr&aacute;fica',
                                   'Reconocimiento de Indicaci&oacute;n Geogr&aacute;fica'));
...
if (($vopc==1) || ($vopc==3) || ($vopc==4)) {
  ...
  if (!$obj_query) { 
    mensajenew("ERROR: Problema al intentar realizar la consulta en la tabla stzpaisr...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
if ($vopc==2) {
  ...
  $req_fields = array("fecha_solic","tipo_marca","nombreig","productosig");
  $valores = array($fecha_solic,$tipo_marca,$nombre,$productosig);
  ...
  $obj_query = $sql->query("SELECT * FROM stzderec WHERE solicitud='$varsol' AND tipo_mp ='I'");
  ...
  if ($modalidad=="N" or empty($modalidad) ) {
      mensajenew("AVISO: No ha seleccionado el tipo de Indicación Geografica ...!!!","javascript:history.back()","N");
      $smarty->display('pie_pag.tpl'); exit(); }
...
//     $smarty->display('pie_pag.tpl'); exit(); 
  ...
  $res_agen=pg_exec("SELECT * FROM $tbname_18 where solicitud='$varsol' AND tipo_mp='I' ORDER BY agente");
...
//      $smarty->display('pie_pag.tpl'); exit(); }
  ...
  if ($accion==2) {
    $resultado=pg_exec("SELECT * FROM stzevder WHERE evento=3200");
    if (!$resultado) { 
    ...
    $insert_str = "'$prox_derecho','$tipo_marca','$varsol','$fecha_solic','I','$vnom',3001,'$vcodpais','$vpoder','$vtra','$vcodage'";
    $insderecho = $sql->insert("$tbname_6","$col_campos","$insert_str","");

    //Insercion del Registro Nuevo en la Maestra de Indicaciones Geograficas   
    $col_campos = "nro_derecho,productodesig,zonageograf,tratadoint,distingue,describezona,vinculo,observacion,modalidad";
    $distingue = trim($distingue);
    $vdis = str_replace("'","´",$distingue);
    $vdis = stripslashes($distingue);

    $insert_str = "'$prox_derecho','$productosig','$zonag','$tratado','$vdis','$deszonag','$vinculo','$observa','$modalidad'";
    ...
    $insert_str = "'$prox_derecho'3200,'$fecha_solic',nextval('stzevtrd_secuencial_seq'),3000,0,'$fechahoy','$usuario','$vdes','$horactual'";
    ...
    $res_prio=pg_exec("SELECT * FROM $tbname_19 where solicitud='$varsol' AND tipo_mp='I'");
    ...
    $del_datos = $sql->del("$tbname_19","solicitud='$varsol' AND tipo_mp='I'");
    ...
    $del_datos = $sql->del("$tbname_18","solicitud='$varsol' AND tipo_mp='I'");

    // Tabla de Solicitantes o Titulares 
    $res_titu=pg_exec("SELECT * FROM $tbname_9 where solicitud='$varsol' AND tipo_mp='I'");
    ...
    $del_datos = $sql->del("$tbname_9","solicitud='$varsol' AND tipo_mp='I'");

   // Tabla de Representante Legal 
    $res_titu=pg_exec("SELECT * FROM stztmprepre where solicitud='$varsol' AND tipo_mp='I'");
    $filas_res_titu=pg_numrows($res_titu); 
    $regtitu = pg_fetch_array($res_titu);
    $numtitu = 0; 
    $ins_solic = true;
     ...
     if ($regtitu[titular]=="0")
       {
         ...
         $col_campos = "nro_derecho,titular,nacionalidad,domicilio,pais_domicilio";
         $insert_str = "'$prox_derecho','$act_titular','$regtitu[nacionalidad]','$vdom','$regtitu[pais_domicilio]'";
         $ins_titur = $sql->insert("stirepid","$col_campos","$insert_str","");
         if ($ins_solic AND $ins_titur) { }
         else { $numtitu = $numtitu + 1; }  
       }
     else
       {
         $obj_query = $sql->query("SELECT * FROM stirepid where nro_derecho='$prox_derecho' and titular='$regtitu[titular]'");
         if (!$obj_query) { 
           Mensajenew("Problema al intentar realizar la consulta en la tabla stirepid ...!!!","javascript:history.back();","N");
           $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
         $filas_titular=$sql->nums('',$obj_query);
         if ($filas_titular==0) {
           $vdomit = str_replace("'","´",$regtitu[domicilio]);
    ...
    $del_datos = $sql->del("stztmprepre","solicitud='$varsol' AND tipo_mp='I'");

    if ($numtitu==0 AND $instram AND $numprio==0 AND $numagen==0 AND $insmarce AND $insderecho) {
      pg_exec("COMMIT WORK");
      //Desconexion de la Base de Datos
      $sql->disconnect();
      Mensajenew('DATOS GUARDADOS CORRECTAMENTE !!!','i_ingresolig.php?vopc=3&nconex=$n_conex&salir=1&conx=0','S'); 
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
      ...
      if (!$insmarce)   { $error_mar = " - Indicacion Geografica "; }
...
$smarty->assign('psoli',$vsol); 
//
$smarty->assign('productosig',$productosig);
$smarty->assign('zonag',$zonag);
$smarty->assign('tratado',$tratado);
$smarty->assign('deszonag',$deszonag);
$smarty->assign('vinculo',$vinculo);
$smarty->assign('observa',$observa);
//
$smarty->display('i_ingresolig.tpl');
