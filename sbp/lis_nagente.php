...
// (Lis_nagente.php por Codigo de Agente)
...
$criterio="Criterio: C&oacute;digo Agente= ".$valor1; 
  ...
  switch($modoconsulta)
   {
	...
	$resultado=pg_exec("SELECT stzderec.solicitud, stzderec.nombre  
			 FROM stzderec, stzautod, stzagenr 
			 WHERE stzagenr.agente like '%$valor1%' 
                         AND stzautod.agente = stzagenr.agente
                         AND stzderec.nro_derecho =stzautod.nro_derecho 
  		         AND stzderec.tipo_mp='P' OFFSET $inicio LIMIT $cuanto");
	$cantidad=pg_exec("SELECT stzderec.solicitud, stzderec.nombre  
...
<input type="hidden" name="agente" value="<?= $_POST['agente'] ?>">
