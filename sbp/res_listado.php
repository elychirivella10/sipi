...
$titulo= "Criterio de Busqueda: ".utf8_decode($criterio);
...
$header_type = Array();
		...
		$i=3;
		$header_type[$i] = $table_default_header_type;
		$header_type[$i]['TEXT'] = utf8_decode("Fecha Ven.");
		$header_type[$i]['WIDTH'] = 19;
		$i=4;
		$header_type[$i] = $table_default_header_type;
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado;$cont++) { 
	...
	$data[3]['TEXT'] = $registro['fecha_venc'];
	$data[4]['TEXT'] = $registro['tipo_derecho'];
	$data[5]['TEXT'] = $registro['estatus']-2000;
   $data[5]['ALIGN'] = "L";
	$data[6]['TEXT'] = utf8_decode(trim($registro['ntitular'])).", Domicilio: ".utf8_decode(trim($registro['domicilio'])).", Nacionalidad: ".$registro['nacionalidad'];
	$registro = pg_fetch_array($resultado);
	$pdf->Draw_Data($data);
  }
