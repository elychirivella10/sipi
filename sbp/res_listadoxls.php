...
$resultado=pg_exec("SELECT DISTINCT ON (consulta4.solicitud) consulta4.solicitud, stzderec.fecha_solic, stzderec.registro, stzderec.fecha_venc, stzderec.nombre,stzderec.tipo_derecho, stzderec.estatus, stzderec.tramitante,stzsolic.nombre as ntitular,stzottid.domicilio,stzottid.nacionalidad, stzderec.nro_derecho,stppatee.resumen  
FROM consulta4, stppatee, stzottid, stzsolic, stzderec WHERE $where ORDER BY 1");
...
echo "<th scope='col'>Fecha Ven.</th>";
...
echo "<th scope='col'>Boletin Pub.</th>";
...
//echo "<td>Estudiante</td>";
  ...
  for($cont=0;$cont<$filas_resultado;$cont++) {
    $v0 = $registro['nro_derecho'];    
    $v1 = $registro['solicitud'];
    $v2 = trim($registro['nombre']);
    $v3 = $registro['fecha_solic'];
    $v4 = $registro['fecha_venc'];
    $v5 = $registro['tipo_derecho'];
    $v6 = $registro['estatus']-2000;
    $v7 = $registro['registro'];
    $respri=pg_exec("SELECT DISTINCT ON (nro_derecho) prioridad
                     FROM stzpriod WHERE nro_derecho='$v0'");
    $regpri=pg_fetch_array($respri);
    $v8 = $regpri['prioridad']; 

    $v9=''; 
    $rescla=pg_exec("SELECT tipo_clas,clasificacion
                     FROM stpclsfd WHERE nro_derecho='$v0' order by tipo_clas,clasificacion");
    $regcla=pg_fetch_array($rescla);
    $filas_resultado2=pg_numrows($rescla);
       for($cont2=0;$cont2<$filas_resultado2;$cont2++) { 
           if ($cont2>0) { $v9=$v9.'; '; }
           $v9=$v9.$regcla['clasificacion'];
	   $regcla = pg_fetch_array($rescla);
	}

    $resbol=pg_exec("SELECT DISTINCT ON (nro_derecho) documento
                     FROM stzevtrd WHERE nro_derecho='$v0' and evento='2124' and estat_ant='2006'");
    $regbol=pg_fetch_array($resbol);
    $v10 = $regbol['documento']; 
    $v11 = trim($registro['ntitular']).", Domicilio: ".trim($registro['domicilio']).", Nacionalidad: ".$registro['nacionalidad'];      
    $v12 = trim($registro['resumen']);
    echo "<tr>";
    echo "<td>$v1</td>";
    echo "<td>$v2</td>";
    echo "<td>$v3</td>"; 
