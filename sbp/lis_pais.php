...
// (Lis_pais.php por Nombre de Pais)
...
$criterio="Criterio: Pa&iacute;s= ".$valor1; 
...
$res_pais=pg_exec("SELECT * FROM stzpaisr WHERE pais='$pais' and pais!=''");
	...
	$resultado=pg_exec("SELECT stzderec.solicitud, stzderec.nombre 
			FROM stzpaisr, stzderec 
			WHERE stzpaisr.nombre = '$valor1' 
	...
	$resultado=pg_exec("SELECT stzderec.solicitud, stzderec.nombre 
			FROM stzpaisr, stzderec 
			WHERE stzpaisr.nombre like '%$valor1%' 
			AND stzderec.pais_resid=stzpaisr.pais
		        AND stzderec.tipo_mp='P' OFFSET $inicio LIMIT $cuanto");
	$cantidad=pg_exec("SELECT stzderec.solicitud, stzderec.nombre 
			FROM stzpaisr, stzderec 
...
<input type="hidden" name="nbpais" value="<?= $_POST['nbpais'] ?>">
