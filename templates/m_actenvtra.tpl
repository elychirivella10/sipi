...
<div align="center">
{if $vopc eq 4}
  <form name="formarcas1" action="m_actenvtra.php?vopc=1" method="post">
{/if} 		  
  ...
  <table>
    <tr>
      ...
      <td class="der-color">
        <input tabindex="1" type="text" name="recibo" size="7" maxlength="8" value='{$recibo}' {$vmodo} onkeyup="checkLength(event,this,8,document.formarcas1.vsol2)" onchange="this.value=this.value.toUpperCase()">
      </td>
    </tr>
    <tr>
      <td class="izq5-color">{$campo5}</td>
      <td class="der-color">
         <input tabindex="2" type="text" name="vsol2" size="7" maxlength="8" value='{$vsol2}' {$vmodo} onKeyPress="return acceptChar(event,2, this)">
         <b>{$campo6}</b>
         <select size='1' name='vbusq'>
           {html_options values=$arraybus selected=$vbusq output=$arraydesbus}
         </select>
      </td>
    </tr>
    <tr>
...
{if $vopc eq 1}
<form name="formarcas2" enctype="multipart/form-data" action="m_actenvtra.php?vopc=2" method="POST" onsubmit='return pregunta();'>
{/if}    
  <input type ='hidden' name='login' value={$login}>
  ...
  <tr>
    ...
    <td class="cnt">
      {if $vopc eq 1 || $vopc eq 4}
         <a href="m_actenvtra.php?vopc=4"><img tabindex="12" src="../imagenes/boton_cancelar_rojo.png" border="0"></a>   
      {/if}    
    </td>      
