...
//(lis_prioridad.php por Numero de Prioridad)
  ...
  switch($modoconsulta)
   {
	...
	$resultado=pg_exec("SELECT *, stzderec.solicitud, stzderec.nombre 
			FROM stzpriod, stzderec 
			WHERE stzpriod.prioridad = '$valor1' 
	 ...
	 $resultado=pg_exec("SELECT *, stzderec.solicitud, stzderec.nombre 
			FROM stzpriod, stzderec 
			WHERE stzpriod.prioridad like '%$valor1%' 
			AND stzderec.nro_derecho=stzpriod.nro_derecho
		        AND stzderec.tipo_mp='P' OFFSET $inicio LIMIT $cuanto ");
	 $cantidad=pg_exec("SELECT *, stzderec.solicitud, stzderec.nombre 
			FROM stzpriod, stzderec 
...
<form method="POST" action="lis_prioridad.php">
