...
// (Lis_tramitante.php por Tramitante de la patente)
// Realizado Por Ing. Rómulo Mendoza
?>
...
// Realizando Consulta por nombre de inventor
$valor1 = $_POST["tramitante"];
...
$criterio="Criterio: Tramitante= ".$valor1; 
  ...
  switch($modoconsulta)
   {
	...
	$resultado=pg_exec("SELECT stzderec.solicitud, stzderec.nombre  
			 FROM stzderec
			 WHERE stzderec.tramitante = '$valor1' 
	...
	$resultado=pg_exec("SELECT stzderec.solicitud, stzderec.nombre  
			 FROM stzderec
			 WHERE stzderec.tramitante like '%$valor1%' 
                         AND stzderec.tipo_mp='P' OFFSET $inicio LIMIT $cuanto");
	$cantidad= pg_exec("SELECT stzderec.solicitud, stzderec.nombre  
			 FROM stzderec
			 WHERE stzderec.tramitante like '%$valor1%' 
...
<input type="hidden" name="tramitante" value="<?= $_POST['tramitante'] ?>">
