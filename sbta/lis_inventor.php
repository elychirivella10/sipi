...
// (Lis_inventor.php por Nombre de Inventor)
...
$criterio="Criterio: Inventor= ".$valor1; 
  ...
  switch($modoconsulta)
   {
	...
	$resultado=pg_exec("SELECT *, stzderec.solicitud, stzderec.nombre 
			FROM stpinved, stzderec 
			WHERE stpinved.nombre_inv = '$valor1' 
	...
	$resultado=pg_exec("SELECT *, stzderec.solicitud, stzderec.nombre 
			FROM stpinved, stzderec 
			WHERE stpinved.nombre_inv like '%$valor1%' 
			AND stzderec.nro_derecho = stpinved.nro_derecho
		        AND stzderec.tipo_mp='P' OFFSET $inicio LIMIT $cuanto");
	$cantidad=pg_exec("SELECT *, stzderec.solicitud, stzderec.nombre 
			FROM stpinved, stzderec 
...
<form method="POST" action="lis_inventor.php">
