...
<?php
// ************************************************************************************* 
// Programa: p_modifica.php 
// Realizado por el Analista de Sistema Romulo Mendoza 
// Coordinación de Informática / Dirección de Soporte Administrativo / SAPI / MICO
// Creado Año: 2009 BD - Relacional 
// *************************************************************************************
...
if ($vopc==1) {
  ...
  if (empty($vsol1) && empty($vsol2)) {
    Mensajenew("No introdujo ningun valor de Expediente ...!!!","javascript:history.back();","N");
  ...
  $tasa      = $objs->tasa;
  $monto     = $objs->monto;
  if (empty($anualidad)) { $anualidad=1; }
  if (empty($planilla)) { $planilla=1; }
  if (empty($tasa)) { $tasa=1; }
  if (empty($monto)) { $monto=1; }
  ...
  if ($vcodage > 0) {
    ...
    $insert_str = "'$varsol',$vcodage,'$objs->nombre','$objs->domicilio','P'";
  ...
  if ($obj_filas > 0) {  
    ...
    for ($contobj=0;$contobj<$obj_filas;$contobj++) {
      ...
      $domi = $objs->domicilio;
      $col_campos = "solicitud,agente,nombre,domicilio,tipo_mp";
      $insert_str = "'$varsol','$vcod','$nomb','$domi','P'";
      $insagen = $sql->insert("$tbname_18","$col_campos","$insert_str","");
      $objs = $sql->objects('',$obj_query); }	  
  }
...
if (($vopc==2) || ($vopc==6)) {
  ...
  if ((empty($planilla)) || (empty($tasa)) || (empty($monto))) {
    Mensajenew("Expediente $varsol sin datos de la primera anualidad ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); } 
  ...
  $obj_filas = $sql->nums('',$obj_query);
  if ($obj_filas==0) {
    Mensajenew("Solicitud $varsol sin Inventores asociados ...!!!","javascript:history.back();","N");
    $smarty->display('pie_pag.tpl'); exit(); }
  ...
  if ($accion==1) {
    ...
    if ($numtitu==0 AND $numcip==0 AND $numprio==0 AND $numagen==0 AND 
      ...
      Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","p_modifica.php?vopc=4&nconexion=".$nconexion."&nveces=".$nveces,"S");
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
