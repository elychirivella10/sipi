...
$encabezado= "Listado con Datos Telefonicos";
...
$resultado=pg_exec("SELECT DISTINCT ON(stzderec.solicitud) stzderec.solicitud, stzderec.fecha_solic, stzderec.nombre, stzderec.estatus, stzsolic.nombre as titular, telefono1,telefono2,email,identificacion, idtramitante,agente,tramitante
		    FROM  stzevtrd, stzottid, stzsolic, stzderec
		    WHERE $where 
...
$header_type = Array();
		...
		$header_type[$i]['TEXT'] = utf8_decode("Correo");
	     	$header_type[$i]['WIDTH'] = 21;
                $i=4;
		$header_type[$i] = $table_default_header_type;
		$header_type[$i]['TEXT'] = utf8_decode("Telefono(s)");
		$header_type[$i]['WIDTH'] = 28;
		$i=5;
		$header_type[$i] = $table_default_header_type;
...
//Dibujando la Tabla para el pdf
 for($cont=0;$cont<$filas_resultado;$cont++) { 
        //Ubicar Agente / Tramitante
	$idagen=$registro['agente'];
	$idtram=$registro['idtramitante'];
	if ($idagen>0) {
   	   $resultado_agen=pg_exec("select nombre,telefono1,telefono2 from stzagenr
                             where agente=$idagen and agente>0 ");
   	   $registro_agen = pg_fetch_array($resultado_agen);
   	   $agentram_nom=trim($registro_agen['nombre']);
   	   $agentram_tel=$registro_agen['telefono1'].' / '.$registro_agen['telefono2'];
	} else {
   	   $resultado_tram=pg_exec("select nombre,telefono1,telefono2 from stztramr
                             where idtramitante=$idtram and idtramitante>0");
   	   $registro_tram = pg_fetch_array($resultado_tram);
   	   $agentram_nom=trim($registro_tram['nombre']);
           if ($agentram_nom==' ' or empty($agentram_nom)) {$agentram_nom=trim($registro['tramitante']);}
   	   $agentram_tel=$registro_tram['telefono1'].' / '.$registro_tram['telefono2'];
	}
	$data = Array();
	$data[0]['TEXT'] = $registro['solicitud'];
	$data[1]['TEXT'] = trim(utf8_decode($registro['nombre']));
//	$data[2]['TEXT'] = $registro['clase'].'-'.$registro['ind_claseni'];
        $data[2]['TEXT'] = trim(utf8_decode($registro['titular']));
   $datostitular='';
   $tel=trim($registro['telefono1']).' / '.trim($registro['telefono2']);
   $ema=trim($registro['email']);
	$data[3]['TEXT'] = $ema;
	$data[4]['TEXT'] = $tel;
	$data[5]['TEXT'] = $agentram_nom;
	$data[6]['TEXT'] = $agentram_tel;
	$registro = pg_fetch_array($resultado);
	$pdf->Draw_Data($data);
  }
