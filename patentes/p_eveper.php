...
function buscarprensa(var1,var2,var3,var4,var5) {
  open("adm_prensap.php?vsol="+var1.value+"-"+var2.value+"&vmod="+var3.value+"&vcod="+var4.value+"&evto="+var5.value,"Ventana", "scrollbars=yes,toolbar=no,directories=no,menubar=no,status=no"); }

 
</script>
<?php
// *************************************************************************************
// Programa: p_eveper.php 
...
if (($usuario!='ggalindez') AND ($usuario!='rmendoza')) {
...
//Carga el tipo de prensa para mostrarlo en el combo
 $blanco='';
 $arraytipo[0]='';
...
if ($esmayor==1) {
   ...
   $fecha = $dia.$mes.$ano;
    ...
    $vnewnombre= "pat_".$tipo."_".$fecha."_".$vpag;
   ...
   for ($cont=0;$cont<$filas_found;$cont++) {     
       ...
       $insert_str = "$vder,2022,'$fechaper',nextval('stzevtrd_secuencial_seq'),2004,'$fechahoy','$usuario','$mensa_automatico',0,'$comentario','$horactual'";
   ...
   for ($cont=0;$cont<$filas_found;$cont++) {     
       ...
       //Inserto Datos en la tabla de Tramite stzevtrd
       $col_campos = "nro_derecho,evento,fecha_event,secuencial,estat_ant,fecha_trans,usuario,desc_evento,documento,comentario,hora";
       $insert_str = "$vder,2023,'$fechaper',nextval('stzevtrd_secuencial_seq'),2005,'$fechahoy','$usuario','$mensa_automatico',0,'$comentario','$horactual'";
       $instram = $sql->insert("$tbname_4","$col_campos","$insert_str",""); 
          
       //Actualizo la maestra en estatus a 11
   ...
   for ($cont=0;$cont<$filas_found;$cont++) {     
       ...
       //Inserto Datos en la tabla de Tramite stzevtrd
       $col_campos = "nro_derecho,evento,fecha_event,secuencial,estat_ant,fecha_trans,usuario,desc_evento,documento,comentario,hora";
       $insert_str = "$vder,2031,'$fechaper',nextval('stzevtrd_secuencial_seq'),2011,'$fechahoy','$usuario','$mensa_automatico',0,'$comentario','$horactual'";
       $instram = $sql->insert("$tbname_4","$col_campos","$insert_str",""); 
          
       //Actualizo la maestra en estatus a 12
   ...
   $resul=pg_exec("SELECT * FROM $tbname_2 WHERE evento='2032'");
   $filas_f = pg_numrows($resul); 
   $regeve  = pg_fetch_array($resul);
   $mensa_automatico=trim($regeve['mensa_automatico']);
   ...
   for ($cont=0;$cont<$filas_found;$cont++) {     
       ...
       //Inserto Datos en la tabla de Tramite stzevtrd
       $col_campos = "nro_derecho,evento,fecha_event,secuencial,estat_ant,fecha_trans,usuario,desc_evento,documento,comentario,hora";
       $insert_str = "$vder,2032,'$fechaper',nextval('stzevtrd_secuencial_seq'), $reg[estatus], '$fechahoy','$usuario','$mensa_automatico',0,'$comentario','$horactual'";
       $instram = $sql->insert("$tbname_4","$col_campos","$insert_str",""); 
          
       //Actualizo la maestra en estatus a 23
       $update_str = "estatus=2023";
       $actestat = $sql->update("$tbname_7","$update_str","nro_derecho=$vder");
   ...
   $resul=pg_exec("SELECT * FROM $tbname_2 WHERE evento='2033'");
   $filas_f = pg_numrows($resul); 
   $regeve  = pg_fetch_array($resul);
   $mensa_automatico=trim($regeve['mensa_automatico']);
   ...
   for ($cont=0;$cont<$filas_found;$cont++) {     
       ...
       //Inserto Datos en la tabla de Tramite stzevtrd
       $col_campos = "nro_derecho,evento,fecha_event,secuencial,estat_ant,fecha_trans,usuario,desc_evento,documento,comentario,hora";
       $insert_str = "$vder,2033,'$fechaper',nextval('stzevtrd_secuencial_seq'),$reg[estatus], '$fechahoy','$usuario','$mensa_automatico',0,'$comentario','$horactual'";
       $instram = $sql->insert("$tbname_4","$col_campos","$insert_str",""); 
          
       //Actualizo la maestra en estatus a 24
       $update_str = "estatus=2024";
       $actestat = $sql->update("$tbname_7","$update_str","nro_derecho=$vder");
   ...
   if ($errorgrabar == 0) {    //Validacion del Numero de Solicitud
         ...
         $sql->disconnect();
         Mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','p_eveper.php?vopc=4','S');
         $smarty->display('pie_pag.tpl'); exit();
   }
   else {
 ...
 } //fin del else que verifica si grabo solicitudes 
