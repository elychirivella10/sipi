...
<?php
// *************************************************************************************
// Programa: z_fileboletin_opp.php 
...
$smarty->assign('subtitulo','ENVIO DE ARCHIVOS PARA MENSAJES DE TEXTOS ASOCIADOS AL BOLETIN'.$fechapub);
...
if ($vopc==2) {
 ...
 $ejecute1 = pg_exec("select '414' as t1, '3115457' as telefono, concat('SAPI INFORMA: SU SOLICITUD 0000-000000 FUE NOTIFICADA CON ORDEN DE PUBLICACION EN PRENSA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
UNION
select '412' as t1, '0259949' as telefono, concat('SAPI INFORMA: SU SOLICITUD 0000-000000 FUE NOTIFICADA CON ORDEN DE PUBLICACION EN PRENSA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
UNION
SELECT substr(trim(telefono1),2,3) as t1, substr(trim(telefono1),6,7) as telefono, concat('SAPI INFORMA: SU SOLICITUD ',solicitud,' FUE NOTIFICADA CON ORDEN DE PUBLICACION EN PRENSA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
 FROM stzderec a, stzevtrd b, stzottid c, stzsolic d
	WHERE a.nro_derecho=b.nro_derecho and a.tipo_mp='P' and
              ...
              b.evento=2201 and b.estat_ant=2002 and
              b.documento=$boletin and
              trim(d.telefono1)<>'' and substr(trim(d.telefono1),1,4) in ('0414','0424','0416','0426','0412')
UNION
SELECT substr(trim(telefono2),2,3) as t1, substr(trim(telefono2),6,7) as telefono, concat('SAPI INFORMA: SU SOLICITUD ',solicitud,' FUE NOTIFICADA CON ORDEN DE PUBLICACION EN PRENSA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
 FROM stzderec a, stzevtrd b, stzottid c, stzsolic d
	WHERE a.nro_derecho=b.nro_derecho and a.tipo_mp='P' and
              a.nro_derecho=c.nro_derecho and c.titular=d.titular and c.titular>0 and
              b.evento=2201 and b.estat_ant=2002 and
...
SELECT substr(trim(telefono2),2,3) as t1, substr(trim(telefono2),6,7) as telefono, concat('SAPI INFORMA: SU SOLICITUD ',solicitud,' FUE NOTIFICADA CON ORDEN DE PUBLICACION EN PRENSA EN EL BOLETIN ',$boletin,'. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
 FROM stzderec a, stzevtrd b, stzagenr d
	WHERE a.nro_derecho=b.nro_derecho and a.tipo_mp='P' and
              ...
              trim(d.telefono2)<>'' and substr(trim(d.telefono2),1,4) in ('0414','0424','0416','0426','0412')
UNION
SELECT substr(trim(telefono1),2,3) as t1, substr(trim(telefono1),6,7) as telefono, concat('SAPI INFORMA: EL LAPSO PARA PAGAR LA PUBLICACION EN PRENSA DE SU SOLICITUD Nro. ',solicitud,' VENCE EL PROXIMO ','$fechapub','. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
 FROM stzderec a, stzevtrd b, stzottid c, stzsolic d
	WHERE a.nro_derecho=b.nro_derecho and a.tipo_mp='P' and
              a.nro_derecho=c.nro_derecho and c.titular=d.titular and c.titular>0 and
              b.evento=2201 and b.estat_ant=2002 and
              ...
              trim(d.telefono1)<>'' and substr(trim(d.telefono1),1,4) in ('0414','0424','0416','0426','0412')
UNION
SELECT substr(trim(telefono2),2,3) as t1, substr(trim(telefono2),6,7) as telefono, concat('SAPI INFORMA: EL LAPSO PARA PAGAR LA PUBLICACION EN PRENSA DE SU SOLICITUD Nro. ',solicitud,' VENCE EL PROXIMO ','$fechapub','. PARA MAYOR INFORMACION INGRESE EN WEBPI.') as mensaje
 FROM stzderec a, stzevtrd b, stzottid c, stzsolic d
	WHERE a.nro_derecho=b.nro_derecho and a.tipo_mp='P' and
              a.nro_derecho=c.nro_derecho and c.titular=d.titular and c.titular>0 and
              b.evento=2201 and b.estat_ant=2002 and
...
//Crea el archivo
$vruta="../../sms/";
$txt=$vruta.'Boletin'.$boletin.'_sms_prensa_p.txt';
$open=fopen($vruta.'Boletin'.$boletin.'_sms_prensa_p.txt',"w+");
...
fclose($open);  
 ...
 $adjuntos[$count+1]['filename']=$vruta.'Boletin'.$boletin.'_sms_prensa_p.txt';
...
$smarty->display('z_fileboletin_oppp.tpl');
$smarty->display('pie_pag.tpl');
...
// $mail->Subject = "Envio de Solicitud(es) de Busqueda(s) en Linea, Tramite WEBPI No. ".$tramite;
 ...
 $mail->Subject = "SAPI - Mensajes de Texto - Boletin ".$boletin." ORDEN PUB. PRENSA";
// $mail->AddAddress($vemail,$nombre);
 $mail->AddAddress('nelson.gonzalez@sapi.gob.ve','Nelson Gonzalez');
// $mail->AddBCC('aaron.noguera@sapi.gob.ve','Aaron Noguera');
...
//  $mail->AddAttachment($filetxt);
 ...
 $body .= "Anexo al presente, se hace entrega de archivo plano con los datos de las solicitudes de patentes publicadas con ORDEN DE PUBLICACION EN PRENSA ";
