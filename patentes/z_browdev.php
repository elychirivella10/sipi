...
if (empty($fecsold) and empty($fecsolh)) {
  mensajenew("ERROR: Hay Informacion Vacia ...!!!","javascript:history.back();","N");
  $smarty->display('pie_pag.tpl'); exit(); 
}

 if ($vopc==1) {
   //Creando tabla temporal
   //pg_exec("CREATE TEMPORARY TABLE $tablas ( solicitud char(11) NOT NULL,registro char(7),nombre char(200),tipo_derecho char(1), evento int2 NOT NULL,fecha_event date NOT NULL,documento int4,comentario char(300),usuario char(10))");
   pg_exec("DELETE FROM $tablas WHERE fecha_carga < '$fechahoy'");
   $resultado=pg_exec("INSERT INTO $tablas SELECT stzderec.solicitud,stzderec.registro,stzderec.nombre,stzevtrd.evento,stzevtrd.fecha_event,stzevtrd.documento,stzevtrd.comentario,stzevtrd.usuario,stzevtrd.fecha_trans,'$fechahoy' FROM stzevtrd,stzderec WHERE (stzevtrd.evento=2502 AND stzevtrd.nro_derecho=stzderec.nro_derecho AND (stzevtrd.fecha_trans between '$fecsold' AND '$fecsolh'))");
 ...
 //$resultado=pg_exec("INSERT INTO $tablas SELECT stmmarce.solicitud,registro,nombre,clase,ind_claseni,evento,fecha_event,documento,comentario FROM stmevtrd,stmmarce WHERE (stmevtrd.evento=502 and stmevtrd.solicitud=stmmarce.solicitud and (stmevtrd.fecha_trans between '$fecsold' and '$fecsolh'))");

 $condicion = " stptmpam.usuario='$vusua' AND (stptmpam.fecha_trans between '$fecsold' AND '$fecsolh')";
 ...
 //$query='SELECT DISTINCT '.$camposquery.' FROM '.$tablas.' ORDER BY '.$orden;
 ...
 if ($filas_resultado ==0) {
   mensajenew("ERROR: No Hay Informacion encontrada ...!!!","javascript:history.back();","N");
   $smarty->display('pie_pag.tpl'); exit(); 
 }
 ...
 if ($vopc==5) {
   ...
   for($cont=1;$cont<=$total;$cont++) { 
     ...
     $vdoc = $reg[documento];
     if ($vtmp=='on') {
       $insert_val = "$vorden,'$vsolh',$vdoc,'N','P','$fechahoy'";
       $sql->insert("stzofdev","","$insert_val","");
     }
     $reg=pg_fetch_array($cantidad);
   }
   Mensaje3("DATOS GUARDADOS CORRECTAMENTE con la Orden $vorden ...!!!","$vurl","p_ofidevreg.php?orden=$vorden","");
   ...
   for($cont=0;$cont<$filas_resultado;$cont++) {
      ...
      for($cont2=0;$cont2<$cantcampos;$cont2++) {
          ...
          switch ($namecampo) {
            case "Solicitud":
              ...
              $nameimagen="../imagenes/patentes/di".$dirano."/".$vs1.$vs2.".jpg"; 
              break;
            case "Nombre":
              $vname = trim(substr($reg['nombre'],0,25));
              break;
            case "Evento":
              $veven = $reg[evento]-2000;
              break;
            case "Comentario":
              $vcomenta = trim($reg[comentario]); 
              break;
          }
      } 
      ...
      echo "</td>";
      echo "<td bgcolor='#CCCCCC' bordercolorlight='#FFFFFF' align='center' width='6%'><font color='#000000'>$veven</font>"; 
      echo "</td>";
      ...
      echo "</td>";
      echo "<td bgcolor='#CCCCCC' bordercolorlight='#FFFFFF' align='center' width='30%'><font color='#000000'>$vcomenta</font>"; 
      echo "</td>";
      echo "</tr>";
      $reg = pg_fetch_array($resultado);
   }
   ...
   <input type='submit' name='fmodo' class='boton_blue' value='<?=$modo?>' /> 
   <? if ($new_windows=='S') { ?>
      <input type="button" img="../imagenes/salir_f2.png" class="boton_blue" value="Salir" onclick="cerrarwindows();"> </font></p> 

   <? } else { ?>  
      <input type="button" img="../imagenes/salir_f2.png" class="boton_blue" value="Salir" onclick="location.href='<?=$vurl?>'"> </font></p> 
   <? } ?>
   ...
   if($inicio > 0) {
     ?>
     <input type="image" src="../imagenes/back_f2.png" class='boton_blue' name='fmodo' value="Anteriores <?= min($inicio, $cuanto) ?> " /> Anteriores <?= min($inicio, $cuanto) ?>   
     <? }
   if($total - $inicio > $cuanto) {
     ?>
     <input type='image' src="../imagenes/next_f2.png" class='boton_blue' name='fmodo' value='Siguientes <?= min(($total - ($inicio + $cuanto)), $cuanto)?>' /> Siguientes <?= min(($total - ($inicio + $cuanto)), $cuanto)?>
