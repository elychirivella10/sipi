...
$resultadosum=pg_exec("select count(*) from stzderec ".$ptabla." where tipo_mp='P' ".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4);

//echo "select count(*) from stzderec ".$ptabla." where tipo_mp='P' ".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4; exit();
...
if ($vano1==$vano2 and $vano1!='') {
   $resultado=pg_exec("select EXTRACT(MONTH FROM fecha_solic) as mes, count(*) as cant 
            ...
            where tipo_mp='P' ".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4.
  ...
  for($cont=$vano1;$cont<=$vano2;$cont++)   {
   $resultado=pg_exec("select count(*) as cant from stzderec ".$ptabla." 
                 ...
                 tipo_mp='P' ".$pquery1.$pquery2.$pquery3.$pquery4);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0]; 
   if ($cont==$vano1) {
    ...
    $resultado=pg_exec("select count(*) as cant from stzderec ".$ptabla." where tipo_mp='P' ".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0];    
   $datay=array($cant_reg);
...
$graph->img->SetMargin(40,30,60,30);
...
// Setup the titles
if ($vano1==$vano2 and $vano1!='') {$graph->title->Set("PATENTES PRESENTADAS EN EL ANO ".$vano1.$ptit."\n \n ");}
if ($vano1==$vano2 and $vano1=='') {$graph->title->Set("PATENTES PRESENTADAS ".$ptit."\n \n ");}
if ($vano1<>$vano2) {$graph->title->Set("PATENTES PRESENTADAS POR ANO ".$ptit."\n \n ");}
...
<tr><td class="cnt"><a href="p_est_prexmes.php?vopc=0"><img src="../imagenes/boton_salir_rojo.png" border="0"></a></td></tr></table><?php
...
$smarty->display('p_est_prexmes.tpl');
$smarty->display('pie_pag.tpl');
}
