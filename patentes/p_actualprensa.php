...
$smarty->assign('subtitulo','Actualizaci&oacute;n de Solicitudes Publicadas en Prensa Digital Patentes');
$smarty->assign('login',$usuario);
$smarty->assign('fechahoy',$fecha);
$smarty->display('encabezado1.tpl');

if (($usuario!='resanchez') AND ($usuario!='ggalindez') AND ($usuario!='dmoreno') AND ($usuario!='mrocha') AND ($usuario!='jperich') AND ($usuario!='chernandez') AND ($usuario!='rmendoza') AND ($usuario!='ngonzalez')) {
  mensajenew('AVISO: Usuario NO Autorizado para usar esta Opci&oacute;n del Sistema ...!!!','javascript:history.back();','N');
  $smarty->display('pie_pag.tpl'); exit();
}

  //if (($usuario!='lchacon') || ($usuario!='saissami') || ($usuario!='cjarquin') || ($usuario!='yalbano') || ($usuario!=='lcastellano') || 
...
$smarty->display('p_actualprensa.tpl');
