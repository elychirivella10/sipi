...
if ($vopc==1) {
  $smarty->assign('subtitulo','B&uacute;squeda Externa de Dise&ntilde;o Industrial'); 
  $smarty->assign('modo1',''); 
  $smarty->assign('modo3',''); 
...
$manphp = vman_php("m_pbexdise.php");
if ($manphp==1) {
...
if ($vopc==1) {
  ...
  if (empty($vsol1)) {
     mensajenew('No introdujo ningún valor de Pedido ...!!!','p_pbexdise.php?vopc=5','N');
  ...
  if ($existe_img==0) {
    mensajenew('ERROR: NO EXISTE la Imagen o Logotipo asociado al pedido ...!!!','p_pbexdise.php?vopc=5','N');
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit();	 
  }
  ...
  if (!$resultado) { 
     mensajenew('ERROR AL PROCESAR LA BUSQUEDA ...!!!','p_pbexdise.php?vopc=5','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
  $filas_found=pg_numrows($resultado); 
  if ($filas_found==0) {
     mensajenew('NO EXISTEN DATOS ASOCIADOS ...!!!','p_pbexdise.php?vopc=5','N');
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }	 
...
$smarty->assign('campo6','Locarno:');
$smarty->assign('campo7','Logotipo:');
$smarty->assign('campo8','Cod. Viena:');
$smarty->assign('lcviena','C&oacute;digos de Locarno '); 
