...
<?php
// ************************************************************************************* 
// Programa: p_modregis.php 
// Realizado por el Analista de Sistema Romulo Mendoza 
// Coordinación de Informática / Dirección de Soporte Administrativo / SAPI / MICO
// Creado Año 2013 II Semestre 
// *************************************************************************************
...
if (($usuario=='ggalindez') || ($usuario=='ftorrico') || ($usuario=='jgil') || ($usuario=='fastudillo') || ($usuario=='breyes') || ($usuario=='xamaris') || ($usuario=='pbravo') || ($usuario=='rmendoza') || ($usuario=='ngonzalez')) { }
else {
  Mensajenew("ERROR: Usuario NO Autorizado para el Uso de esta Pantalla ...!!!","p_infosol.php?vopc=6","N");
...
if ($vopc==1) {
  ...
  if (!empty($varreg)) {
     $resultado=pg_exec("SELECT * FROM $tbname_6 WHERE registro='$varreg' AND tipo_mp='P'"); }
  else {
  ...
  $estatus=$reg[estatus];
  if (($estatus==2001) || ($estatus==2002) || ($estatus==2004) || ($estatus==2202) || ($estatus==2006) || ($estatus==2025) || ($estatus==2200) || ($estatus==2918) || ($estatus==2919)) { }
  else {
    Mensajenew("ERROR: Solicitud en estatus NO Modificable ...!!!","p_infosol.php?vopc=6","N");
    $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
  }
  ...
  $sql->del("$tbname_9","solicitud='$varsol' AND tipo_mp='P'");
  ...
  for ($contobj=0;$contobj<$obj_filas;$contobj++) {
     ...
     $insert_str = "'$varsol','$vcod','$nomb','$domi','$pais','$indo','P'";
...
if (($vopc==2) || ($vopc==6)) {
  ...
  if ($accion==2) {
    ...
    mensajenew("ERROR: N&uacute;mero de Solicitud Vacio o con Error ...!!!","javascript:history.back();","N");
  ...
  if ($dirano != $annno) { 
    mensagenew("ERROR: Los Cuatros primeros digitos del Expediente no son iguales a los cuatros ultimos de su Fecha ...!!!","N");
    $smarty->display('pie_pag.tpl'); exit();  }
  ...
  if ($tipo_paten=='N') {  
    mensagenew("ERROR: No ha seleccionado el Tipo de Patente ...!!!","N");
    $smarty->display('pie_pag.tpl'); exit();  }
  ...
  if ($tramitante!="") {
    if (($vpoder!="") || ($vcodage!="0")) {
      mensajenew("ERROR: Solo puede tener Tramitante o Poder o Agente(s) ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); exit(); } }

  //$obj_query = $sql->query("SELECT * FROM $tbname_11 WHERE solicitud ='$varsol'");
  //$obj_inven = $sql->nums('',$obj_query);
  //if ($obj_inven==0) {
  //  Mensajenew("ERROR: Solicitud $varsol sin Inventores asociados ...!!!","javascript:history.back();","N");
  //  $smarty->display('pie_pag.tpl'); exit(); }
  ...
  if ($accion==1) {
    ...
    //     $insert_str = "'$vder','$act_titular','$regtitu[nacionalidad]','$regtitu[domicilio]'";
    ...
    $reg_tit=pg_exec("SELECT * FROM $tbname_5 WHERE nro_derecho='$vder' AND evento IN (2204,2205,2206,2208,2209)");
    ...
    if ($filas_reg_eve==0) {
      ...
      { 
        ...
        if ($filas_titular!=0) { 
          ...
          $update_str2 = "domicilio='$vdom',nacionalidad='$vnac'";         
    ...
    if ($numtitu==0 AND $numcip==0 AND $numprio==0 AND $numagen==0 AND 
      ...
      Mensajenew("DATOS GUARDADOS CORRECTAMENTE ...!!!","p_modregis.php?vopc=4","S");
      $smarty->display('pie_pag.tpl'); exit();
    } 
    else {
      ...
      Mensajenew("Falla de Actualizaci&oacute;n / Inserci&oacute;n de Datos en la BD $error_anu, $error_pat, $error_der, $error_tit, $error_inv, $error_pri, $error_age, $error_cip ...!!!","javascript:history.back();","N");
      $smarty->display('pie_pag.tpl'); exit();
    }
