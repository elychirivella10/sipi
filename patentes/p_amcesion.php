...
$smarty ->assign('fechahoy',$fecha); 
   ...
   if ($vopc==1 || $vopc==2) {
      ...
      if (!$resultado) {
         ...
         mensajenew('ERROR AL PROCESAR LA BUSQUEDA','p_amcesion.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      ...
      if ($filas_found==0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('NO EXISTEN DATOS ASOCIADOS','p_amcesion.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      ...
      if ($vreg!='') { //Esta bien 
         ...
         $sql->disconnect(); 
         $smarty->display('encabezado1.tpl');
         mensajenew('Solo aplica a PATENTES con REGISTRO ASIGNADO','p_amcesion.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
   ...
   if ($vopc==3) {
	 ...
	 $smarty->display('pie_pag.tpl'); exit(); 
      ...
      $restmp=pg_exec("select * from temptitu where solicitud='$vsolh' and tipo_mp='P'");
      $numtmp=pg_numrows($restmp);
      if ($numtmp==0) {
         pg_exec("ROLLBACK WORK"); 
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('Error! No existe(n) Titular(es) Final(es). Transaccion Abortada...',
                    'javascript:history.back();','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
       
      if ($vtipo==1) {$vcodeven=2205; $vdeseven='SOLICITUD DE CESION';}
      if ($vtipo==2) {$vcodeven=2206; $vdeseven='SOLICITUD DE FUSION';}
      if ($vtipo==3) {$vcodeven=2209; $vdeseven='SOLICITUD DE CAMBIO DEL TITULAR';}
      ...
      {  $temptit=$regco[titular];
         ...
         if ($tempdom=='' or $tempnac=='') {
            $sql->disconnect();
            $smarty->display('encabezado1.tpl');
            mensajenew('ERROR - Falto ingresar Nacionalidad o Domicilio del Titular Final...', 
                       'p_amcamdom.php','N');
            $smarty->display('pie_pag.tpl'); exit(); 
         }
         ...
         $insert_valores ="$vderh,$vdoc,'$vsolh','P',$vcodeven,$temptit,'$tempnom',
      ...
      if ($cant_error==0) {
           ...
           $smarty->display('encabezado1.tpl');
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','p_amcesion.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
