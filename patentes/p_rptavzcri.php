...
if(!empty($fecpubd) and !empty($fecpubh)) { 
	if(!empty($where)) {
	   ...
	   $titulo= $titulo." Fecha Pub.:"." $fecpubd"." Hasta:"." $fecpubh";
	}
	else { 
...
if(!empty($titular)) { 
	if(!empty($where)) {
	   ...
	   $where = $where." (stzsolic.titular = '$titular'
				   AND stzottid.titular=stztitur.titular
         		AND stzderec.nro_derecho=stmottid.nro_derecho )";
           $titulo= $titulo." Titular:"." $titular";
	}
}
if(!empty($agente)) { 
...
// Armando el query
$resultado=pg_exec("SELECT stzderec.solicitud,stzderec.fecha_solic,stzderec.tipo_derecho, stzderec.nombre, stzderec.estatus,stzderec.registro,stzderec.fecha_regis,stzderec.fecha_venc, stzderec.tramitante
