...
$smarty->assign('subtitulo','Estad&iacute;sticas - Examinadas Fondo/Forma');
...
if ($dif>15) {
    $sql->disconnect(); 
    mensajenew('ERROR: El rango no puede ser mayor a 15 a&ntilde;os...!!!',
               'javascript:history.back();','N');    
    $smarty->display('pie_pag.tpl'); exit(); }

$ptit=''; $ve='VE'; $ptabla='a, stzevtrd b ';
...
if ($vclas=='') { $pquery1=''; }
      ...
      $ptabla=' a, stzevtrd b, stpclsfd c ';}
...
if ($vano1=='' and $vano2=='') {
   ...
   $resultadosum=pg_exec("select count(*) from stzderec ".$ptabla." where tipo_mp='P' and a.nro_derecho=b.nro_derecho 
                                 and evento in (2028,2029,2051,2052,2053,2056,2059)".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4);
...
if ($vano1==$vano2 and $vano1!='') {
   $resultado=pg_exec("select EXTRACT(MONTH FROM fecha_trans) as mes, count(*) as cant 
            from stzderec ".$ptabla."
            where tipo_mp='P' and a.nro_derecho=b.nro_derecho 
                  and evento in (2028,2029,2051,2052,2053,2056,2059)".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4.
  ...
  for($cont=$vano1;$cont<=$vano2;$cont++)   {
   $resultado=pg_exec("select count(*) as cant from stzderec ".$ptabla." 
                 where EXTRACT(YEAR FROM fecha_trans)= '$cont' and 
                 tipo_mp='P' and a.nro_derecho=b.nro_derecho 
                 and evento in (2028,2029,2051,2052,2053,2056,2059)".$pquery1.$pquery2.$pquery3.$pquery4);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0]; 
   if ($cont==$vano1) {
    ...
    $resultado=pg_exec("select count(*) as cant from stzderec ".$ptabla." where tipo_mp='P' and a.nro_derecho=b.nro_derecho 
                               and evento in (2028,2029,2051,2052,2053,2056,2059) ".$pquery0.$pquery1.$pquery2.$pquery3.$pquery4);
   $registro=pg_fetch_array($resultado);
   $cant_reg=$registro[0];    
   $datay=array($cant_reg);
...
// Setup the titles
if ($vano1==$vano2 and $vano1!='') {$graph->title->Set("PATENTES EXAMINADAS FONDO/FORMA EN EL ANO ".$vano1.$ptit."\n \n ");}
if ($vano1==$vano2 and $vano1=='') {$graph->title->Set("PATENTES EXAMINADAS FONDO/FORMA ".$ptit."\n \n ");}
if ($vano1<>$vano2) {$graph->title->Set("PATENTES EXAMINADAS FONDO/FORMA POR ANO ".$ptit."\n \n ");}
...
<tr><td class="cnt"><a href="p_est_exaxmes.php?vopc=0"><img src="../imagenes/boton_salir_rojo.png" border="0"></a></td></tr></table><?php
...
$smarty->display('p_est_exaxmes.tpl');
$smarty->display('pie_pag.tpl');
}
