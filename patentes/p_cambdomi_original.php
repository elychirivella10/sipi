...
$vuser    = $usuario;  
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
         ...
         mensaje('ERROR AL PROCESAR LA BUSQUEDA','p_cambdomi.php');
	 $smarty->display('pie_pag.tpl'); exit(); 
      ...
      if ($filas_found==0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensaje('NO EXISTEN DATOS ASOCIADOS','p_cambdomi.php');
	 $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($vreg<>'       ' and $vreg<>null ) {
         ...
         mensajenew('Solicitud con Registro Asignado. 
                     Corresponde a una Anotacion Marginal','p_cambdomi.php','N');
   ...
   if ($vopc==3) {     
	 ...
	 $smarty->display('pie_pag.tpl'); exit(); 
       ...
       if ($cantfil>0) {
         $sql->disconnect();
         $smarty->display('encabezado1.tpl');
         mensajenew('ERROR - El Evento ya fue Cargado','p_cambdomi.php','N');
	 $smarty->display('pie_pag.tpl'); exit(); 
       }
	 ...
	 $smarty->display('pie_pag.tpl'); exit(); 
       ...
       $resule=pg_exec("select * from stzevtrd where nro_derecho='$vderh' and evento=2010 and 
                       documento='$vdoc'");
       $rege = pg_fetch_array($resule);
       $cantfil = pg_numrows($resule);
       if ($cantfil>0) {
	 ...
	 $smarty->display('pie_pag.tpl'); exit(); 
      ...
      $insert_campos="nro_derecho,evento,fecha_event,secuencial,estat_ant,documento,fecha_trans,
                      usuario,desc_evento,comentario,hora";
      $insert_valores ="'$vderh','2010','$vfevh',nextval('stzevtrd_secuencial_seq'),$vest+2000,
      ...
      if ($cant_error==0) {
           ...
           $smarty->display('encabezado1.tpl');
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','p_cambdomi.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
