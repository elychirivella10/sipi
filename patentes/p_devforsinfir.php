...
$smarty->assign('subtitulo','Generaci&oacute;n de Oficios de Devoluci&oacute;n Patentes x Bolet&iacute;n sin Firma Digital');
...
if ($vopc==2) {
 ...
 if ((empty($boletin)) && (empty($vsol1)) && (empty($vsol2)) && (empty($vsol3)) && (empty($vsol4))) {
   mensajenew('ERROR: Debe introducir un Criterio de B&uacute;squeda ...!!!','../patentes/p_devforsinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 }

 if ($boletin < 613) {
   mensajenew('ERROR: El Boletin a generar y firmar NO puede ser menor al 615 ...!!!','../patentes/p_devforsinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 }

 if ((!empty($vsol1)) && (!empty($vsol2)) && ((empty($vsol3)) || (empty($vsol4)))) {
   mensajenew('ERROR: Debe introducir el rango final de Solicitud para proseguir ...!!!','../patentes/p_devforsinfir.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
 } 

 if ((!empty($vsol1)) && (!empty($vsol2)) && (!empty($vsol3)) && (!empty($vsol4))) {
   if (empty($boletin)) {
     mensajenew('ERROR: Debe introducir el Numero de Boletin para proseguir ...!!!','../patentes/p_devforsinfir.php?vopc=1','N');   
     $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
   } 
   else {
     ...
     if ($vsoli > $vsolf) {  
       $smarty->display('encabezado1.tpl');
       mensajenew('ERROR: Rango de solicitudes erroneo ...!!!','../patentes/p_devforsinfir.php?vopc=1','N');    
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); 
     }
...
//       AND c.documento='$boletin'
     ...
     if (!$resultado) { 
       mensajenew('ERROR: Problema al Procesar la Busqueda ...!!!','../patentes/p_devforsinfir.php?vopc=1','N');   
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
...
//       AND c.documento='$boletin'
 ...
 //$cmd="scp -P 3535 /var/www/apl/sipi/documentos/devueltas/patentes/*.pdf www-data@172.16.0.130:/var/www/apl/multimedia/patentes/devueltas/"; 
 //exec($cmd,$salida);
 //foreach($salida as $line) { 
 //echo "Holaa<br>";	
 //echo "$line<br>"; }

 $cmd="scp -P 3535 /var/www/apl/sipi/documentos/devueltas/patentes/forma/boletin$boletin/*.pdf www-data@172.16.0.7:/var/www/apl/webpi/documentos/devolucion/patentes/forma/boletin$boletin"; 
 exec($cmd,$salida);
   ...
   echo "   </tr>";
   echo "</table>";
   echo "<p align='center'><a href='../patentes/p_devforsinfir.php?vopc=1'><input type='image' name='continuar' value='Continuar' onclick='window.close()' src='../imagenes/apply_f2.png' alt='Continuar' align='middle' border='0' />Continuar</a></p>";
   echo "<br>";
...
$smarty->display('p_devforsinfir.tpl');
$smarty->display('pie_pag.tpl');
}
$sql->disconnect();
?>
