...
$vuser     = $usuario;  
   ...
   if ($vopc==1) {
      ...
      if (!$resultado) { 
         $sql->disconnect();
         mensaje('ERROR: PROBLEMA AL PROCESAR LA BUSQUEDA','p_cambtitu.php');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }	 
      $filas_found=pg_numrows($resultado); 
      if ($filas_found==0) {
         $sql->disconnect();
         mensaje('ERROR: NO EXISTEN DATOS ASOCIADOS','p_cambtitu.php');
	      $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($vreg<>'       ' and $vreg<>null ) {
         mensajenew('AVISO: Solicitud con Registro Asignado. Corresponde a una Anotacion Marginal','p_cambtitu.php','N');
   ...
   if ($vopc==3) {     
      ...
      if ($objfilas==0) {
        mensajenew('ADVERTENCIA: Factura NO existe en la Base de Datos del SISFAC ...!!!','p_cambtitu.php','N');
        $smarty->display('pie_pag.tpl'); exit(); }
      ...
      if ($objtotdtalle==0) {
        mensajenew('ADVERTENCIA: Factura NO presenta ning&uacute;n Servicio de Cambio de Peticionario ...!!!','p_cambtitu.php','N');
        $smarty->display('pie_pag.tpl'); exit(); 
      }
      ...
      if ($vcant_am>=$canservi) {
        ...
        mensajenew('ADVERTENCIA: Factura '.$nfactura.' YA asociada a la cantidad de Solicitud(es) permitidas o indicadas...!!!','p_cambtitu.php','N');
        $smarty->display('pie_pag.tpl'); exit();
      } 
      ...
      if ($esmayor==1) {
       ...
       // Inserta en Stzevtrd
       // Primer evento: 165
       $resule=pg_exec("select * from stzevtrd where nro_derecho='$vderh' and evento=2165 and 
                        documento='$vdoc'");
       $rege = pg_fetch_array($resule);
       $cantfil = pg_numrows($resule);
       if ($cantfil>0) {
         $sql->disconnect();
         mensajenew('ADVERTENCIA: El Evento ya fue Cargado','p_cambtitu.php','N');
	      $smarty->display('pie_pag.tpl'); exit(); 
       }
       ...
       // Segundo evento: 234
       $resule=pg_exec("select * from stzevtrd where nro_derecho='$vderh' and evento=2234 and 
                       documento='$vdoc'");
       $rege = pg_fetch_array($resule);
       $cantfil = pg_numrows($resule);
       if ($cantfil>0) {
      ...
      for ($contobj=0;$contobj<$numo;$contobj++) {
          ...
          {  pg_exec("ROLLBACK WORK");
             $sql->disconnect();
             mensajenew('ERROR... Domicilio del Titular vacio!  Transaccion Abortada!', 
                        'p_cambtitu.php','N');
             $smarty->display('pie_pag.tpl'); exit();
          }
      ...
      if ($cant_error==0) {
           ...
           mensajenew('DATOS GUARDADOS CORRECTAMENTE!!!','p_cambtitu.php','S');
           $smarty->display('pie_pag.tpl'); exit();   
      } else {
