...
if(!empty($boletin1)) { 
	if(!empty($where)) {
	   $where = $where." and"." (stzevtrd.documento >= '$boletin1' and stzevtrd.documento <= '$boletin2')";
  	   $titulo= $titulo." Boletín:"."$boletin1"." al "."$boletin2";
	}
	else { 
...
if(!empty($palabras)) { 
   $atecnica = explode(",", $palabras);
      
   //saco el numero de elementos
   $longitud = count($atecnica);
   ...
   for($i=0; $i<$longitud; $i++) {
        ...
        if (empty($area)) {
	       ...
	       $area= $area.",'".$atecnica[$i]."'";
        }
   }   
   $area = strtoupper($area);
   
   $tablas = $tablas.",stptesar, stppacld ";
   $rjoin = $rjoin." AND (stzderec.nro_derecho = stppacld.nro_derecho) AND (stptesar.apuntador = stppacld.apuntador) ";  
	if(!empty($where)) {
	   //$where = $where." AND "." (stptesar.palabra ilike '%$palabras%')";
	   $where = $where." AND "." (stptesar.palabra IN (".$area.")) ";
  	   $titulo= $titulo." Area Técnica:"."$palabras";
	}
	else { 
		//$where = $where." (stptesar.palabra ilike '%$palabras%') ";
		$where = $where." (stptesar.palabra IN (".$area.")) ";
  	   $titulo= $titulo." Area Técnica:"."$palabras";
	}
}
