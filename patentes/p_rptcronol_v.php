...
//Inventores	 
	 for($cont_inv=0;$cont_inv<$filas_cons_inv;$cont_inv++) { 
            $inv= $inv.trim($reg_inv['nombre_inv']).'; ';
   	    $reg_inv = pg_fetch_array($cons_inv);
	}
         $pdf->ln(2); 
...
// Buscando los titulares de la solicitud
	...
	//Table Header
		...
		$aSimpleHeader[$i]['WIDTH'] = 75;
                $i=2;
		$aSimpleHeader[$i] = $table_default_header_type;
		$aSimpleHeader[$i]['TEXT'] = "Nacionalidad ";
...
$filas_found_tit=pg_numrows($result);
	...
	for ($j=0; $j<$filas_found_tit; $j++)
	{
		...
		$data[1]['TEXT'] = trim($res['nombre']);
		$pais_nombre=pais($res['nacionalidad']);
		$data[2]['TEXT'] = trim($res['nacionalidad']."-".$pais_nombre);
		$data[3]['TEXT'] = trim($res['domicilio']);
		$pdf->tbDrawData($data);
