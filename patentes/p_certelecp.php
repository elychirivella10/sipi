...
if((!empty($registrod)) and ($registrod!= '000000')) {  
   $resultado=pg_exec("SELECT nro_derecho,solicitud FROM stzderec 
                       WHERE tipo_mp='P' 
		         AND estatus=2555
...
if((!empty($varsol)) and ($varsol != '000000')) {  
   $resultado=pg_exec("SELECT nro_derecho,solicitud FROM stzderec 
                       WHERE tipo_mp='P' 
		         AND estatus=2555
		         AND solicitud='$varsol'");
}

//verificando los resultados
if (!$resultado) { 
     $smarty->display('encabezado1.tpl');
     mensajenew('Error al Procesar la Busqueda ...!!!','p_rptpcertep.php?vopc=1','N');   
...
if ($filas_found==0) {
     $smarty->display('encabezado1.tpl');
     mensajenew('Error: No existe el Nro. de Registro o Solicitud, o NO se encuentra en el estatus adecuado ...!!!','p_rptpcertep.php?vopc=1','N');   
     $smarty->display('pie_pag.tpl');
     $sql->disconnect();
     exit(); 
      } 
...
$resulbol=pg_exec("SELECT nro_derecho,documento FROM stzevtrd 
                    WHERE nro_derecho='$nderec' and evento IN ('2122') and (estat_ant='2101')");
...
if ($resulbol and $filas_bol>0) {
     ...
     if ($var_doc>=562) {
        ...
        $nameweb ="http://172.16.0.130/patentes/certificados/".$vdir."/".$vnsol.".pdf";
        $smarty->display('encabezado1.tpl');  
       ...
       mensajenew('Patente Concedida en Boletin '.$var_doc.' (anterior al 562). No posee Certificado Electr&oacute;nico!!! ','p_rptpcertep.php?vopc=1','N');
       $smarty->display('pie_pag.tpl'); $sql->disconnect(); exit(); }
} else {
   $smarty->display('encabezado1.tpl');
   mensajenew('Error al Procesar la Busqueda del Certificado...!!!','p_rptpcertep.php?vopc=1','N');   
   $smarty->display('pie_pag.tpl');
   $sql->disconnect();
   exit(); 
}
